...
- 预计进入代码修改、构建、测试或需要长期开发上下文时，必须先使用 `task-worktree` 声明完整 repository set，创建或复用 canonical task environment；随后用明确 target/workdir、membership 与 receipt-bound CLI/runtime identity 取得 `executionReady: true`。Agent session 可从 canonical Workspace 启动，不要求 session root 等于 environment root。
