...
`openspec-update-change` 只修订既有 planning artifacts，不授予实现、同步或归档权限。若本次修订将进入代码修改、构建或测试，先创建或复用 canonical task environment，并用明确 target/workdir、repository membership 与 checkout-local CLI 取得 `executionReady: true`；随后用 `openspec-apply-change` 进入实现。
