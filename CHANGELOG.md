...
- 新增工作空间（Workspace）总览，统一资源浏览、详情阅读与操作入口，并提供可复用的前端开发和界面原型（UI Prototype）技能。
- 扩展资源目录与主要页面的可用宽度，统一窄屏布局和信息区域展示。
- 新增项目服务组成维护能力，完善讨论中已有原型到任务实施及交付的接续指引。
- 解耦前端验证协作与正式验证记录，使用独立 Chromium 隔离浏览器回归，完善服务检查与项目验证声明。
- 修复启动器（Launcher）并发接管时等待启动锁释放后无人继续启动的竞争，完善候选验证（Candidate Verification）材料边界与超时诊断。

## 0.1.0-rc.36 - 2026-09-24

- 统一工作空间资源目录的登记与保留文件移除操作，完善技能登记、目录选择和资源删除体验。
- 建立结构化项目知识与主动探索入口，扩展知识主题导航、本机内容阅读和知识初始化流程。
- 修复连续编辑服务时误报版本冲突，并完善相关前端与接口验证。
- 收敛发布回读和启动器（Launcher）交接的时序判断，修正候选验证及公开材料中的路径和链接问题。
- 更新产品定位、首次使用说明及智能体（Agent）协作指引。

## 0.1.0-rc.35 - 2026-09-22

- 重构任务（Task）详情与实施清单，按工作路径组织专业记录，并优化任务列表加载、筛选重置与每日演进体验。
- 新增以日常关注为中心的工作台（Workbench），统一资源阅读、文章编辑与项目主页，并完善本地配置读取与对象删除。
- 建设可追溯的架构知识与维护流程，补齐数据领域解释、数据库实体关系图与应用内图示文件关联及阅读验收。
- 完善代码库（Repository）编辑与真实代码库管理，按真实根代码库归并自举服务并简化项目登记与知识阅读。
- 按 Qoder 真实支持面重设计运行时投射，收敛到单一 `.qoder` Skills 根，并按回执归属处理共享根清理。
- 将 Archify 绘图组件纳入工作空间管理，完善项目测试验证框架与收尾、验证登记指引。

## 0.1.0-rc.34 - 2026-09-16

- 解耦项目（Project）、服务（Service）与代码库实例（Repository Instance），支持共享服务关联和按分支隔离代码目录，统一相关管理界面。
- 优化工作空间（Workspace）标签页与自适应分屏，项目内服务在主屏标签页打开，副分屏默认均分信息区。
- 保留栏目浏览上下文，简化关联选择与抽屉交互，完善对象详情的连续浏览体验。
- 持久文件改动默认使用独立工作树（Worktree），并补齐从改动到交付的验证指导。

## 0.1.0-rc.33 - 2026-09-15
...
- 整理产品模块职责、生成代码与构建输出目录，保持开发入口、前端托管和发布包资源一致。
- 修复生产页面的依赖装配，恢复真实浏览器验证，补齐页面访问与操作回归。
- 修复开发版启动器（Launcher）的资源目录检查，并对齐安装与诊断行为。
- 将变更内容查询归入 OpenSpec，收窄模块依赖并分离运行时执行职责。
- 按实际改动执行自举同步，取消任务记录前置条件，并精简智能体（Agent）工作指令。

## 0.1.0-rc.31 - 2026-09-07

- 统一候选验证、发布演练与正式发布的产物消费配方，补齐无公开事实失败恢复、分片定点重跑和受保护发布后的收尾恢复。
- 完善干净检出候选环境准备，覆盖生成代码、测试上下文、前端工具链、`web-dist` 与固定 npm 消费路径，减少平台分片串行暴露准备缺口。
- 加固 macOS 与 Windows 启动器交接、单实例锁、工作树路径和测试工作进程预算，保留真实退出与所有者证据。

## 0.1.0-rc.30 - 2026-09-07

- 重构任务系统并收敛任务记录（Task Record）、任务审查（Task Review）、任务验证（Task Verification）、父子任务与工作树边界，删除旧任务环境、研发回执和收尾历史依赖。
- 支持百万级任务记录查询与滚动续载，简化 Buildr Web 任务界面并修复历史引用诊断误报。
- 将 Buildr 生产源码、工具与测试迁移到 TypeScript，生成 DTO、测试上下文和 Web 产物不再作为 Git 源资产，保持开发、候选版和 npm 包使用同一生成链。
- 收敛 Workspace Source、CLI、前端领域与产品知识边界，并新增代码架构指导和用户体验设计法则技能。
- 修复开发 Web 产物准备、预览失败回收、发布收尾清理，以及发布 owner 对当前 Task v3 读取端口的兼容性。

## 0.1.0-rc.29 - 2026-08-31

- 支持任务记录（Task Record）的状态、目标、范围及父子关系更正，保留原有阶段成果历史，并以已观察版本防止覆盖并发修改。
- 简化父子任务管理（Parent/Child Task Management），由智能体依据目标和真实成果组织协作；父任务完成继续要求明确授权与总体验收。
- 解除任务收尾（Task Finish）对旧候选、交接和收尾运行的依赖，独立保存交付、任务完成、自举激活与环境清理事实；修复数据库升级和已核验交付后的工作树清理。
- 将核心规则（Core Rules）内联到工作空间入口，完善产物（Artifact）中心协作原则与智能体软件（Agentic Software）的适用边界，提供智能体优先设计指引。

## 0.1.0-rc.28 - 2026-08-28

- 统一正式任务收尾与交付入口，收敛 Task Finish、交付对账与自举 Workspace 激活边界。
- 修正 Doctor 收尾投影与 Task Triage 预检断言，更新 Buildr Workspace 约定。
