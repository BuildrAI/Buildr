---
name: ui-preview
description: 用户已明确确认需要在正式前端开发前查看本次提案实施后的完整页面预演时使用；调查现有真实界面，生成并在浏览器验证一个或多个自包含 HTML。未明确确认时不得使用。
---

# UI Preview

本 Skill 只生成“界面预演稿（UI Preview）”：它把当前提案或设计中的 UI 变化放回系统现有完整页面中，供用户、设计师和后续 Agent 在正式开发前对齐预期。
...
只有当前对话中存在用户对本次任务“需要 UI Preview”的明确确认，才继续执行。拒绝、没有回答、只说继续任务或历史任务曾经需要预演都不构成确认；此时返回 `not-requested`，不创建占位文件、不写 waiver/Result/Receipt，也不阻塞原任务。

读取当前 Task 的 intent、Project/Service scope、关联 Change，以及适用的 proposal、design、delta specs 和 current knowledge。正式 Task 需要在 Buildr Web 中展示时，必须已有 Task 关联 OpenSpec Change；文件写入该 Change 内由当前任务决定的目录。不得为预演新增 Task Record 字段、数据库状态、descriptor/schema、CLI 或固定目录。
...
不要把调查笔记放进预演页面。若页面、运行环境或必要 UI 事实不可访问，先向用户说明缺失事实和影响，停止生成；不得凭空声称延续了现有产品。

## 3. 确定最小预演范围

按当前方案选择真正需要展示的页面和状态，不固定文件数量、目录或状态切换方式。只修改一个模块时，也必须把它放回现有导航、页面框架、相邻模块和真实信息层级中，交付修改后的完整页面，不能只交付孤立组件。

多页面或多状态只有在核心流程无法用一个页面说明时才增加。使用静态/mock 数据，不连接真实后端，不读取 secret，不执行真实写入。预演中的按钮、筛选、Tab、弹层或状态切换可以用本地 JavaScript 模拟关键交互。
...
- 在文件中包含精确发现标记 `<!-- buildr:ui-preview -->`；
...
使用当前 runtime 可用的浏览器能力直接打开每个 HTML 文件：
...
用户确认后，有设计师参与时把文件作为设计输入；没有设计师时，后续 Agent 可以加载完整文件作为正式前端开发的视觉与交互参考。任何确认选择需要进入正式行为时，仍应写回 design、delta specs、Brief 或 tasks 等对应 authority，不能只留在预演稿中。
