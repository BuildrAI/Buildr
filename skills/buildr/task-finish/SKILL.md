...
每步保存非空 fingerprint、effects、evidence、attempt timing 和失效依赖。`resume` 只重跑 blocked/stale 及其下游，不重复已证明的验证、push 或清理前动作。
...
确定性 OpenSpec 收敛由产品 action 内部完成并只返回 passed、blocked 或 recovery-unprovable。Agent 只处理语义冲突或人工事实核对，不恢复 canonical、不刷新 baseline、不选择内部 stage。
...
最终报告验证、交付、retained convergence、runtime、清理、receipt 和风险；分别计量 verification、repair、re-verification、closeout-only 与 end-to-end，不从不可观察区间推断 token 或调用数。
