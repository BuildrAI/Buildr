...
索引保留身份和关联，正文仍是独立文件。列表显式展开，不使用 YAML 锚点或别名；程序写入后按实际读取器校验格式。下面为最小示例，实际交付时替换为真实路径和已核对内容的 SHA-256：
...
- `observedDigest` 是人工或智能体（Agent）完成语义核对后记录的内容 SHA-256。缺省表示尚未核对；读取不会写回。变化后先核对表达，再更新相关观察，不仅为消除提示刷新摘要。
- 技术图（Technical Diagram）可声明 `graphSource` 与 `graphDigest`，核对图源及展示配对。维护后将生成展示与图源一并提交，图源摘要不能单独证明展示未被修改。
