...
|---|---|---|---|
| `implementation` | `buildr.task-worktree-lifecycle/v2` | 完整 repository set；在任何实现或 Change artifact 写入前创建或复用 canonical task environment，并在 environment-root Agent session 中取得 `executionReady: true` 的 adoption evidence | 只阻塞 execution |
