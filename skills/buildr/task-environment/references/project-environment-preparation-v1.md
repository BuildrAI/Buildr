...
Task选择使用`buildr.task-environment-plan-request/v1`，必须恰好覆盖Task Record中的全部Project/Service scope。每个scope选择当前声明中的Recipe identity，或明确`not-applicable`。Application在Task执行根解析声明并保存Plan v2快照；声明改变后旧Plan/Receipt会stale或blocked，Agent需重新提交选择。`inspect`不会升级声明或Plan。
