...
- `prepare` 同时承担首次准备和幂等恢复；没有单独 `restore`。
- `inspect` 只读返回当前机器的最新 probe。
- `cleanup` 只在 Task 已明确 abandon，或由 Task Finish 提交 durable handoff 时成立。普通 Agent 不绕过授权。

## 执行边界

取得 `ready` 后，只在结果的实际 execution roots / validation root 中写入、构建和测试，并使用结果指定的执行 CLI。Environment Receipt 独占 Runtime、CLI、依赖、projection、动态资源、ready、恢复和总 cleanup；Task Record 不保存这些字段。
