...
- 其他专业能力返回的closed `planRequest`可以带`auxiliaryPreparation`。Agent只能原样交给本Skill的`prepare --plan`；它引用同Project已登记Recipe，不进入Task scope、Change、Content Target、allowed execution roots或源码写入authority。Task Verification不再生成此类Plan；不得为跑测试自行把辅助Service加入Task Record，也不得手写安装命令。
