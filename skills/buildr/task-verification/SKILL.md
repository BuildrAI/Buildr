...
- consumer 声明 task environment 时，必须提供 environment receipt、repository membership/identity、allowed execution roots、checkout-local CLI/runtime projection identity 与明确 target/workdir；只有 context 返回 `executionReady: true` 才执行正式验证。不得要求 session root 等于 environment root。
...
- task environment evidence 还必须记录 execution binding 与 runtime projection identity；任一 environment、repository、target/workdir 或 CLI identity stale/mismatch 时 evidence 不可复用。Activation evidence 只在 Rules、Skills/runtime adapter 变更的专项验收中记录。
