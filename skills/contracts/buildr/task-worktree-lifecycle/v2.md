...
provider 必须为 environment 和每个 repository 返回可核对 identity，并将 environment/runtime readiness 与 Agent session adoption 分开。新 environment 必须返回 checkout-local runtime source/projection identity、adapter activation 与 `handoff-required`；consumer 必须让 Agent runtime 以 environment root 启动或重新进入 session，使用 `worktree adopt` 提交 host-visible session evidence，再用带相同 session root/handle 的 `worktree context` 取得 `executionReady: true`。只改变工具 `cwd`、文件投射成功或 doctor 通过不得视为 adoption。

采用完成后，artifacts、编辑、checkout-local CLI、构建、测试和验证 evidence 只使用 `allowedExecutionRoots`。原 Workspace、其他 environment、未登记仓库、不匹配 environment/runtime/session identity 必须 fail closed。Buildr 直接核验的 environment evidence 标为 `buildr-verified`；Agent/runtime host 提供的 session evidence 标为 `agent-attested`，不得描述为 Buildr 内省或密码学认证。Git working tree/index 隔离不得表述为 OS 沙箱；Git objects/refs/worktree metadata 是 shared。外部依赖沿用 Project 既有环境，不要求 Buildr 复制；只有并发任务可能修改同一共享状态时，consumer 才必须使用项目已有租户、账号、数据前缀、串行化或显式授权边界。
...
返回 task id、owner、environment root/state、receipt、repository plan/digest、每仓 selector/source/checkout/branch/HEAD/clean/state、`allowedExecutionRoots`、实际 membership、CLI source、runtime expectation/projection identity、adoption status/receipt、session root/handle/mode、assurance、`executionReady`、created/reused/blocked/retained/removed 状态、`treeChanged`、清理前置条件，以及 `source`、`gitMetadata`、`localRuntime`、`externalSystems` 隔离披露。

## Decision Points

repository set 无法消歧、canonical path 被占用、registry/remote/branch identity 冲突、当前 cwd 或自举任务 CLI source 越界、session root/handle/adoption mode 缺失或不匹配、runtime projection/adoption receipt stale、缺少验证/集成证据、任一仓库有未提交工作，或清理会影响其他 environment/入口/进程时，必须停止并保留整个 environment。清理 environment receipt 时必须同时删除其 Buildr-owned adoption receipt；合并后主 runtime 仍从 retained checkout sync。
