...
`WorkspaceStage` 使用可用舞台宽度计算主内容，默认上限 1440 像素；右侧先使用留白，空间不足或手动拖动后才压缩主内容。对象标签与页面标签视觉对齐；项目中的服务可在右侧阅读或显式进入主区域，服务目录仍直接进入服务主页。浏览器存储缺失、损坏或不可写只失去偏好恢复，不阻断浏览。旧全局像素宽度不再读取。数据新鲜度仍由页面既有刷新和写入版本校验负责，隐藏页面保留不代表数据始终最新。
