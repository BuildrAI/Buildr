...
- 全局壳层采用顶部区域与左侧导航：顶部依次呈现共同工作空间范围、“工作台”“工作空间”，工作台在前。工作台提供概览、任务和动态入口，默认进入工作概览；工作空间左侧为项目、服务、技能、设置四个平级行式入口，不展开项目树。工作空间内容区采用双栏组页签模型：常驻页面级标签页（Tab）条承载项目、服务、技能、设置和领域全景（可关闭、可拖动排序、自适应限宽居中），右组对象级页签条就地展开文档（页内对照、全关退场），两组之间贯连分隔线可拖调宽，手动比例按工作空间保存到浏览器本地存储（localStorage），窗口尺寸裁剪不回写偏好；修改与交给 Agent 动作统一走右侧抽屉。手机通过“打开导航菜单”使用同一导航，桌面窄窗口仍并排并允许拖动，极窄手机窗口上下排列。任务列表与详情继续并排；项目编辑入口仍在详情右上角。既有任务地址保留。技能页支持搜索、来源筛选和详情抽屉，按需只读技能说明与附属文本；维护动作生成智能体指令，不直接安装、启停或同步。壳层读取 sibling `buildr` 的只读Release Awareness API，在顶栏下展示GA/RC更新；用户可以复制精确`buildr update --track stable|candidate`命令，或把同一选择交给Agent 。首版不从网页执行npm更新，也不替用户决定轨道。
...
- 工作概览的“等我回应”只展示已登记的决定、验收或补充信息请求，与普通待办分开；没有请求时显示紧凑空态。回应沿用任务工作摘要（Work Context），不自动执行或完成任务（Task）。
- 项目主页的“项目动态”、工作概览摘要和动态菜单统一进入动态页。指定项目与日期时复用 `DailyProgressPanel` 完整展示本机 v2 文件的四问摘要与提交（不展示变更文件列表；`files` 仍由命令行（CLI）`inspect` 返回），支持切换日期和按日/人/任务分组；项目、日期和分组保存在网址（URL）中。按任务只聚合已关联的自己的提交。旧 `?document=daily` 地址兼容跳转，缺省日期沿用本机今天；生成指令携带当前项目和所选日期。页面只读且不扫描 Git，缺少内容时可交给智能体（Agent）基于明确提交范围生成，不要求先同步代码或资产。任务（Task）详情概览不展示每日演进反向关联。详见[项目每日演进](../flows/project-daily-progress.md)。
...
`src/app/WorkspacePages.tsx` 维护已访问页面的路由出口与位置上下文；隐藏页面保留挂载、筛选、滚动和草稿，关闭页面或切换工作空间时释放，不按隐藏时长回收。`pageTabs.tsx` 提供同一壳层上下文；`PageTabStrip.tsx` 管理排序反馈和键盘操作；`workspace-pages.ts` 校验存储中的工作空间路径、排序与分屏尺寸。技能、设置与项目、服务使用同一标签条，最后一个标签关闭后回到项目目录。仅访问过的页面挂载，恢复的其他标签按点击加载。

`WorkspaceStage` 使用可用舞台宽度计算主内容，默认上限 1440 像素；没有已保存手动比例时，左右并排的两组默认均分信息区可用宽度（扣除分隔线）；拖动后优先恢复该工作空间的手动比例，关闭副分屏后主屏恢复全部宽度。对象标签与页面标签视觉对齐；项目中的服务与服务目录中的服务均直接进入主屏服务主页标签页（Tab），已有服务标签按所属项目与服务身份复用，原项目主页现场保持；文档仍在右侧展开。浏览器存储缺失、损坏或不可写只失去偏好恢复，不阻断浏览。旧全局像素宽度不再读取。数据新鲜度仍由页面既有刷新和写入版本校验负责，隐藏页面保留不代表数据始终最新。
