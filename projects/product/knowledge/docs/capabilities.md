...
本文提供当前能力的简短定位和阅读入口，不复制详细解释。产品定位和解释见 [Buildr Product](overview.md)，规范行为以 [canonical specs](../../openspec/specs/) 和 active Change delta specs 为准。
...
- OpenSpec：在 Product Project 管理 proposal、design、delta specs、tasks、当前变更材料与归档历史。
...
- 项目每日演进：每个已登记 Project 按本机日历日保存 Git 提交驱动的四问摘要，权威是被 Git 忽略的 `.buildr/daily-progress/<project-code>/<YYYY-MM-DD>.yml`。Agent 默认按明确本地引用收集当日提交与更改文件、对比 `git config user.email` 后写入，并在摘要中注明引用、截至时间与未覆盖范围；需要远端最新信息时按授权独立获取，不要求更新工作目录、同步资产或通过全局诊断。自己的提交可挂 0..N 个本机 Task，他人提交禁止挂 Task。Buildr Web 的项目主页通过“项目动态”进入统一动态页；工作概览和动态摘要按同一项目、日期打开完整只读详情，并保留日期及分组。它不是当前认知、Task Record 或跨机器共享数据；产品读取路径不扫描 Git，也不内置 cron。
