...
- 项目每日演进：每个已登记 Project 按本机日历日保存 Git 提交驱动的四问摘要，权威是被 Git 忽略的 `.buildr/daily-progress/<project-code>/<YYYY-MM-DD>.yml`。Agent 默认按明确本地引用收集当日提交与更改文件、对比 `git config user.email` 后写入，并在摘要中注明引用、截至时间与未覆盖范围；需要远端最新信息时按授权独立获取，不要求更新工作目录、同步资产或通过全局诊断。自己的提交可挂 0..N 个本机 Task，他人提交禁止挂 Task。Buildr Web 在项目详情「每日演进」Tab 只读展示。它不是当前认知、Task Record 或跨机器共享数据；产品读取路径不扫描 Git，也不内置 cron。
