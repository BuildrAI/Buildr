...
已核对结构重放、图源生成、脚本语法与相对链接，并在浏览器检查字体、关系连线、缩放上下限、搜索命中与空结果、下一项及重置。实体关系图（ERD）在 1440×900、1920×1080、2048×1320 下无页面横向溢出；图内按原尺寸滚动，不以缩小到不可读代替完整字段展示。产品知识读取器已验证正文和图源可读；已在 Buildr Web Dev 从“项目知识”检索并打开主文，验证文内关系图的对照阅读、字段搜索、缩放和重置。应用内图源与依据从外层“图示文件”打开；独立 HTML 中的相对文件链接用于本机文件或静态托管阅读。
