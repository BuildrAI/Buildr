...
本文约束 Buildr Product 当前代码组织。规范说明可观察行为；本文和[全项目代码地图](../../code-map/README.md)说明行为当前落在何处。
...
完整表见[关键调用、数据与副作用](../../code-map/calls-data-effects.md)。必须保持的核心不变量是：
...
6. [全项目代码地图](../../code-map/README.md)与 Archify 技术图是否仍对应真实文件和调用。

本次只交付当前地图与技术图，不建设自动发现、自动更新、自动生成或自动同步机制。

## 业务服务与代码库实例

覆盖：工作空间管理产品模型，依据 `workspace/domain/asset-relationships.ts` 和关系应用。上述 Buildr 自身源码布局仍是当前物理事实，不能据此假定用户已迁移目录。

项目（Project）引用服务（Service），服务以单个 `repositoryId` 引用代码库实例（Repository Instance）。同一实例可以支撑多个服务；仓库地址相同不合并稳定身份。集成分支属于实例，当前分支属于观察态。全局清单经版本保护的事务维护；UI 保存来源不触发克隆。旧项目服务定位通过明确别名兼容，关系解除不删除历史任务事实。

代码库位置必须对应真实 Git 根目录，工作空间根可声明为 `.`；服务子目录用 `modulePath` 表达。`repositories/` 仅为新代码的默认目录，已有内部或外部仓库不搬迁。旧 workspace 子目录经显式规范化按实际根归并，服务身份保留。项目、服务及未被服务引用的代码库使用“移除”取消登记，文件和历史任务保留。服务新增直接选择 projects/*/services/ 下未登记目录或在所选项目下新建，系统解析 repositoryId/modulePath；已有目录不补写文件。技能移除也只取消清单登记，保留源目录，组件受管边界保持。独立服务、代码库列表共享声明逻辑；Git 状态单独按需观察，目录存在不等于仓库身份已验证。

代码库声明可在页面完整编辑；保存只写清单，不更改真实 Git 远端、当前分支或目录内容。已有目标核对真实根和模块边界，缺失目标及模块显示待对齐；当前分支不同于集成分支（Integration Branch）本身不是身份错误。本地仓库可独立设置集成分支；远端地址为空表示撤销远端声明，既有 Git 配置仍保留。

管理页面按需读取本地 Git 配置作为实际状态。没有远端声明不代表本地没有配置；详情展示实际值及不同的声明目标，编辑补入可确定的缺失地址但不覆盖草稿，读取本身不写回清单。多远端无可靠选择时展示候选，由用户明确选择。
