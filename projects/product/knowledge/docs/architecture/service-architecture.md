...
代码库位置必须对应真实 Git 根目录，工作空间根可声明为 `.`；服务子目录用 `modulePath` 表达。`repositories/` 仅为新代码的默认目录，已有内部或外部仓库不搬迁。旧 workspace 子目录经显式规范化按实际根归并，服务身份保留。删除项目或服务只移除登记与引用，文件和历史任务保留。独立服务、代码库列表共享声明逻辑；Git 状态单独按需观察，目录存在不等于仓库身份已验证。
