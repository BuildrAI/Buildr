...
收尾与交付在日常对话中可以表示同一目标：结束本轮工作，把成果交到约定位置，登记已有任务结果，处理可安全处理的临时资源，并交代遗留事项。用户明确要求“收尾”时使用 `task-finish`，该指令已授权当前任务范围内常规 Git 提交、集成和普通推送，不再逐项索取同一授权。
...
写入前检查对象、归属、版本、目标、完整推送范围与实际副作用；写入后回读真实结果；删除前检查资源归属、未保存内容以及成果仍被保留。范围或目标不明时停止受影响的写入；强推、共享历史改写、远端删除、丢弃内容或语义冲突取舍仍需额外授权。业务请求成功不等于业务写入成功。
