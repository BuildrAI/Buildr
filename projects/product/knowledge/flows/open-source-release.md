...
源码分片和产物生产尽量并行；消费者只等待真正必需的产物。矩阵采用收集全部失败策略。作业阶段记录准备或测试失败；聚合将证据缺失与已执行测试失败分开，依赖不满足而未运行的作业保留其实际状态。上传只包含唯一包与必要清单，构建中间目录不作为发布消费者输入。
...
保留当前 300 秒集成步骤上限和 60 秒 Launcher 交接等待；根据准备、执行、内部等待和进程回收证据判断长尾，不通过无限扩大超时掩盖死锁或泄漏。网络请求有单次截止时间和有限重试；没有断言失败并不能证明超时是暂态。测试内层工作进程必须遵守外层分配的预算，已完成等待的计时器必须取消，异常退出保留具体退出码。

启动器的单实例启动复用既有原子文件锁。完整锁记录先写入再公开；未完成或未知所有者的锁不会被抢走。旧进程号格式保持可读，只有确认原进程已退出才回收；释放时核验所有权，不删除继任者的锁。
