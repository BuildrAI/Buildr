...
图源和展示覆盖 Buildr 服务（Service）中技能源文件到目标目录的主要数据关系，以 Codex 单目标根与 Qoder 主根加镜像根的工作空间（Workspace）输出作为例子。它是数据流图（Data Flow Diagram），不表达逐函数执行顺序，也不表示新增持久对象。下面说明覆盖 JSON 与 HTML 两个同名文件的事实来源及表达边界。
...
| `runtime-files`、`write-runtime` | `adapter-contract.ts` 的 `skillDestinationRoots` 给出主根与镜像根，`buildRuntimeSkillDirectory` 逐根使用 `getRuntimeAdapter` 的布局；协调器写入实际改变的受管目标 |
| `ownership`、`write-ownership` | `buildSkillProjectionReceipt` 按根构造本机记录（镜像根文件名带 `--root-<slug>` 并在记录内写明 `runtimeRoot`），由计划的记录写入分支保存；目录内容通常先于标记为 `commitLast` 的记录 |
