...
- Task Record固定组织为`src/features/task-record/{pages,hooks,components,api}`。`api/task-record-api.ts`消费Task-owned JSON Schema生成到`api/generated/`的ignored DTO并复用全局HTTP/session/Workspace transport；页面只组装Hook与组件，Hook管理请求、mutation、取消和竞态，组件不直接调用后端。`src/api`不得反向依赖Task feature。Buildr Web不安装Ajv、不拥有Schema或Application authority；构建入口先生成两端DTO，再通过重复生成、typecheck、正式build与Task Browser Smoke验证。
