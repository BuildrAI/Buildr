...
- 有有效 `verification.yml` 时，checkout 或 npm 安装后的 `buildr verification run` 提供正式产品 executor：它按声明 DAG 并发运行能力，以 task/environment/run/token 绑定的 Git common-dir lease 协调跨任务共享容量，并返回 `buildr.verification-run/v1` 候选 evidence。Buildr 只执行调用方启动的 run，不创建或调度 Agent 任务；Task Finish 可核对同一 evidence，候选或 policy 变化后才重新执行。
...
- `task-finish` 是 `buildr.task-finish/v1` 的薄入口。ready Task Environment 中，产品通过单一 `task finish run --task <task-id> --project <code> [--change <id>]` 以 Task 为主身份区分 Change/code-only 候选，并固定执行 `preflight → prepare → verify → deliver → cleanup`；code-only 明确跳过 Change/OpenSpec 专属动作，其余冻结、Node identity、required assurance、target fencing、retained convergence 与 cleanup 保证一致。产品缺陷、语义冲突和验证失败退出收尾并回到研发流程；Agent 不提交 caller-authored 协议。retained metadata-only 不进入 dirty tree 产品 executor，只在精确任务 paths、验证 identity 与目标 ref 可证明时由 Skill 交接 optional selected Git 单项 provider并保留无关改动。客户端只公开 `run|inspect`，不提供旧 action 或兼容执行器。
