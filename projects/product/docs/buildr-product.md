...
- `task-verification` 是 `buildr.task-verification/v3` 的默认 provider。Project 只使用 `buildr.project-verification/v3` 声明既有 capability family 的 scope、proves、evidence、usable targets、discovery、affected/full/provider入口和执行边界；Request 形成内容寻址Plan，正式Execution Record必须绑定matching Plan。声明缺失或能力不存在只形成 coverage gap，Verification 不自动开发测试。
