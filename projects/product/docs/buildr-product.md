...
- 实现型 OpenSpec Change 在 propose 前取得 matching ready Task Environment；artifacts、实现和合并前候选验证只写 Environment 允许的根，不与 retained source 双写。候选 Skill、CLI、功能和 runtime 可以在自身 Task Validation Workspace 测试，只有集成到 retained source 后正式 runtime 才同步生效。
...
- 每个正式 Task 在 `.buildr/tasks/<task-id>/verification.yml` 维护一份 closed `buildr.task-verification-result/v1` current Result。唯一 Task Verification Application 自行观察 declaration identity，整值原子替换 Result，并在读取时按 target/declaration identity 派生 `current / stale / unknown`；Result 只保存执行能力的 `passed / failed` 精炼事实、coverage gaps 和 `passed / not-passed` 结论，不保存 stdout/stderr、Environment Receipt、风险接受、推进决定或 Candidate generation。CLI、Skill、Local App 和临时 Finish consumer 复用该 authority。
...
- Buildr Product 的 delivery-required `product.delivery` capability 绑定明确 target identity；它在冻结 Candidate 上按 changed owner 选择 affected 或 full 证据。commit、相同 tree 集成、push 和 worktree 清理不改变该 identity 时可以复用 current Result，tree 或声明变化后 Result 派生为 stale。
- `task-finish` 是 `buildr.task-finish/v1` 的薄入口。ready Task Environment 中，产品通过单一 `task finish run --task <task-id> --project <code> [--change <id>]` 固定执行 `preflight → prepare → verify → deliver → cleanup`。verify 只通过 Task Verification Application 读取 current Result；单 Project 的 delivery-required command capability 可由临时 adapter 按 scope/path 保守选择并执行一次，自然语言 conditions 不用于跳过；多 Project、Agent capability、显式授权或需要语义排除时必须已有正式 Result。验证失败退出收尾，Finish 不写 `proceed / blocked`、不创建 Candidate generation 或接管 Task 状态。retained readiness 按既定 Workspace Metadata Store 边界排除未 staged 的 `.buildr/**`，源码/文档 dirty 或 staged metadata 仍阻塞；Finish 不承担 Metadata Publication。retained metadata-only 不进入 dirty tree 产品 executor，只在精确任务 paths、验证 identity 与目标 ref 可证明时由 Skill 交接 optional selected Git 单项 provider 并保留无关改动。客户端只公开 `run|inspect`，不提供旧 action 或兼容执行器。
- `task-asset-review` 是 optional `buildr.task-asset-review/v3` provider。非简单 Workspace 任务从探索、设计、诊断、实现或验证阶段开始后，Agent 按高信息量节点把精炼信号写入 canonical Workspace 的 untracked `.buildr/asset-review/inbox/`；主 checkout 与 linked task worktree 共享同一 inbox，但不同物理 checkout 不通过用户级状态合并，每个任务拥有独立 observation 文件。provider 自己完成资格审查、覆盖核验和 Rule、Skill、capability Contract、product follow-up 分类；没有候选时直接 discard，Task Finish 只触发 finalize 并等待人工 accept/reject，不拥有门禁。accept 必须进入 identity 不同的新 task-triage，完成时按 outcome 核验维护记录与 Git 集成、OpenSpec artifact 或 no-change 结论证据。只有新任务实际修改前三类资产时才随修改提交 `asset-maintenance/<type>/<asset-id>/records/` 历史；product follow-up 由 OpenSpec 吸收来源。该能力不保存完整对话或轨迹，不引入公共 CLI、Hook、daemon、watcher、数据库、复杂锁或 `asset.yml`。
