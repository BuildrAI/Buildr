...
- `task-finish` 是 `buildr.task-finish/v1` 的薄入口，产品通过单一 `task finish run` 固定执行 `preflight → prepare → verify → deliver → cleanup`。preflight 一次聚合廉价问题，prepare 完成全部候选 mutation 并冻结 identity，verify 对冻结候选最多执行一次 required assurance，deliver 与 cleanup 只恢复 target/network/retained/本地清理等暂态阻塞。产品缺陷、语义冲突和验证失败退出收尾并回到研发流程修正当前实现候选；Agent 不提交 evidence、fingerprint、repair authorization 或 recovery manifest。客户端只公开 `run|inspect`，不提供旧 action 或兼容执行器。
