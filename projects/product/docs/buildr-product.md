...
- Buildr Product 的 delivery-required `product.candidate` capability 绑定明确 target identity；commit、相同 tree 集成、push 和 worktree 清理不改变该 identity 时可以复用 current Result，tree 或声明变化后 Result 派生为 stale。
