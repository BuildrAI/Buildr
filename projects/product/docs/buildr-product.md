...
当前本地产品通过 `buildr app` 启动或复用只监听 loopback 的全局本机 Web 应用，并在默认浏览器中提供工作空间（Workspace）、项目（Project）、服务（Service）、任务（Task）与变更（Change）管理视图。用户级登记列表只保存 Workspace root 和最近使用项；Workspace identity、metadata 与下属资源仍由各 Workspace 文件实时提供，不建立跨 Workspace 第二事实源。页面复用与 CLI 相同的应用用例：Task Record 可以创建、编辑、完成和放弃；Task 详情的独立“环境”“审查”“验证”页签分别只读调用 Task Environment、Task Review 与 Task Verification Application，展示各自 current read model 和受限 Agent action，不提供 Environment prepare/cleanup 或 Result CRUD。全局 Change 视图保持 retained-only；从 Task 打开关联 Change 时按 matching Task Environment execution root 与 retained baseline 分开显示 provenance，其审查按钮进入同一 Planning Review action。
...
- `task-development` 是 `buildr.task-development@1` 的默认 provider，也是 Development Receipt、Content Target、verification policy、Task Candidate/generation、gates/decision 与 immutable handoff 的唯一 Application authority。正式顺序固定为 ready Environment → Planning Review → stable Content Target/policy → Formal Verification → Candidate freeze → Completion Review → `proceed / blocked` → handoff。Verification `not-passed`、coverage gap 或 Completion `changes-required` 不阻止 freeze，但只有绑定当前 Result digest、当前 Task scope 与明确来源的用户风险接受才允许 `proceed`。OpenSpec 是 `0..N` 个可选关联；通用 Application 不硬编码 Product、Git、Node/npm、worktree、OpenSpec 或测试框架。第一版只提供 bundled Skill 与 internal driver，不公开 Development CLI 或 Local App surface。
- `task-finish` 是 `buildr.task-finish/v1` 的窄 handoff adapter。ready Task Environment 中，产品通过单一 `task finish run --task <task-id>` 固定执行 `preflight → prepare → verify → deliver → cleanup`：从 Development Application 取得 current immutable handoff，准备内容等价 Delivery Carrier，verify 只证明 carrier 等价且 Formal Verification execution count 固定为 0，随后普通 fast-forward/push、retained sync/install/doctor 与 Environment cleanup。Finish 不接受 Project/Change、Candidate/generation 或 Verification authority 输入，不收敛 Change/current knowledge、不修改内容、不运行 Formal Verification/Completion Review、不生成 Candidate 或风险决定；target race、内容或 handoff 漂移终止 run 并返回 Development。run/result 使用 breaking v2 schema且不新建 Finish Receipt。retained metadata-only 只在精确任务 paths、handoff identity 与目标 ref 可证明时，由 Task Finish 把 commit/push 作为两个已选 operation 交给 optional Git Operations provider。
