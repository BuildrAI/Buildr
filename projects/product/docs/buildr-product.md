...
当前本地产品通过 `buildr web` 启动或复用只监听 loopback 的全局本机 Web 应用，并在默认浏览器中提供工作空间（Workspace）、项目（Project）、服务（Service）、任务（Task）与变更（Change）管理视图。Buildr Web 是任务记录（Task Record）的观察与有限维护客户端：正式 Task 由 Agent/Task Manager 创建，页面只允许编辑、完成和放弃已有 active Task。任务详情固定为“概览、原型、证据、复盘、环境”五个一级视图；概览只组合Task Record、Review、Verification与Environment摘要，不展示研发聚合或旧机器交付历史。“证据”分别调用Task Review与Task Verification，“复盘”维护Task Retrospective处置，“环境”只读Task Environment。全局Change保持只读，Agent依据当前Change artifacts和真实现场继续工作。
...
- 选择Buildr-managed OpenSpec路径时，实现型Change在propose前取得matching ready Task Environment；artifacts、实现、开发期测试和Formal Verification只写Environment允许的根，不与retained source双写。
...
- `task-environment` 通过 `buildr.task-environment/v1` 管理正式 Task 的 prepare/inspect/cleanup；公共 CLI 不暴露内部资源 mutation。Preview 等已知 provider 在健康后登记为 Task-owned 动态资源，登记失败立即回收；Environment根据已完成或明确放弃的任务以及当前内容保全事实停止资源并调用provider。
