...
当前本地产品通过 `buildr app` 启动或复用只监听 loopback 的全局本机 Web 应用，并在默认浏览器中提供工作空间（Workspace）、项目（Project）、服务（Service）、任务（Task）与变更（Change）管理视图。用户级登记列表只保存 Workspace root 和最近使用项；Workspace identity、metadata 与下属资源仍由各 Workspace 文件实时提供，不建立跨 Workspace 第二事实源。页面复用与 CLI 相同的应用用例：任务记录（Task Record）可以创建、编辑、完成和放弃；任务详情固定为“概览、研发、证据、环境”四个一级视图。“研发”只读调用 Task Development Application `inspect`，展示当前结论、候选、门禁、决定与最近保存的研发交接；“证据”分别调用 Task Review 与 Task Verification reader，任一读取失败不隐藏另一份证据；“环境”继续只读调用 Task Environment Application。页面不提供 Development mutation、Environment prepare/cleanup 或 Result CRUD。全局 Change 视图保持 retained-only；从 Task 打开关联 Change 时按 matching Task Environment execution root 与 retained baseline 分开显示 provenance，其审查按钮进入同一 Planning Review action。
...
- 每个正式 Task 在 `.buildr/tasks/<task-id>/verification.yml` 维护一份 closed `buildr.task-verification-result/v1` current Result。唯一 Task Verification Application 自行观察 declaration identity，整值原子替换 Result，并在读取时按 Content Target/declaration identity 派生 `current / stale / unknown`；Result 只保存执行能力的 `passed / failed` 精炼事实、coverage gaps 和 `passed / not-passed` 结论，不保存 Candidate/generation、stdout/stderr、Environment Receipt、风险接受或推进决定。CLI、Skill、Local App 和 Task Development consumer 复用该 authority；Task Finish 不读取或发起 Verification。
...
- `task-review` 通过唯一 `buildr.task-review/v1` capability 执行方案审查或完成审查。一个 closed Result 模型在 `.buildr/tasks/<task-id>/reviews/planning.yml|completion.yml` 维护两个可选 current 槽位；每份 Result 绑定明确目标 identity，并如实记录执行方式、reviewed/uncovered、findings 和结论。同类型完整替换、跨类型隔离，中断不覆盖旧 bytes；读取时比较目标派生 `current / stale / unknown`。Result 不持久化 revision、history、current、applicability 或 digest，也不生成 Candidate、替代 Verification 或建立门禁。语义 Skill 执行 Review，Application/CLI 只确定性记录；`task-asset-review` 仍是独立资产维护能力。
...
- `task-development` 是 `buildr.task-development@1` 的默认 provider，也是 Development Receipt、Content Target、verification policy、Task Candidate/generation、gates/decision 与研发交接（Development Handoff）的唯一 Application authority。正式顺序固定为 ready Environment → Planning Review → stable Content Target/policy → Formal Verification → Candidate freeze → Completion Review → `proceed / blocked` → handoff。Verification `not-passed`、coverage gap 或 Completion `changes-required` 不阻止 freeze，但只有绑定当前 Result digest、当前 Task scope 与明确来源的用户风险接受才允许 `proceed`。OpenSpec 是 `0..N` 个可选关联；通用 Application 不硬编码 Product、Git、Node/npm、worktree、OpenSpec 或测试框架。第一版提供 bundled Skill、internal driver 与 Local App 只读 `inspect` 投影，不公开 Development CLI、写 API 或浏览器 mutation。
- `task-finish` 是 `buildr.task-finish/v1` 的窄研发交接适配器。ready Task Environment 中，产品通过单一 `task finish run --task <task-id>` 固定执行 `preflight → prepare → verify → deliver → cleanup`：从 Development Application 取得当前不可变研发交接，准备内容等价的交付载体（Delivery Carrier），verify 只证明 carrier 等价且 Formal Verification execution count 固定为 0，随后普通 fast-forward/push、retained sync/install/doctor 与 Environment cleanup。Finish 不接受 Project/Change、Candidate/generation 或 Verification authority 输入，不收敛 Change/current knowledge、不修改内容、不运行 Formal Verification/Completion Review、不生成 Candidate 或风险决定；target race、内容或 handoff 漂移终止 run 并返回 Development。run/result 使用 breaking v2 schema且不新建 Finish Receipt。retained metadata-only 只在精确任务 paths、handoff identity 与目标 ref 可证明时，由 Task Finish 把 commit/push 作为两个已选 operation 交给 optional Git Operations provider。
...
- `task-asset-review` 是 optional `buildr.task-asset-review/v3` provider。非简单 Workspace 任务从探索、设计、诊断、实现或验证阶段开始后，Agent 按高信息量节点把精炼信号写入 canonical Workspace 的 untracked `.buildr/asset-review/inbox/`；主 checkout 与 linked task worktree 共享同一 inbox，但不同物理 checkout 不通过用户级状态合并，每个任务拥有独立 observation 文件。provider 自己完成资格审查、覆盖核验和 Rule、Skill、capability Contract、product follow-up 分类；没有候选时直接 discard。存在 observation 时由 Task Development 在正式 handoff 前 finalize，Task Finish 不读取 observation 或触发 finalize。accept 必须进入 identity 不同的新 task-triage，完成时按 outcome 核验维护记录与 Git 集成、OpenSpec artifact 或 no-change 结论证据。只有新任务实际修改前三类资产时才随修改提交 `asset-maintenance/<type>/<asset-id>/records/` 历史；product follow-up 由 OpenSpec 吸收来源。该能力不保存完整对话或轨迹，不引入公共 CLI、Hook、daemon、watcher、数据库、复杂锁或 `asset.yml`。
