...
| 24. 对齐 development onboarding 与 npm CLI 隔离 | Windows旧owner在约102.917s准备后才因POSIX `install-buildr-cli` 返回`ENOENT`。实现删除PATH installer lifecycle，以显式Project bridge、双sentinel和共享Git object candidate snapshot保留clean checkout/sync/Launcher/Doctor证据；三次本地成功focus为15.507s、15.659s、15.572s，中位数15.572s、范围0.152s，完整changed资源竞争下为22.115s且185.121s DAG全绿。测试本体已先优化，稳定必要链路及并发波动证明15s/6s预算与调度成本均不现实，因此非阻断target校准为25s、调度成本20s；最终hosted Windows timing只绑定archive后的Formal Task Content Target，不重复跑预归档SHA。 |
