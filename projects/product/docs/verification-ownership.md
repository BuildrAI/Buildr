...
1. `public-json-contracts` 的剩余成本来自公开 Doctor/JSON 子进程；先按输出事实审查唯一 owner，再决定是否能合并只读扫描，不用 Application fixture 掩盖公共入口成本。
2. `worktree-create` 剩余约 24 秒主要是唯一的共享占用恢复与 Git-backed Environment lifecycle；只有产品 Environment prepare/cleanup 自身存在重复工作时再优化，不把唯一 System 事实下沉成 fake。
