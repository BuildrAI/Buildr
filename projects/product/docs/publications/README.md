...
- 先在本目录完成写作、审校和版本修改，再发布到外部平台。
- 同一篇文章可以对应多个发布目标，不按平台拆分目录。
- `status` 表示文章源的状态，`targets` 表示各平台的发布状态。
- 已发布文章保留原始平台链接；后续修改应在项目文件中完成，再决定是否同步更新平台内容。
- 本目录暂不承载产品规范、当前实现事实或内部研发过程文档。
