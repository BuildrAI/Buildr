...
- Buildr Web 的文章编辑直接维护本目录内同一份 Markdown 文件。保存和删除使用当前文件版本，遇到外部修改时先对照最新内容。
- 图片和附件放在 `assets/`，正文使用 `![说明](assets/文件名)` 或 `[附件说明](assets/文件名)` 引用。编辑器可查看既有资源、上传并插入引用；上传成功后仍需保存正文。
- 上传使用唯一名称，保留已有同名文件；删除文章不自动删除可能被其他文章共用的资源。
