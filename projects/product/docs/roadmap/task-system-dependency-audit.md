...
**整体重构仍在进行：收尾、父子管理、审查和验证已独立；Task Development、Planning Identity与旧Finish历史已退役。** 第 1—3 节保存初次审查，只用于解释历史问题，不能代表当前架构。当前任务架构以[任务生命周期架构](task-lifecycle-architecture.md)为准。

**先看系统有哪些部分，再看谁依赖谁，最后决定先讨论哪一组。** 下面第一张图是现状，第二张图是可调整的重构顺序；两者不是同一件事。

## 当前总计划

`refactor-task-system` 持续组织整体目标，当前状态和真实子任务关系以任务记录为准。已交付成果保持完成，剩余工作先保留在计划中，选定独立目标后再建立子任务。

| 部分 | 当前定位 |
|---|---|
| 收尾与设计方法 | 已交付，后续按实际案例修正 |
| 父子任务管理 | 已交付，完成授权与独立成果边界已建立 |
| 清理与补测边界 | 已交付，原工作树遗留已处理 |
| 任务记录更正 | 补齐状态更正、历史保留与终态关系修订 |
| 研发组织（Task Development） | 已整体退役；Agent直接组合真实现场、专业Skill与现有工具 |
| 任务审查（Task Review） | 已独立维护可选的方案或完成结果证据 |
| 任务验证（Task Verification） | 已独立保存实际验证报告，不成为统一完成门禁 |
| 环境、工作树与执行记录 | 根据上层真实需要继续收敛，不预设全部重写 |

总任务只在已确定的整体改造范围交付、遗留明确处置并取得用户明确完成授权后结束，不因当前一批子任务结束而缩小目标。

## 当前依赖（2026-09-02）

```mermaid
flowchart TB
  H["人<br/>目标、判断、授权、验收"] --> A["智能体（Agent）<br/>读取现场并选择Skill与工具"]
  A --> R["任务记录（Task Record）<br/>目标、范围、关系、结果"]
  A --> E["任务环境（Task Environment）"]
  A --> D["任务研发（Task Development）"]
  A --> V["任务审查（Task Review）"]
  A --> Q["任务验证（Task Verification）"]
  A --> F["任务收尾（Task Finish）"]
  A --> G["Git、文件、代码、测试与外部系统"]
  P["父子管理（Parent Management）"] --> R
  P -.只读历史.-> L["Task-owned旧Parent Plan"]
  O["Buildr Web / Overview"] -.分别读取.-> R
  O -.分别读取.-> D
  O -.分别读取.-> V
  O -.分别读取.-> Q
  O -.分别读取.-> E
  T["Terminal Delivery"] --> R
  T -.只读历史.-> F
```

结论：没有统一`task next`、统一`proceed/blocked`或跨专业writer。各专业Application只保护自身写入；Agent依据目标、Skill和现场组合动作。能从Git、文件、测试或外部系统重新观察的事实不复制到Task系统。

## 1. 当前系统的整体架构

以下为首次审查时的历史结构，不是当前依赖图。
...
后续父子管理重构见[完整历史梳理](../archive/2026-08-30-parent-child-task-audit.md)和[新产品架构](../../openspec/knowledge/flows/parent-child-management.md)。总任务状态代表整体目标；阶段成果完成不能自动结束总任务。
...
## 4. 第一阶段记录：极简收尾与方法沉淀（已完成）

当前实现职责集中见 [任务收尾](../../openspec/knowledge/flows/task-closeout.md)。前文架构图是审查时基线，不能替代该实现说明。

总体父任务（Parent Task）记录为 **“任务系统渐进重构”**，标识 `refactor-task-system`；第一阶段由 `simplify-task-closeout` 和 `capture-agent-first-design` 两个子任务承接；当前总任务继续覆盖整体路线。当前任务状态以任务记录为准，本报告保存审查、决定和成果导航。本节取代此前“先建设多类接口和应用”的建议，第 1—3 节仍保留为现状及依赖参考，不是本轮建设清单。
