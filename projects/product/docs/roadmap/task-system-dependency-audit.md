...
总体父任务（Parent Task）记录为 **“任务系统渐进重构”**，标识 `refactor-task-system`，当前进行中（`active`）；已创建 `simplify-task-closeout`（实施中）与 `capture-agent-first-design`（待首项实际交付后推进）。本节取代此前“先建设多类接口和应用”的建议，第 1—3 节仍保留为现状及依赖参考，不是本轮建设清单。
...
>
> 智能体（Agent）依据现场选择、组合和调整工具，持续推进收尾；软件不预先规定所有场景必须经过同一条流程。
>
> 组合方式开放、事实可以重新观察、动作可以独立恢复、局部失败不扩散。

技能（Skill）提供的方法本身就是工作流（Workflow）的沉淀，可以替代原本编码的许多编排方法。稳定、反复且收益明确的局部动作仍可由程序封装；涉及数据一致性、真实授权和删除安全的检查不能只剩一句提示。

设计技能（Skill）主要帮助判断：现成工具能否完成；哪些判断应留给智能体（Agent）；哪些方法放进技能（Skill）即可；只有哪些状态和动作确实需要接口（Interface）或应用（Application）。它不默认输出一套应用分层，也不要求每个概念都对应一个程序模块。

下图只说明这一范式。Buildr 下面的分区不是本轮新增接口或应用的清单；首版仅复用实际用到的已有入口。

```mermaid
flowchart TB
    A["智能体（Agent）<br/>负责完成本轮收尾目标"]
    S["收尾技能（Skill）<br/>目标、判断方法、安全边界"]
    G["Git、系统工具、外部工具"]
    I["已有 Buildr 接口（Interface）<br/>命令行（CLI）及其他入口"]
    A --> S
    A --> G
    A --> I
    I --> W["已有工作空间查询"]
...
- 本轮直接交付和真实自举的耗时尚未测完，完成后在后续设计技能的实践案例记录；不拿测试执行时间冒充收尾时间。
