...
Task Finish 已经是固定五阶段、可恢复且会形成终态交付证明的产品执行器，但当前仍把 run、completion、lease 和大体积诊断持久化在 `.buildr/task-finish/`。这套 File Store 与已经承载 Task/Development/Review/Verification/Lifecycle current records 的 Workspace SQLite 并存，导致本机状态散落、终态读取依赖成对 JSON、失败与历史文件长期累积，并让一次收尾留下远大于最终交付事实的持久数据。
...
- 为升级执行一次性 cutover：只把可验证的已完成交付摘要导入 SQLite，未完成/失败/blocked run 不沿用旧 token 或 checkpoint，而由新执行器依据 current Development、Git、remote 与 Environment 事实重新建立 run；完成 cutover 后清理旧 Finish-owned 文件。
...
- `task-finish-execution`：将 run/checkpoint/resume/lease/completion/Result authority 从 `.buildr/task-finish` File Store 收敛到 Workspace SQLite，并定义成功后的 transient cleanup 与一次性 cutover。
...
- 需要覆盖正常完成、blocked/resume、target-race、Delivery Adaptation、远端回读失败、Environment 已清理但 Finish 未完成、进程崩溃恢复、SQLite busy/corrupt、一次性 cutover 和 transient cleanup 的集成/System 验证。
