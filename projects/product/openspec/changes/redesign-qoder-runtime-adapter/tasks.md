...
- 既有失败（不属于本变更）：同一文件的 `renderedFinish.includes('没有匹配任务就继续实际工作，不补建记录')` 断言与 `task-finish` 现文案"没有匹配任务就交付实际成果，不补建记录"不一致，在未改动的 main checkout 同样失败（源自提交 `de40f2bd`），需由该文案的归属变更决定是修正断言还是恢复文案。
