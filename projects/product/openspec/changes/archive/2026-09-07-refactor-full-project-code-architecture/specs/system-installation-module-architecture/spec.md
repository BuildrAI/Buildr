...
**Reason**: 该要求只约束此前单独的 Installation 子切片；本次获授权的全项目架构迁移明确包含 Diagnostics、Web 公共宿主、React/Vite 前端结构和 npm 工程路径，继续保留会与本次目标直接冲突。

**Migration**: 由本 Change 对 `runtime-host-doctor-module-architecture`、`buildr-web-service`、`npm-cli-package` 和 Installation 的完整兼容要求共同约束；公开行为、安全和发布边界仍必须等价。
