...
Buildr MUST将 Task Review Domain、Application、Persistence、CLI 与直接 HTTP adapter 归入 `src/modules/task` 对应技术层，以 `.ts` 作为唯一人工源码并使用实际类型。迁移后旧 `src/task`、旧全局技术层路径和同名 `.mjs` MUST不再保留第二份实现或转发入口。

#### Scenario: 检查生产源码归属
- **WHEN** 架构验证扫描 Task Review 生产实现
- **THEN** Domain、Application、Repository、CLI 与 HTTP adapter MUST只存在于 `src/modules/task` 对应 `.ts` 技术层
- **AND** 这些文件 MUST无 `@ts-nocheck` 或公共 `any` 边界

### Requirement: Task Review 必须通过窄模块入口唯一装配
Buildr MUST由 `src/modules/task/module.ts` 的 Task Review descriptor 显式声明 required capabilities、provided capabilities 与 CLI/HTTP contributions。Bootstrap MUST是唯一 composition root，legacy runtime、全局 Task persistence 聚合、CLI Host 与 HTTP Host MUST不直接注册或导入 Task Review 内部实现。
