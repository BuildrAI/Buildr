...
**Reason**: worktree 不再等同于 Task Environment，且 Git 不是环境 ready 的前提。
**Migration**: 使用 Task Environment Receipt 汇总生命周期事实；需要 Git 隔离时引用窄 Git provider evidence。

### Requirement: 多仓 repository set 必须显式、稳定且可验证
**Reason**: 该要求属于 Git worktree provider，而不是 Environment Receipt 自身。
**Migration**: 由 `buildr.git-worktree-provider/v1` 按显式 selectors、registry 与真实 Git boundary 继续执行同等预检。

### Requirement: Repository worktrees 必须保持 canonical Workspace 布局
**Reason**: canonical nested layout 是 provider evidence 的约束，不是所有 Task Environment 的固定布局。
**Migration**: Git provider 继续在所选环境 checkout 内使用 canonical `source.path`，非 Git/shared roots 记录实际位置。

### Requirement: Task environment 创建必须完整预检并可恢复
**Reason**: 旧 requirement 把 worktree repository plan 当作完整环境计划。
**Migration**: 新 `prepare/restore` 先编排 provider，再统一准备 Runtime/CLI/依赖/projection，并在 v2 receipt 保留 partial effects。

### Requirement: Task environment 必须提供统一 execution context identity
**Reason**: 旧 context 由 worktree receipt/cwd 解析，无法覆盖共享根和 provider-independent Environment。
**Migration**: 使用 Task Environment `inspect` 按 Task ID 返回 v2 receipt 与重新探测后的 execution binding。

### Requirement: Task environment CLI JSON 必须表达多仓事实
**Reason**: 旧 JSON 把多仓 Git worktree 事实冒充为完整 Task Environment，并由 worktree CLI 拥有环境结论。
**Migration**: 新 `buildr task environment prepare|inspect|cleanup --json` 使用 `buildr.task-environment-result/v1` 表达 Environment 总结果；`worktree create|inspect|cleanup --json` 使用 `buildr.git-worktree-result/v1` 只表达 Git provider evidence。

### Requirement: Task environment 清理必须保护全部成员和其他任务
**Reason**: 旧 cleanup 只覆盖 worktree repositories，且由 worktree provider拥有总清理结论。
**Migration**: Task Environment 编排全部动态资源/工作范围，Git provider 只执行其被授权的本地 Git cleanup。

### Requirement: Task environment 必须核验 execution binding
**Reason**: 旧 binding 硬编码 environment-local CLI/worktree membership，并混入 provider 与 environment authority。
**Migration**: v2 Environment 从实际 scope、stable controller、执行 CLI、Runtime、依赖与 projection probe 形成统一 binding。

### Requirement: Runtime activation evidence 必须是按影响触发的特例
**Reason**: 真实 Agent session activation 证明属于 Task Verification，不属于 Environment Receipt。
**Migration**: Environment 只记录 projection identity/ready；P0.4 在需要时持有实际 session evidence。

### Requirement: Task environment 清理必须移除本地 adoption state
**Reason**: P0.2 删除 adoption receipt，不再把 session 采用建模为环境本机状态。
**Migration**: 迁移时删除已安全关联的旧 adoption state；实际 activation evidence 以后由 Verification Result 管理。

### Requirement: Task environment 清理必须证明 owned runtime 已停止
**Reason**: 旧 requirement 由 worktree application 直接扫描少数资源，无法成为 provider-independent cleanup authority。
**Migration**: 动态资源创建者立即登记已知 provider/handle，Task Environment 统一停止并核对，Git cleanup 在其后执行。

### Requirement: Task environment execution binding 必须包含 Workspace Node identity
**Reason**: Workspace Node 仍是 Buildr 自举执行基础，但不应作为所有 Task Environment 的硬编码唯一 Runtime。
**Migration**: Environment Receipt 记录每个 scope 的适用 Runtime；Buildr/Node scope 继续强制 Workspace Node identity、npm 与真实 probe。
