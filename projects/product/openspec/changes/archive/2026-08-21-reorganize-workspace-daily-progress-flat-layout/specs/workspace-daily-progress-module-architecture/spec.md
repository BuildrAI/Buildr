...
Buildr MUST 将 Project Daily Progress 的 Domain、Application、业务 Persistence、CLI 与 HTTP Adapter 归入 `src/workspace/` 对应技术层，并 MUST 由 `src/workspace/module.mjs` 私有组装唯一 Store/Application、公开命名 capability 和 interface contributions。迁移完成后，全局 `src/domain`、`src/application`、`src/infrastructure`、`src/interfaces` 与 Bootstrap compatibility composition MUST NOT 保留第二份实现、转发入口或业务注册。
...
- **AND** `src/workspace/module.mjs` MUST 是其唯一业务组装入口
