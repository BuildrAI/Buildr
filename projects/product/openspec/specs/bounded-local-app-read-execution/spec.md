...
### Requirement: Local App 只读阻塞工作必须使用固定容量执行器
Buildr MUST 为 Local App 的 development、reviews、verification Task read view 提供固定容量的 long-lived executor；executor MUST 使用有限数量的 Worker 和有限长度的 FIFO 等待队列，MUST NOT 为每个请求创建 Worker，MUST NOT 允许等待队列无界增长。
...
- **WHEN** Local App 将已解析的 Workspace root 与 Task ID 交给 development operation
...
- **WHEN** Local App 将已解析的 Workspace root 与 Task ID 交给 reviews 或 verification operation
...
- **WHEN** 调用方提交不在白名单中的 operation、缺失 Task ID 或未由 Local App registry 解析的 root
...
### Requirement: Local App 三个 Task 专业 Tab 必须通过 executor 读取
Local App HTTP interface MUST 将 `/tasks/<task-id>/development`、`/reviews` 和 `/verification` 的 GET 请求提交给 bounded read executor，并 MUST 保持 existing no-store、已解析 Workspace identity、独立失败与只读安全边界。
