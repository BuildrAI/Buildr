...
## Purpose
定义 Project 与 Service canonical terminology 的作用域、先调查后追问原则，以及可替换术语治理能力的结果契约。
## Requirements

### Requirement: Project 必须提供 canonical glossary 作用域
Buildr Project MUST 将 `knowledge/docs/glossary.md` 作为默认 canonical glossary；术语条目 MUST 表达 canonical 名称、定义、适用范围、需要避免的歧义表达和来源。Service 特有术语 MAY 位于对应 `knowledge/docs/services/<service-code>.md` 的局部术语小节，但 MUST NOT 静默重定义 Project term。

#### Scenario: 维护 Project 通用术语
- **WHEN** 一个术语适用于 Project 的多个模块、角色或 Services
- **THEN** Agent MUST 在 `knowledge/docs/glossary.md` 中维护该术语
...
- **THEN** Agent MUST 在 `knowledge/docs/services/<service-code>.md` 中显式标注作用域和与 Project term 的关系
