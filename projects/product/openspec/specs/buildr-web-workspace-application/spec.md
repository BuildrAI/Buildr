# buildr-web-workspace-application Specification
...
- **AND** 页面、帮助、错误、日志说明与安装提示 MUST NOT 将其称为 Buildr Web 或 Buildr Web
...
#### Scenario: Buildr Desktop 保留给未来桌面产品
- **WHEN** 当前 Buildr Web 产品表面需要命名图形入口或本机 Runtime
- **THEN** MUST 使用 Buildr Web Launcher 或 Buildr Web Runtime
- **AND** MUST NOT 以 Buildr Desktop 名义注册当前命令、bundle display name、shortcut 或帮助主题
...
### Requirement: 当前本机产品的所有公开 surface 必须使用 Buildr Web 分层术语
当前本机浏览器产品的 CLI、HTTP、页面、日志、安装入口、Launcher、规则和架构说明 MUST 使用 Buildr Web、Buildr Web Runtime、Buildr Web Frontend Service、Buildr Web Launcher 或 Buildr Web Preview；MUST NOT 新增 Buildr Web 或 Buildr Web 的用户可见称呼。
