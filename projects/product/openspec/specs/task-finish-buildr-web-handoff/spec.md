# task-finish-buildr-web-handoff Specification

## Purpose

Define the safe Buildr Web handoff required before task-finish cleans an integrated task worktree.
...
### Requirement: Task Finish handoff 的公开命名必须使用 Buildr Web
Task Finish、Delivery handoff、cleanup 诊断和相关测试说明 MUST 使用 Buildr Web Runtime/Preview 术语，并 MUST 保持 repository set、端口 ownership、cleanup 隔离和 delivery authority 不变。

#### Scenario: 生成 Finish handoff
- **WHEN** Task Finish 读取当前 Development handoff
- **THEN** 公开结果 MUST 使用 Buildr Web 命名
- **AND** handoff 的 target、ownership 和 delivery 事实 MUST 与迁移前一致
