...
- 左组的项目、服务、代码库与技能资源目录 MUST 铺满可用主区域并保留两侧留白；标题、搜索和表格 MUST 共享对齐边界，打开右组后目录 MUST 填满剩余左组空间，关闭右组或折叠侧栏后 MUST 自动利用新增空间；其他正文及表单内容 MUST 保持适合阅读的宽度上限；右组打开且没有已保存手动比例时，左右两组 MUST 均分信息区可用宽度（不含左侧导航及中间分隔线）；已保存手动比例时 MUST 优先恢复该比例。
...
- **AND** 详情阅读、表单及其他非目录页面 MUST 保持既有宽度约束与操作行为
