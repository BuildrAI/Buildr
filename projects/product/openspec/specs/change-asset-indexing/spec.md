...
Buildr MUST 只在明确 Task context 的引用校验和详情读取中使用 task-scoped Change resolution。Workspace 全局 Change collection MUST 继续只从 retained Project canonical OpenSpec root 索引 active/archived Change，不得扫描Worktree或把未集成working copy混入全局列表。
...
- **AND** MUST NOT 因某个Worktree存在working copy而新增、替换或隐藏全局条目
...
- **AND** HTTP/Web MUST复用共享Resolver，不得实现第二套root selection

#### Scenario: 候选集成到 retained source
- **WHEN** Worktree working copy已进入retained Project且Worktree随后清理
...
- **AND** 全局 Change collection MUST不扫描Worktree

#### Scenario: Worktree Change暂时不可用
...
Buildr MUST 提供任务范围 Change 引用解析器（Task-scoped Change Reference Resolver），以 canonical Workspace、Task ID 与限定 `project/change` 为唯一调用身份，并 MUST通过Worktree provider读取matching repository set和实际checkout。只要Task、Project scope、Project source path与Worktree evidence归属仍可证明且Change目录当前可读，Resolver MUST允许读取Task working copy。Resolver MUST NOT信任请求filesystem path、server cwd、branch、remote或worktree名，也 MUST NOT建立第二套持久Change状态。

#### Scenario: Change只存在于matching Worktree
- **WHEN** matching Worktree的Project根包含合法active或archived Change，而retained Project尚无该Change
- **THEN** task-scoped resolution MUST返回该Change的真实lifecycle与working-copy provenance
- **AND** MUST 使用与 canonical Change indexing 相同的 path/symlink/artifact 安全校验

#### Scenario: Worktree与retained同时存在同名Change
- **WHEN** matching task Project root 与 retained Project canonical root 都包含同名 active Change
- **THEN** task-scoped resolution MUST将Worktree副本作为当前Task working copy，并把retained副本作为`retained baseline` provenance分开返回
- **AND** MUST NOT 合并 artifacts、覆盖其中一份或把两份误报为两个 Task Record 引用

#### Scenario: 安装版 Buildr Web 读取 candidate-only Change
- **WHEN** 安装版Buildr Web的product sourceRoot不同于Task Worktree，且matching Worktree Project根含有retained Project不存在的合法Change
- **THEN** Task-scoped detail route MUST通过共享Resolver返回该Change与working-copy provenance
- **AND** MUST NOT仅因Buildr Web bundle root不同而回退到retained Project

#### Scenario: Worktree副本不可用
- **WHEN** Task没有matching Worktree、Worktree已清理、当前机器没有该checkout、Project scope/source path不匹配或归属无法证明
...
- **AND** Resolver MUST只从Workspace registry、Task identity、Worktree provider与Project registry构造安全候选根
