...
P0.3 同时提供可选 Completion Review slot，但不会生成 Candidate identity 或通用状态机；只有调用方已经持有明确 Candidate target identity 时才能记录。未来是否在 handoff 要求 Planning/Completion Review，由 Task Development Change 基于实际工作流决定，不由 Review Result schema 预设。
