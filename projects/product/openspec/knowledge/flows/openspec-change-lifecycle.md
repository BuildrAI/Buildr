...
5. 候选达到 finish-ready 后，`task-finish` 在产品 run 外完成 asset review finalize；仍需人工决定时停止。随后只调用一次 `buildr task finish run`，产品固定执行 `preflight → prepare → verify → deliver → cleanup`，Agent 不逐阶段补 evidence 或 recovery。
6. `preflight` 一次聚合 execution binding/CLI probe、Change/tasks/knowledge、OpenSpec strict/pure plan、verification policy、Git/target 与 retained readiness。`prepare` 调用 `openspec converge`、runtime sync、candidate commit、target rebase 与 fixed point，之后冻结最终 candidate identity；freeze 后不再允许候选 mutation。
