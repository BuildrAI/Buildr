...
9. Finish不解析或收敛Change，不修改候选内容，不运行formal Verification/Completion Review，不生成Candidate或风险决定。Git adapter区分Task Contribution与Delivery Baseline，只在run-owned isolated carrier重放并比较delta identity；target race可凭产品token从最新基线重做`prepare → verify → deliver → cleanup`，Candidate/generation与既有gates不变。内容/上下文/policy/Result/贡献漂移、冲突、不等价或语义判断都终止run并返回Development；network、retained或cleanup等暂态阻塞按产品token恢复。多个run独立并发，不使用Workspace全局锁；v2 run/result拒绝旧v1 shape且不建立第二Finish Receipt authority。
