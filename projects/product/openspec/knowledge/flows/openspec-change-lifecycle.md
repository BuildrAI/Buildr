...
8. `deliver` 在短 target lease 中重新观察 ref，只允许 fast-forward 与普通 push，然后执行 retained doctor/sync 和必要的 CLI/Local App bundled runtime install。交付事实 durable 后，Finish 向 Environment 提交每个 scope 的 delivery identity；Environment 先停止 Task-owned 动态资源，再调用 provider cleanup 并保存唯一 cleanup 结果。target ref 在 freeze 后前进形成的 `deliver + task-finish.target-race` 只能由当前产品 token 恢复：保留 preflight，失效 candidate-dependent prepare 下游并重新 rebase/freeze、验证和交付；network、retained 与 cleanup 等候选未变的暂态阻塞仍只从最早 blocked phase 恢复。
