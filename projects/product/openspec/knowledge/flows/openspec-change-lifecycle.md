...
5. `task-finish` 创建独立持久化 run，通过 `inspect|advance|resume|run|recover` 推进 context、current knowledge、contract/canonical、candidate commit、target 与 runtime convergence；每步保存 fingerprint、effects、evidence、失效依赖和 retry policy。identity变化时，版本化recovery manifest一次提交before/after environment、candidate、target、runtime、change、assurance与transition proof，状态机原子失效并由safe executor续跑；未知分类fail closed。OpenSpec convergence 由产品入口在同一 attempt 内推进 rehearsal、pre-sync、deterministic plan/apply、strict 与 post-sync；deterministic expected tree先在temporary Project通过绑定版本strict validation才替换canonical，歧义或验证失败整批零写入。默认 CLI 只返回含结构化child diagnostic的compact checkpoint，完整输出由digest绑定的run-owned引用承载。
...
- 收尾报告分别呈现命令执行、provider/Agent编排、正式验证wall-clock、convergence workflow checks、blocked recovery、可归因重试成本和端到端耗时；observation ledger只计Buildr-owned invocation与原始output bytes，并以`product-complete|product-partial|external-unobserved`声明coverage和checkpoint间不可观察区间，不统计Agent token。不得把guard、doctor、Git或rehearsal计入task-verification时间，也不得用调用方手写duration替代provider计时。
