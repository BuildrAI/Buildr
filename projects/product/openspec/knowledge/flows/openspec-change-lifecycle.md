...
5. `task-finish` 创建独立持久化 run，通过 `inspect|advance|resume|run|recover` 推进 context、current knowledge、contract/canonical、candidate commit、target 与 runtime convergence；每步保存 fingerprint、effects、evidence、失效依赖和 retry policy。identity变化时，版本化recovery manifest一次提交before/after environment、candidate、target、runtime、change、assurance与transition proof，状态机原子失效并由safe executor续跑；未知分类fail closed。formal assurance失败先返回repair decision并等待用户授权，普通“收尾”不允许修改候选；授权必须绑定task/change、failure identity与changed-path scopes。OpenSpec convergence 由产品入口在同一 attempt 内推进 rehearsal、pre-sync、deterministic plan/apply、strict 与 post-sync；deterministic expected tree先在temporary Project通过绑定版本strict validation才替换canonical，歧义或验证失败整批零写入，receipt只持久化portable executable reference/version/content identity。默认 CLI 只返回含failure-first结构化child diagnostic的compact checkpoint，完整输出由digest绑定的run-owned引用承载。
