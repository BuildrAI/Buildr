...
5. CLI `inspect`/`list`、本机 HTTP 与 Buildr Web 只读展示。项目详情第三 Tab「每日演进」展示四问与提交（不展示变更文件列表；路径清单仍保存在 YAML/`files`），可切日期并按日/人/任务分组；按任务只聚合已关联的自己的提交。Task 概览只反向展示已关联该 Task 的提交。v1 文件标 incompatible，需 Agent 重跑覆盖。空态交给 Agent，页面不根据 Git 自动填充。
