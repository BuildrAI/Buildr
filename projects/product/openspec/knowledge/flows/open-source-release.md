...
1. npm-only release contract绑定`v<version>`、source commit、dist-tag、`engines.node`、协议identity与CHANGELOG release notes；它不声明Product Node、SEA、installer、平台矩阵或binary Assets。
...
5. 可逆门禁通过后才允许进入`npm-production`Environment。目标version缺失时发布冻结tarball；已存在时只接受相同integrity。随后有界确认version、dist-tag与integrity，并从Registry安装精确版本重新smoke。
