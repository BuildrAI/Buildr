...
7. 维护者对current frozen context明确授权后，runner才显式dispatch一次唯一protected workflow。Workflow从matching Candidate run下载并验证aggregate与`candidate-package`，所有Host Node、Launcher和protected consumer复用同一tarball bytes，不重建payload或重新pack；可逆门禁通过后请求一次`npm-production`审批，同一approved execution完成OIDC proof、final pre-tag convergence、tag ensure、同一tarball publish、dist-tag、GitHub Release与Registry安装readback。Terminal evidence按run/attempt保留逐步事实。发布成功后才执行main→dev收敛；先证明dev branch policy允许普通双亲merge commit。策略不兼容、冲突或remote race保留Publication并以稳定recovery identity报告`published-but-dev-convergence-blocked`，不得管理员绕过、`ours`、reset、force push、删除tag或unpublish。随后幂等closeout默认保留并核验正式远端release ref，清理generation carrier、本地selection refs/branch、owned worktree与临时资源；全部通过后lifecycle才`closed`并允许no-change完成唯一协调Task。
