...
2. 唯一身份链为`dev baseline → ordered selection chain → pre-reconciliation freeze → current main coverage → tree-preserving history commit/final generation → Product Candidate → frozen tarball manifest/integrity → generation carrier → main tree → post-publication dev provenance reconciliation → closeout → transaction evidence`。每个节点由专业owner提供current identity/read model；任一上游变化使下游evidence stale。即使tree相同，pre-reconciliation Candidate也不能用于final generation。
