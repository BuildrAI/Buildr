...
6. Candidate通过后，若current main与冻结release发生历史或内容冲突，release owner先在matching execution worktree执行一次`release-selection.mjs reconcile-main --confirm --reason <text>`；未解决冲突保持现场并停止，解决后产生独立main reconciliation provenance、新generation与新的release HEAD/tree，旧Candidate/artifact/readiness/context全部失效并必须重跑Candidate。随后为每个generation创建或复用确定性`codex/release-main-<version>-g<generation>`中间载体，并只从该carrier创建一个release→main受保护PR；正式远端`release-<version>`不承担PR carrier职责。该PR固定使用merge commit，readback必须证明两个父提交且`main^{tree}`等于当前冻结release tree。维护者尚未授权正式发布时，transaction runner默认只生成closed context与分阶段collect-all readiness，返回hosted deferred checks与`effects: []`；lifecycle进入`awaiting-publication-authorization`且协调Task保持active。
