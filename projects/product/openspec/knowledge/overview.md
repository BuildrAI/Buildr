...
- Task workflow：正式持久交付在首次写入前通过`task-manager`创建或恢复canonical Task Record；Agent依据当前目标和现场按需使用`task-environment`、OpenSpec、current knowledge、`task-review`、`task-verification`、Git、业务工具、`task-finish`或父子管理Skill。各Application只维护自身必要事实与动作安全，Agent在每次动作前重新读取真实代码、文件、Git、外部系统和对应专业结果。
