...
- Local App：以 Workspace 为全局目录，提供 Project、Service、Task Record 和 Change 的理解与受控操作入口；Task 详情固定为“概览、研发、证据、环境”四个一级视图，并可按 Task 范围读取未集成 Change。“研发”通过 Task Development Application `inspect` 只读展示当前结论、Candidate/generation、三个 gates、保存的决定和最近研发交接；“证据”连续展示两个 Review Result 槽位与一个 Verification Result，两个 reader 独立 loading/diagnostic。人可以管理 Task Record，但页面不提供 Development mutation、Environment prepare/cleanup、Review/Verification 执行或 Result writer；Change lifecycle 和其他专业动作仍交给 Agent/对应模块。
