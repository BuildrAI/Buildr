...
- Task Development：一个`buildr.task-development@1` capability和唯一Application维护Development Receipt、stable Content Target、verification policy、Task Candidate/generation、推进决定与不可变研发交接（Development Handoff）。它只通过Task Record、Environment、Review、Verification Applications/read models消费专业事实；OpenSpec、Git和具体测试框架都不是其通用schema前提；Local App 仅调用同一 Application 的只读 `inspect` 投影。
