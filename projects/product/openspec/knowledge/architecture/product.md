...
- Task Retrospective：一个`buildr.task-retrospective/v1`capability。Task Record或Formal Finish成功形成终态后，结束任务的Agent非阻塞询问用户是否进行“任务复盘”，只有用户明确同意才按需分析执行时间、Token消耗、重复尝试和人机协作成本；Token只记录可信可得的来源与覆盖范围，不可得时明确缺失。唯一Application按Task ID维护SQLite current Result；Local App只读展示，不参与Task终态、Development、Finish或cleanup门禁。
