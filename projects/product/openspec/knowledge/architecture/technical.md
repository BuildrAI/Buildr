...
- Local App 只监听 loopback，以 Workspace registry 为全局目录，通过 application/domain 层读取和受控管理 Project、Service 与 Task Record，并只读展示 Change。Task 详情固定为“概览、研发、证据、环境”：研发通过 Workspace-scoped、no-store 的 GET 调用 Task Development Application `inspect`；证据分别调用 Task Review 与 Task Verification `inspect`，两个区块独立失败；环境继续调用 Task Environment `inspect`。Agent action 只生成受限 prompt，不提供专业 Result 或 Development mutation API。Task-scoped Change route 复用共享 Resolver 并进入 Planning Review，全局 Change generic review 保持 retained-only；接口不接收任意 filesystem path。
...
Task Record 由 `domain/task-record` 验证 closed schema，`application/task-record` 持有五个 lifecycle action、引用校验和 read/result model，SQLite repository在`tasks`及Project/Service/Change关系表中以单事务维护。CLI interface只解析action参数、调用Application并适配输出/退出码；Local App HTTP/Web共用该Application，interface不直接解析SQL或返回数据库path。Repository以Git topology拒绝linked-worktree Task Record target；旧`.buildr/tasks/<task-id>/task.yml`不读取、不迁移、不双写、不删除，Development/Review/Verification/Environment文件继续由各自writer维护。Local App mutation携带读取时的逻辑`recordDigest`；不匹配时在同一写事务内fail closed并要求刷新，digest不持久化，也不表示revision、锁或自动合并。
