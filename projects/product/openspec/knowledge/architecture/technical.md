...
Task Record 由 `domain/task-record` 验证 closed schema，`application/task-record` 持有五个 lifecycle action、Parent setter、引用校验和直接关系 read/result model，SQLite repository在`tasks`、Project/Service/Change关系表及窄`task_parent_relations`中以单事务维护。关系写入拒绝不存在/terminal Parent、自引用和祖先循环；每个Child至多一个Parent，Parent可有多个直接Children。Parent/Child不传播状态、Result或专业动作，读取不递归展开整树。CLI interface只解析action参数、调用Application并适配输出/退出码；Local App HTTP/Web共用该Application，interface不直接解析SQL或返回数据库path。Repository以Git topology拒绝linked-worktree Task Record target；旧`.buildr/tasks/<task-id>/task.yml`不读取、不迁移、不双写、不删除，Development/Review/Verification/Environment文件继续由各自writer维护。Local App mutation携带读取时的逻辑`recordDigest`；不匹配时在同一写事务内fail closed并要求刷新，digest不持久化，也不表示revision、锁或自动合并。
