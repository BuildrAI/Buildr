...
`preflight`只聚合handoff、ready Environment、retained target、adapter readiness与retained activation声明；`prepare`用临时index观察原任务基线到冻结source snapshot的Task Contribution，在run-owned detached worktree的最新Delivery Baseline上机械应用，并把声明digest与`none | render-runtime | sync-workspace` plan冻结进carrier。clean apply形成commit并记录`deterministic-reuse`；Git conflict保留baseline carrier并blocked返回交付适配（Delivery Adaptation）。Agent只在该carrier中处理语义兼容；resume核验ownership、baseline ancestry、source/handoff current、cleanliness及Project policy要求的bounded compatibility checks。`verify`对适配记录`agent-reviewed-delivery-adaptation`和path/mode/blob/tree/head/check facts，不声称Buildr证明语义等价，不写Task Verification Result，`formalVerificationExecutions`固定为0。`deliver`先完成carrier普通fast-forward/push与`remoteAfterRef`回读，再执行类型化activation和Doctor：Workspace根runtime source只render且禁止tracked delta；只有retained Project/Service binding命中package输入才sync，sync产生的受管delta精确暂存为独立convergence commit并普通push/readback。`finalRemoteRef`等于carrier或convergence ref后，`cleanup`才清理原Task Environment与carrier。只有Development Application报告applicability stale才返回Development rebuild；其他无法证明facts保持blocked。
