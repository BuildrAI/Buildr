...
`preflight`只聚合handoff、ready Environment、retained target与adapter readiness；`prepare`用临时index观察原任务基线到current source snapshot的canonical raw Git delta，在run-owned detached worktree的最新Delivery Baseline上应用并形成carrier commit，不改写原Task worktree/index/branch；`verify`只调用Development carrier-equivalence read model并复核contribution/baseline/carrier identity，`formalVerificationExecutions`固定为0。`deliver`只做普通fast-forward/push，以及retained source更新后的适用sync/install/Doctor；`cleanup`把bounded contribution proof交给Task Environment/Git provider独立复算后清理原Task Environment与isolated carrier。target race用精确token重做`prepare → verify → deliver → cleanup`并保持Candidate generation与既有gates不变；冲突、贡献漂移、不等价、handoff漂移或需要语义判断时terminal返回Task Development。路径不重叠与clean apply不构成语义安全结论。
