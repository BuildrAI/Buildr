...
- Local App 只监听 loopback，以 Workspace registry 为全局目录，通过 application/domain 层读取和受控管理 Project、Service 与 Task Record，并只读展示 Change。Task 列表与详情通过 Task Record read model 展示直接 Parent/Children，创建和编辑 active Task 可选择、重挂或清除 Parent，并支持关系导航。Task 详情固定为“概览、研发、证据、环境”：研发通过 Workspace-scoped、no-store 的 GET 调用 Task Development Application `inspect`；证据分别调用 Task Review 与 Task Verification `inspect`，两个区块独立失败；环境继续调用 Task Environment `inspect`。Agent action 只生成受限 prompt，不提供专业 Result 或 Development mutation API。Task-scoped Change route 复用共享 Resolver并进入 Planning Review，全局 Change generic review 保持 retained-only；接口不接收任意 filesystem path。
