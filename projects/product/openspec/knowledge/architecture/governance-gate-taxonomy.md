...
| Formal Verification preparation gap | 记录正式 Verification Result | Verification Preparation Admission | 未准备的 capability 会产生未执行或不匹配的正式证据 | Project verification declaration + Task Environment preparation evidence | matching Formal Verification Result | 形成受控 auxiliary preparation；开发期 focused/affected 工作继续 | Hard Gate，仅阻止 formal result；恢复事项同时是 Attention | `development-evidence-flow`、`task-admission-environment` |
