...
type Props = { disabled?: boolean; loading?: boolean; label: string; options: { value: string; label: string }[]; value?: string | string[] | null; multiple?: boolean; placeholder?: string; onChange: (value: string | string[]) => void; onCreate?: () => void };
/** A shared option picker: filter, create action, existing resources. */
export function CreatableResourceSelect({ disabled, loading, label, options, value, multiple, placeholder, onChange, onCreate }: Props) {
  const [open, setOpen] = useState(false), [query, setQuery] = useState('');
  return <Select disabled={disabled} loading={loading} showSearch={false} aria-label={label === '服务' ? '关联服务' : label} style={{ width: '100%' }} mode={multiple ? 'multiple' : undefined} value={value} placeholder={placeholder || `选择${label}`} onChange={next => { onChange(next); if (!multiple) { setQuery(''); setOpen(false); } }} open={open} onDropdownVisibleChange={setOpen}
