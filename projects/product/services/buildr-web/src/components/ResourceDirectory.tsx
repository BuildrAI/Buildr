...
export function ResourceDirectory<T extends object>(props: Props<T>) {
  ...
  const columns: ColumnsType<T> = [
    ...
    ...(props.columns ?? []),
    { title: '操作', key: 'actions', width: props.onDelete ? 112 : 72, fixed: 'right', className: 'resource-directory-action-cell', align: 'left', render: (_, item) => <div className="resource-directory-row-actions"><Button type="text" disabled={props.editDisabled} onClick={event => { event.stopPropagation(); props.onEdit(item); }}>编辑</Button>{props.onDelete && <Button type="text" disabled={props.editDisabled} onClick={event => { event.stopPropagation(); props.onDelete?.(item); }}>移除</Button>}</div> },
  ];
  return <section className="resource-directory">
    <header className="resource-directory-head"><div><p className="resource-eyebrow">工作空间</p><h1>{props.title}</h1><p>{props.description}</p></div><div className="resource-directory-actions">{props.onRefresh && <RefreshButton label={`刷新${props.noun}`} loading={props.refreshing ?? props.loading} onClick={props.onRefresh} />}{props.actions}</div></header>
    {props.notice}
    ...
    {props.error ? <Alert showIcon type="error" message={props.error} /> : <div className="resource-directory-table" id={props.listId}><div id={props.tableId}>
      <Table<T> rowKey={props.rowKey} columns={columns} dataSource={rows} loading={props.loading} pagination={false} tableLayout="fixed" scroll={{ x: 320 + (props.onDelete ? 112 : 72) + (props.columns ?? []).reduce((width, column) => width + (typeof column.width === 'number' ? column.width : 160), 0) }}
