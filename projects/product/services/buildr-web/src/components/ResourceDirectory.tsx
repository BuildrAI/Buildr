...
export function ResourceDirectory<T extends object>(props: Props<T>) {
  ...
  const columns: ColumnsType<T> = [
    ...
    ...(props.columns ?? []),
    { title: '操作', key: 'actions', width: props.onDelete ? 172 : 88, fixed: 'right', align: 'right', render: (_, item) => <><Button type="link" disabled={props.editDisabled} onClick={event => { event.stopPropagation(); props.onEdit(item); }}>编辑</Button>{props.onDelete && <Button type="link" danger disabled={props.editDisabled} onClick={event => { event.stopPropagation(); props.onDelete?.(item); }}>移除登记</Button>}</> },
  ];
  return <section className="resource-directory">
    ...
    {props.error ? <Alert showIcon type="error" message={props.error} /> : <div className="resource-directory-table" id={props.listId}><div id={props.tableId}>
      <Table<T> rowKey={props.rowKey} columns={columns} dataSource={rows} loading={props.loading} pagination={false} tableLayout="fixed" scroll={{ x: 320 + (props.onDelete ? 172 : 88) + (props.columns ?? []).reduce((width, column) => width + (typeof column.width === 'number' ? column.width : 160), 0) }}
