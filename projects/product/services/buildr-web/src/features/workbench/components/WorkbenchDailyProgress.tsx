    ...
    {items.length ? <div className={'workbench-activity-items' + (full ? ' full' : '')}>
      {items.map(item => <article className="workbench-activity-item" key={item.project + '/' + item.date}>
        ...
        <div className="workbench-section-heading">
          ...
          {!compact && <Link className="workbench-link" to={workspaceHref(workspaceId, dailyProgressActivityPath(item.project, item.date))}>查看每日演进 <ArrowRightOutlined /></Link>}
        </div>
        ...
        {compact && <Link className="workbench-link workbench-daily-detail" to={workspaceHref(workspaceId, dailyProgressActivityPath(item.project, item.date))}>查看每日演进 <ArrowRightOutlined /></Link>}
      </article>)}
    </div> : compact ? <div className="workbench-daily-empty">
      <span>暂无演进记录</span>
      <Button size="small" onClick={() => openAgentAction('daily-progress', dailyProgressActionContext(project, date))}>生成每日演进</Button>
