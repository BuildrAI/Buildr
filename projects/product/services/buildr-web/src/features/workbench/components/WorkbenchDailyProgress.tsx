...
export function WorkbenchDailyProgress({ data, full = false, project = '', onRefresh }: {
  data: WorkbenchResponse['dailyProgress']; full?: boolean; project?: string; onRefresh(): void;
}) {
  const { workspaceId, openAgentAction } = useAppShell();
  const items = full ? data.items : data.items.slice(0, 2);
  return <div id="workbench-daily-progress">
    ...
    {items.length ? <div className={'workbench-activity-items' + (full ? ' full' : '')}>
      {items.map(item => <article className="workbench-activity-item" key={item.project + '/' + item.date}>
        ...
        <div className="workbench-section-heading">
          ...
          <Link className="workbench-link" to={workspaceHref(workspaceId, '/projects/' + encodeURIComponent(item.project) + '?document=daily')}>查看每日演进 <ArrowRightOutlined /></Link>
        </div>
        ...
        {!full && item.daySummary.drawbacks ? <p className="workbench-coverage"><strong>影响与不足：</strong>{item.daySummary.drawbacks}</p> : null}
        <p className="workbench-coverage"><HistoryOutlined /> 摘要记录于 {formatDateTime(item.recordedAt)} · {item.commitCount} 个提交</p>
