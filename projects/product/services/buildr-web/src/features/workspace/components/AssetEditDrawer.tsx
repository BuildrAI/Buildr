...
export function AssetEditDrawer({ catalog, kind, id, onClose }: { catalog: AssetCatalog; kind: AssetKind; id: string; onClose: () => void }) {
  ...
  return <MetadataEditDrawer open title={`编辑${label}`} objectName={item.name} formId="catalog-edit" saveButtonId="catalog-edit-save" saving={busy} disabled={catalog.migrationRequired} onClose={onClose}>
