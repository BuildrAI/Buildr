...
export function AssetHome({ kind, previewId }: { kind: 'service' | 'repository'; previewId?: string }) {
  ...
  return <WorkspaceStage pageTabs={tabs.tabs} onClosePageTab={tabs.close}
    ...
    <div className="resource-home">{kind === 'service' && <section className="resource-section"><h2>服务知识</h2><Link to={href(`/knowledge/service/${encodeURIComponent(assetId)}${projectContext ? `?fromProject=${encodeURIComponent(projectContext)}` : ''}`)}>理解服务架构与实现 →</Link></section>}
      <header className="resource-home-head"><div><p className="resource-eyebrow">{label} <span> / {item.code}</span></p><h1 id={kind === 'service' ? 'service-detail-name' : 'repository-detail-name'}>{item.name}</h1><p id={kind === 'service' ? 'service-detail-description' : 'repository-detail-description'}>{item.description || '尚未填写说明。'}</p></div><Space wrap><ResourceActions resource={{ kind: kind === "service" ? "service" : "repository", key: kind + ":" + item.id, label: item.name, href: workspaceHref(workspaceId, "/" + (kind === "service" ? "services" : "repositories") + "/" + encodeURIComponent(item.id)) }} />{kind === 'service' && <Button danger disabled={data.migrationRequired} onClick={() => setDeleting(true)}>删除服务</Button>}<Button disabled={data.migrationRequired} onClick={() => setEditing(true)}>编辑{label}</Button></Space></header>
      ...
      <section className="resource-section"><div className="resource-section-head"><h2>{kind === 'service' ? '代码库' : '代码来源'}</h2>{kind === 'repository' && <Button size="small" onClick={() => void prepare()}>准备代码</Button>}</div>
        {!repository ? <Alert type="warning" message="代码库引用缺失，请核对清单。" /> : kind === 'service' ? <div className="resource-repository-summary"><Link to={href(`/repositories/${repository.id}`)}><span className="resource-row-icon"><BranchesOutlined /></span><span><strong>{repository.name}</strong><small>{repositoryBranch(repository)}{service?.modulePath ? ` · ${service.modulePath}` : ''}</small></span><RightOutlined /></Link><RepositoryStatus key={repository.id} id={repository.id} revision={data.revision} onAlign={() => void prepare()} onRefresh={localConfig.reload} />{!repository.present && <Button type="text" onClick={() => void prepare()}>准备代码</Button>}</div> : <Descriptions className="resource-facts" column={1} colon={false} size="small" items={[{ key: 'url', label: 'Git 地址', children: localConfig.loading ? '读取本地配置…' : actualRemote?.url || (localConfig.data?.available ? localConfig.data.remotes.map(remote => `${remote.name}: ${remote.url}`).join('；') || '本地未配置远端' : localConfig.error || localConfig.data?.diagnostic || '尚未读取') }, { key: 'remote', label: '远端名称（Remote）', children: actualRemote?.name || localConfig.data?.remotes.map(remote => remote.name).join('、') || '未读取或未配置' }, ...((repository.source.git as { url?: string } | undefined)?.url && (repository.source.git as { url?: string }).url !== actualRemote?.url ? [{ key: 'declared', label: '声明地址', children: String((repository.source.git as { url?: string }).url) }] : []), { key: 'branch', label: '集成分支', children: <code>{repositoryBranch(repository)}</code> }, { key: 'path', label: '目录', children: <span title={repository.location}>{repository.source.path}</span> }, { key: 'state', label: '状态', children: <RepositoryStatus key={repository.id} id={repository.id} revision={data.revision} onAlign={() => void prepare()} onRefresh={localConfig.reload} /> }]} />}
      </section>
    </div>
