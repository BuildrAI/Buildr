...
export function AssetDeleteDialog({ kind, id, onClose, onDeleted }: { kind: 'project' | 'service'; id: string; onClose: () => void; onDeleted?: () => void }) {
  ...
  return <Modal open title={`删除${label}登记${item ? `：${item.name}` : ''}`} okText="删除登记" cancelText="取消" confirmLoading={busy} okButtonProps={{ danger: true, disabled: !item || data?.migrationRequired || Boolean(catalog.error) }} onCancel={() => { if (!busy) onClose(); }} onOk={async () => {
    if (!data || !item) return;
    ...
    {data?.migrationRequired && <Alert type="warning" message="请先迁移旧服务登记，再删除。" />}
    {(error || catalog.error) && <Alert type="error" message={error || catalog.error} description="未完成删除。请关闭后重新核对当前登记。" />}
  </Modal>;
}
