...
import { reviewRecords, taskStageLabels } from './taskWorkContent';
...
export function TaskReadingPane({ target, task, context, artifacts, evidence, workspaceId, href, onRead, onClose, onRespond, onRelativeLink, refreshTask, embedded = false, inDrawer = false, refreshToken = 0 }: {
  ...
  if (target.kind === 'artifact') {
    ...
    return <div className="task-reader"><TaskArtifactReader embedded={embedded} change={source.change} artifactPath={target.path} onClose={onClose} onProjectDocument={path => void artifacts.openChangeDocument(target.changeKey, path)} onSelect={path => onRead({ ...target, path, title: path.split('/').at(-1) || '文档' })} /></div>;
  }
  ...
  if (target.kind === 'context' || target.kind === 'acceptance') {
    ...
    return <article className="task-reader"><h2>{target.title}</h2>{target.kind === 'context' && <><p className="task-reader-origin">{context ? `更新于 ${formatDateTime(context.updatedAt)}` : '尚未记录'}</p><h3>当前节点</h3><p>{context?.stage ? taskStageLabels[context.stage].title : '未记录'}</p><h3>最近进展</h3><p>{context?.progress || '暂无进展'}</p><h3>下一步</h3><p>{context?.nextStep || '暂无下一步'}</p></>}{attention ? <section className="task-reader-section"><p>{attention.reason}</p>{attention.response ? <><p id="task-attention-response">{attention.response.text}</p><small>{formatDateTime(attention.response.recordedAt)}</small></> : <Button id="task-closeout-respond" type="primary" onClick={onRespond}>记录验收意见</Button>}</section> : target.kind === 'acceptance' ? <p>暂无用户确认记录。</p> : null}</article>;
  }
