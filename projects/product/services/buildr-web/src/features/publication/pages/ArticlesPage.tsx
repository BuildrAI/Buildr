...
export function ArticlesPage() {
  ...
  return <WorkspaceStage pageTabs={tabs.tabs} onClosePageTab={tabs.close}><div className="publication-page publication-directory">
    ...
    <div id="publications-list" className="publication-table"><Table<Publication> loading={loading && !data} dataSource={visible} rowKey={articleResourceKey} pagination={false} size="middle" scroll={{ x: 700 }} onRow={article => ({ onClick: event => { if (!(event.target as HTMLElement).closest('button,a')) navigate(href(articlePath(article)), { state: returnState }); } })} locale={{ emptyText: <div id="publications-empty"><Empty description={publications.length ? '没有找到匹配的文章' : '暂无文章'} />{publications.length ? <Button onClick={() => setParams({}, { replace: true })}>清除筛选</Button> : <Button type="primary" disabled={!projects.length} onClick={() => setCreating(true)}>新建文章</Button>}</div> }} columns={[
      ...
      { title: '操作', width: 94, align: 'right', key: 'actions', render: (_, article) => <Space size={0}><Button type="text" icon={isSaved(articleResourceKey(article)) ? <StarFilled /> : <StarOutlined />} loading={saving === articleResourceKey(article)} disabled={preferences.loading} aria-label={`${isSaved(articleResourceKey(article)) ? '取消收藏' : '收藏'}：${article.title}`} onClick={() => void toggleSaved(article)} /><Dropdown trigger={['click']} menu={{ items: [{ key: 'edit', label: '编辑文章', icon: <EditOutlined /> }, { key: 'source', label: '查看原文' }, { key: 'copy', label: '复制链接' }, { key: 'export', label: '导出 Markdown' }, { type: 'divider' }, { key: 'delete', label: '删除文章', danger: true }], onClick: ({ key }) => {
        if (key === 'edit') navigate(href(`${articlePath(article)}/edit`), { state: returnState });
        if (key === 'source') navigate(href(articlePath(article)), { state: { ...returnState, articleView: 'source' } });
    ...
    {creating && <ArticleCreateModal projects={projects} defaultProject={filters.project} onClose={() => setCreating(false)} onCreated={detail => { setCreating(false); navigate(href(`${articlePath(detail.publication)}/edit`), { state: returnState }); }} />}
