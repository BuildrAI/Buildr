...
import { articleDraft, articlePath, insertMarkdown, publicationStatus, writingMethods } from '../publication-model';
import { ArticleAssetsPanel } from '../components/ArticleAssetsPanel';
...
export function ArticleEditPage() {
  ...
  const tabs = useWorkspacePageTabs(workspaceId), navigate = useNavigate(), location = useLocation();
  ...
  useEffect(() => {
    if (!article) return;
    tabs.register({ key: `publication-edit:${projectCode}:${publicationId}`, kind: 'proj', title: `编辑 · ${article.title}`, path: location.pathname });
    setBreadcrumbParts([workspace?.name || '工作空间', '文章', article.title, '编辑']);
  }, [article?.title, workspaceId, projectCode, publicationId]);
  useEffect(() => {
    if (!dirty) return;
  ...
  const back = () => { if (!article) navigate(href('/articles')); else navigate(href(articlePath(article)), { state: { articleListSearch: location.state?.articleListSearch || '' } }); };
  const cancel = () => { if (dirty) setLeave(true); else back(); };
  const save = async () => {
    ...
    try {
      ...
      setBase(saved); setDraft(articleDraft(saved)); setAssets(saved.assets || []); setConflict(false); messages.success('文章已保存');
      navigate(href(articlePath(saved.publication)), { state: { articleListSearch: location.state?.articleListSearch || '' } });
    } catch (err) {
  ...
  return <WorkspaceStage pageTabs={tabs.tabs} onClosePageTab={tabs.close}><div className="publication-page publication-editor">{holder}<Button className="publication-back" type="link" onClick={cancel}>← 返回文章</Button>
    ...
    {!draft || !base || !article ? read.loading ? <div className="publication-loading"><Spin /><p>正在读取当前稿件…</p></div> : <Empty description="当前稿件不可用" /> : <>
      <header className="publication-page-head"><div><p className="eyebrow">文章</p><h1>编辑文章</h1><p className="page-copy">修改当前稿件，图片和附件可直接上传后引用。</p></div><Space><Button onClick={cancel} disabled={busy}>取消</Button><Button id="article-save" type="primary" icon={<SaveOutlined />} loading={busy} onClick={() => void save()}>保存修改</Button></Space></header>
      ...
      <div className="publication-editor-layout"><main className="publication-editor-main"><Form layout="vertical">
        ...
        <footer className="publication-editor-footer"><span id="article-edit-state">{dirty ? '有尚未保存的修改' : '当前稿件已保存'}</span><Button type="primary" loading={busy} onClick={() => void save()}>保存修改</Button></footer>
      ...
      <Modal open={leave} title="放弃尚未保存的修改？" okText="放弃修改" cancelText="继续编辑" onCancel={() => setLeave(false)} onOk={() => { setDraft(articleDraft(base)); setLeave(false); back(); }}><p>已上传的资源文件会保留，正文改动尚未保存。</p></Modal>
