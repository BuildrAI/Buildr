...
export function useTaskRequestLifecycle(): TaskReadLifecycle {
  const lifecycle = useRef<TaskReadLifecycle | null>(null);
  if (!lifecycle.current) lifecycle.current = createTaskReadLifecycle();
  useEffect(() => () => lifecycle.current?.abortAll(), []);
