...
export function TaskTable({ tasks, selectedTaskId, taskHref, onOpen }: {
  tasks: TaskListItem[];
  ...
  return <Table rowKey={(item) => item.record.taskId} pagination={false} showHeader={false} tableLayout="fixed" dataSource={tasks} columns={columns} rowClassName={(item) => item.record.taskId === selectedTaskId ? 'task-row-active' : ''} onRow={(item) => ({ onClick: () => onOpen(item.record.taskId) })} components={{ body: { wrapper: TableBody } }} />;
}
