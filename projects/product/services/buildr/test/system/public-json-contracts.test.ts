...
const fixtureRuntime: any = createRuntime();
const fixtureRoot: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-json-contract-context-'));
const fixtureEnv: any = { ...process.env, BUILDR_APP_DATA_DIR: path.join(fixtureRoot, 'local-app') };
...
function run(args: any, options: any = {}): any  {
  return new Promise((resolve: any, reject: any) => {
    const child: any = spawn(process.execPath, [buildr, ...args], { cwd: productRoot, env: options.env || process.env });
    let stdout: any = '';
    let stderr: any = '';
...
function fixtureWorkspace(t: any, context: any): any  {
  const root: any = fs.mkdtempSync(path.join(fixtureRoot, `${context}-case-`));
...
test('全部 workspace JSON command family 输出登记的 schemaVersion', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'plain');
  const env: any = { ...process.env, BUILDR_APP_DATA_DIR: path.join(root, 'local-app') };
...
test('doctor JSON默认compact且full必须显式请求', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'codex');
  const compact: any = await run(['doctor', '--agent', 'codex', '--target', root, '--json']);
  const explicitCompact: any = await run(['doctor', '--agent', 'codex', '--target', root, '--json', '--detail', 'compact']);
  ...
  const full: any = await run(['doctor', '--agent', 'codex', '--target', root, '--json', '--detail', 'full']);
  ...
  const contracts: any = full.capabilities.graphs.flatMap((graph: any) => graph.contracts);
  assert.ok(contracts.length > 0);
  assert.ok(contracts.every((contract: any) => /^[a-f0-9]{64}$/.test(contract.digest)));
});

test('human Doctor separates npm, development, Launcher and Host identities', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'codex');
  const appData: any = path.join(root, 'human-installation-status');
  ...
  const output: any = await run(['doctor', '--agent', 'codex', '--target', root], { json: false, env });
  ...
  const status: any = await run(['installation', 'status'], { json: false, env });
  for (const expected of [
...
test('doctor 严格报告 workspace identity 与独立 readiness', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'plain');

  const initialized: any = await run(['doctor', '--target', root, '--json', '--detail', 'full']);
  ...
  assert.ok(incomplete.findings.some((finding: any) => finding.code === 'workspace.identity_incomplete'));

  fs.rmSync(path.join(root, 'AGENTS.md'));
  fs.rmSync(path.join(root, 'projects'), { recursive: true, force: true });
  const absent: any = await run(['doctor', '--target', root, '--json', '--detail', 'full'], { expectedStatus: 1 });
  ...
  assert.ok(absent.findings.some((finding: any) => finding.code === 'workspace.not_initialized'));
});

test('Codex partial inventory 作为 assurance metadata 保留且不产生 doctor warning', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'codex');
...
test('doctor 从 canonical 所有权回执发现 runtime，并明确报告旧路径迁移与 dual conflict', async (t: any) => {
  ...
  const legacyFile: any = fs.readdirSync(legacyRoot, { recursive: true })
    .map((relative: any) => path.join(legacyRoot, relative))
    .find((file: any) => fs.existsSync(file) && fs.lstatSync(file).isFile() && file.endsWith('.json'));
  const relative: any = path.relative(legacyRoot, legacyFile);
  const canonicalFile: any = path.join(canonicalRoot, relative);
  const conflicting: any = JSON.parse(fs.readFileSync(legacyFile, 'utf8'));
  ...
  const conflict: any = await run(['doctor', '--agent', 'codex', '--target', root, '--json', '--detail', 'full'], { expectedStatus: 1 });
  assert.match(JSON.stringify(conflict), /canonical and legacy receipts differ/);
});

test('doctor 默认只盘点受管 runtime，显式 agent 才把对应 drift 变为可操作项', async (t: any) => {
  ...
  const inventory: any = await run(['doctor', '--target', root, '--json']);
...
test('doctor 对未登记 Project 只报告登记根因并输出去重 repair plan', async (t: any) => {
  const root: any = fixtureWorkspace(t, 'plain');
  fs.mkdirSync(path.join(root, 'projects', 'orphan'), { recursive: true });

  const report: any = await run(['doctor', '--target', root, '--json']);
  const orphanCodes: any = report.findings
    .filter((finding: any) => finding.path === 'projects/orphan')
    .map((finding: any) => finding.code);
  ...
  assert.equal(report.repairPlan.filter((step: any) => step.codes.includes('projects.unregistered')).length, 1);
  assert.equal(report.nextSteps.filter((step: any) => step.codes.includes('projects.unregistered')).length, 1);

  const textReport: any = await run(['doctor', '--target', root], { json: false });
