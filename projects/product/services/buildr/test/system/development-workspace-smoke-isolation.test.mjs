...
const runner = path.join(serviceRoot, 'tools/development/run-isolated-workspace-smoke.mjs');
const failingScenario = path.join(serviceRoot, 'test/fixtures/failing-workspace-smoke.mjs');
...
function setupUserState(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-smoke-user-state-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const appData = path.join(root, 'released-web');
  const productData = path.join(root, 'product');
...
test('Workspace smoke 使用独立数据根并在成功后保持用户状态零副作用', (t) => {
  const state = setupUserState(t);
  const result = runSmoke([], state);
...
test('Workspace smoke 子场景失败仍清理并保持用户状态零副作用', (t) => {
  const state = setupUserState(t);
  const result = runSmoke(['--script', failingScenario], state);
