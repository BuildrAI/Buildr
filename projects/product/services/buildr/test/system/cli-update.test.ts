...
function origin(channel: any, version: any = '1.0.0'): any  {
  const runtimeRole: any = channel === 'npm' ? 'host' : channel === 'platform' ? 'product' : 'development';
...
function formalOriginOptions(channel: any, version: any = '1.0.0'): any  {
...
function writePackage(root: any): any  {
  const serviceRoot: any = path.join(root, 'projects', 'product', 'services', 'buildr');
...
test('CLI 来源识别关联 Git workspace 中的 Product Project 与 Buildr Service', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-git-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const productRoot: any = writePackage(root);
  git(root, 'init', '-q');
  git(root, 'config', 'user.email', 'buildr@example.com');
  git(root, 'config', 'user.name', 'Buildr Test');
  git(root, 'add', '.');
  git(root, 'commit', '-qm', 'initial');
  const source: any = identifyCliSource(productRoot);
...
test('无法证明来源时 fail closed', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-unknown-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  fs.writeFileSync(path.join(root, 'package.json'), '{"name":"@buildr-ai/buildr","version":"1.0.0"}\n');
  const source: any = identifyCliSource(root);
  assert.equal(source.mode, 'unknown');
  assert.equal(source.blockingReasons.length, 1);
});

test('update rejects a self-consistent formal receipt bound to another payload digest', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-payload-mismatch-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  fs.writeFileSync(path.join(root, 'package.json'), '{"name":"@buildr-ai/buildr","version":"1.0.0"}\n');
  const formal: any = formalOriginOptions('npm');
  const mismatched: any = createInstallationOrigin({ ...formal.origin, applicationPayloadDigest: `sha256-${'f'.repeat(64)}` });
  const source: any = identifyCliSource(root, { origin: mismatched, payloadManifest: formal.payloadManifest });
  assert.equal(source.mode, 'unknown');
...
test('开发 checkout 缺少 upstream 时 check 返回稳定阻塞结构', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-plan-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const productRoot: any = writePackage(root);
  git(root, 'init', '-q');
  git(root, 'config', 'user.email', 'buildr@example.com');
  ...
  git(root, 'add', '.');
  git(root, 'commit', '-qm', 'initial');
  const plan: any = buildCliUpdatePlan(productRoot, { fetch: false, registryLookup: () => '1.0.0' });
...
test('npm 来源必须由 receipt 证明，目录形状本身保持 unknown', (t: any) => {
  const prefix: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-prefix-'));
  t.after(() => fs.rmSync(prefix, { recursive: true, force: true }));
  const productRoot: any = path.join(prefix, ...(process.platform === 'win32' ? [] : ['lib']), 'node_modules', '@buildr-ai', 'buildr');
  ...
  const updateAuthority: any = createProductUpdateAuthority({
...
test('开发者模式自动 fast-forward 到 upstream', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-ff-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const remote: any = path.join(fixture, 'remote.git');
  const seed: any = path.join(fixture, 'seed');
  const client: any = path.join(fixture, 'client');
  ...
  git(seed, 'push', '-q');
  const plan: any = buildCliUpdatePlan(path.join(client, 'projects', 'product', 'services', 'buildr'), { registryLookup: () => '1.0.0' });
...
test('开发者模式只对本地未发布提交执行 rebase', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-rebase-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const remote: any = path.join(fixture, 'remote.git');
  const seed: any = path.join(fixture, 'seed');
  const client: any = path.join(fixture, 'client');
...
test('发布模式更新保持 package identity 与安装 prefix', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-npm-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const prefix: any = path.join(fixture, 'safe-prefix');
  fs.mkdirSync(prefix);
  const log: any = path.join(fixture, 'npm.log');
  const npmCli: any = path.join(fixture, 'npm-cli.mjs');
  fs.writeFileSync(npmCli, `import fs from 'node:fs'; fs.writeFileSync(process.env.INSTALL_LOG, process.argv.slice(2).join(' ') + '\\n');\n`);
  const updateAuthority: any = createProductUpdateAuthority({ nodeExecutable: process.execPath, npmCliPath: npmCli, prefix });
  const plan: any = {
    mode: 'npm',
  ...
  const result: any = executeCliUpdatePlan(plan, { env: { ...process.env, PATH: '', INSTALL_LOG: log } });
  assert.equal(result.ok, true);
  assert.equal(fs.readFileSync(log, 'utf8').trim(), `install --global --prefix ${prefix} @buildr-ai/buildr@2.0.0`);
});

test('npm update query and install both use registered Host Node and npm CLI with an empty PATH', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-authority-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const prefix: any = path.join(fixture, 'prefix');
  const productRoot: any = path.join(prefix, 'lib', 'node_modules', '@buildr-ai', 'buildr', 'payload', 'product');
  const npmCli: any = path.join(fixture, 'npm-cli.mjs');
  const queryLog: any = path.join(fixture, 'query.log');
  fs.mkdirSync(productRoot, { recursive: true });
  fs.writeFileSync(path.join(productRoot, 'package.json'), '{"name":"@buildr-ai/buildr","version":"1.0.0"}\n');
  ...
  const updateAuthority: any = createProductUpdateAuthority({ nodeExecutable: process.execPath, npmCliPath: npmCli, prefix });
  const plan: any = buildCliUpdatePlan(productRoot, {
    ...formalOriginOptions('npm'),
    registration: { status: 'installed', entry: { updateAuthority }, reason: null },
  ...
  fs.rmSync(npmCli);
  const tampered: any = buildCliUpdatePlan(productRoot, {
    ...formalOriginOptions('npm'),
    registration: { status: 'installed', entry: { updateAuthority }, reason: null },
    registryCommandOptions: { env: { ...process.env, PATH: '', QUERY_LOG: queryLog } },
  });
...
test('npm --ignore-scripts authority remains null and update check fails closed without PATH lookup', (t: any) => {
  const prefix: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-no-authority-'));
  t.after(() => fs.rmSync(prefix, { recursive: true, force: true }));
  const productRoot: any = path.join(prefix, 'payload', 'product');
  fs.mkdirSync(productRoot, { recursive: true });
  fs.writeFileSync(path.join(productRoot, 'package.json'), '{"name":"@buildr-ai/buildr","version":"1.0.0"}\n');
  const plan: any = buildCliUpdatePlan(productRoot, {
    ...formalOriginOptions('npm'),
...
test('npm update authority 缺失时不会从 PATH 或 prefix 猜测 package manager', () => {
  const result: any = executeCliUpdatePlan({
...
test('开发 checkout 区分 Git source 与 prerelease version 漂移', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-version-drift-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const productRoot: any = writePackage(root);
  ...
  git(root, 'add', '.');
  git(root, 'commit', '-qm', 'initial');
  const branch: any = git(root, 'branch', '--show-current');
  git(root, 'remote', 'add', 'origin', root);
  git(root, 'update-ref', `refs/remotes/origin/${branch}`, 'HEAD');
  git(root, 'branch', '--set-upstream-to', `origin/${branch}`, branch);
  const plan: any = buildCliUpdatePlan(productRoot, { fetch: false, registryLookup: () => '0.1.0-rc.5' });
...
test('开发 checkout registry 不可用时保留 Git source 结论', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-cli-update-version-unknown-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const productRoot: any = writePackage(root);
  git(root, 'init', '-q');
  git(root, 'config', 'user.email', 'buildr@example.com');
  ...
  git(root, 'update-ref', `refs/remotes/origin/${branch}`, 'HEAD');
  git(root, 'branch', '--set-upstream-to', `origin/${branch}`, branch);
  const plan: any = buildCliUpdatePlan(productRoot, { fetch: false, registryLookup: () => { throw new Error('offline'); } });
  assert.equal(plan.sourceStatus, 'up-to-date');
...
test('Release Awareness 同时解释 GA 与 RC 并默认让 prerelease 跟随 candidate', () => {
  const awareness: any = buildReleaseAwareness({
...
test('Release Awareness 不把 latest 上的历史 RC 称为 GA', () => {
  ...
  assert.match(awareness.notices.find((notice: any) => notice.track === 'stable').message, /GA 正式版尚未发布.*历史候选版/);
});

test('Release Awareness 对未变化的错误轨道头只主动通知一次', (t: any) => {
  const dataRoot: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-awareness-invalid-'));
  ...
  const first: any = buildReleaseAwareness(source, options);
  const second: any = buildReleaseAwareness(source, { ...options, observedAt: '2026-08-15T02:00:00.000Z' });
  assert.equal(first.notices.find((notice: any) => notice.track === 'stable').notify, true);
  assert.equal(second.notices.find((notice: any) => notice.track === 'stable').notify, false);
});

test('Release Awareness 用户级状态原子保存每轨道 seen/notified 并抑制重复主动通知', (t: any) => {
  const dataRoot: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-awareness-state-'));
  t.after(() => fs.rmSync(dataRoot, { recursive: true, force: true }));
  const source: any = {
    mode: 'npm', channel: 'npm', package: '@buildr-ai/buildr', version: '0.1.0-rc.12',
    productRoot: '/product', installPrefix: '/prefix', installationIdentity: 'installation', updateAuthority: null,
  ...
  const first: any = buildReleaseAwareness(source, options);
  const second: any = buildReleaseAwareness(source, { ...options, observedAt: '2026-08-15T01:00:00.000Z' });
  ...
  const persisted: any = readReleaseAwarenessState({ dataRoot }).state;
...
test('npm update 默认保持当前轨道并允许显式选择 GA 或 RC', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-track-plan-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const prefix: any = path.join(fixture, 'prefix');
  const productRoot: any = path.join(prefix, 'lib', 'node_modules', '@buildr-ai', 'buildr', 'payload', 'product');
  const npmCli: any = path.join(fixture, 'npm-cli.mjs');
  ...
  const implicit: any = buildCliUpdatePlan(productRoot, base);
  const stable: any = buildCliUpdatePlan(productRoot, { ...base, track: 'stable' });
...
test('npm update 不从正式版自动切换到 RC 且拒绝自动降级', (t: any) => {
  const fixture: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-track-safety-'));
  t.after(() => fs.rmSync(fixture, { recursive: true, force: true }));
  const prefix: any = path.join(fixture, 'prefix');
  const productRoot: any = path.join(prefix, 'lib', 'node_modules', '@buildr-ai', 'buildr', 'payload', 'product');
  const npmCli: any = path.join(fixture, 'npm-cli.mjs');
  ...
  const implicit: any = buildCliUpdatePlan(productRoot, base);
  const candidate: any = buildCliUpdatePlan(productRoot, { ...base, track: 'candidate' });
  ...
  const downgrade: any = buildCliUpdatePlan(productRoot, {
