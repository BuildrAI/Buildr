...
test('Buildr Web Task API 提供轻量查询与既有任务维护，不暴露创建入口', async (t: any) => {
  ...
  runtime.createTask(root, { taskId: 'order-todo', title: '待办排序', intent: '四态顺序', status: 'todo', projects: [], services: [], changes: [] });
  const orderCompleted: any = runtime.createTask(root, { taskId: 'order-completed', title: '完成排序', intent: '四态顺序', projects: [], services: [], changes: [] });
  runtime.completeTask(root, 'order-completed', { expectedRecordDigest: orderCompleted.recordDigest, summary: '完成排序夹具' });
  const orderAbandoned: any = runtime.createTask(root, { taskId: 'order-abandoned', title: '放弃排序', intent: '四态顺序', projects: [], services: [], changes: [] });
  runtime.abandonTask(root, 'order-abandoned', { expectedRecordDigest: orderAbandoned.recordDigest, reason: '放弃排序夹具' });
  ...
  assert.equal(bulkView.tasks.length, 205); assert.equal(bulkView.totalTaskCount, 205); assert.equal(bulkView.matchingTaskCount, 205);
  assert.equal(bulkView.pageSize, null); assert.equal(bulkView.hasMore, false); assert.equal(bulkView.nextCursor, null);
  assert.equal(preparedStatements, 9, '未分页列表 SQL 次数必须与 Task 数量无关');
  ...
  assert.equal(firstPage.tasks.length, 50); assert.equal(firstPage.matchingTaskCount, 205); assert.equal(firstPage.pageSize, 50); assert.equal(firstPage.hasMore, true); assert.ok(firstPage.nextCursor);
  assert.equal(preparedStatements, 10, '首批分页只增加一次完整匹配计数查询');
  const pagedIds: any[] = firstPage.tasks.map((item: any) => item.record.taskId);
  const pagedStatuses: any[] = firstPage.tasks.map((item: any) => item.record.status);
  let cursor: any = firstPage.nextCursor;
  let finalPage: any = firstPage;
  while (cursor) {
    finalPage = runtime.queryTasks(root, { pageSize: '50', cursor });
    assert.equal(finalPage.filterOptions, null, '续载批次不得重复读取filter options');
    pagedIds.push(...finalPage.tasks.map((item: any) => item.record.taskId));
    pagedStatuses.push(...finalPage.tasks.map((item: any) => item.record.status));
    cursor = finalPage.nextCursor;
  }
  assert.equal(pagedIds.length, 205); assert.equal(new Set(pagedIds).size, 205, '相同 updatedAt 的分页边界不得重复或遗漏 Task');
  assert.deepEqual([...new Set(pagedStatuses)], ['active', 'todo', 'completed', 'abandoned']);
  ...
  assert.throws(() => runtime.queryTasks(root, { q: '%_' }), (error: any) => error.code === 'task_record_filter_invalid');
  assert.throws(() => runtime.queryTasks(root, { q: '轻量' }), (error: any) => error.code === 'task_record_filter_invalid');
  assert.deepEqual(runtime.queryTasks(root, { q: 'app-task' }).tasks.map((item: any) => item.record.taskId), ['app-task'], 'q 必须使用FTS trigram匹配任务编号');
  ...
  assert.deepEqual(runtime.queryTasks(root, { q: '#app-task' }).tasks.map((item: any) => item.record.taskId), ['app-task'], '#前缀必须精确查询完整Task ID');
  assert.deepEqual(runtime.queryTasks(root, { project: 'other', service: 'demo/api' }).tasks, [], 'Project与Service必须保持AND语义');
  assert.throws(() => runtime.queryTasks(root, { q: "%' OR 1=1 --" }), (error: any) => error.code === 'task_record_filter_invalid');
  bulkStore = runtime.openWorkspaceStructuredStore(root, { writable: true }); bulkStore.database.prepare("DELETE FROM tasks WHERE task_id LIKE 'bulk-%' OR task_id LIKE 'order-%'").run(); bulkStore.database.close();
  ...
  let response: any = await request(endpoint); assert.equal(response.body.schemaVersion, 'buildr.task-record-list/v7'); assert.equal(response.body.totalTaskCount, 3); assert.equal(response.body.matchingTaskCount, 3); assert.equal(response.body.pageSize, null); assert.deepEqual(new Set(response.body.tasks.map((item: any) => item.record.taskId)), new Set(['app-parent', 'app-task', 'app-retrospective']));
  ...
  response = await request(`${endpoint}?q=%E8%BD%BB%E9%87%8F%E8%AF%BB%E5%8F%96&project=demo&service=demo%2Fapi&status=active&hasChildren=no&retrospectiveState=missing`);
  assert.deepEqual(response.body.tasks.map((item: any) => item.record.taskId), ['app-task']);
  assert.deepEqual(response.body.filters, { q: '轻量读取', project: 'demo', service: 'demo/api', status: 'active', hasChildren: 'no', retrospectiveState: 'missing' });
  ...
  assert.equal(secondPage.body.filterOptions, null);
