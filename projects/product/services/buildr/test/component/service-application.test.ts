...
import { compareServiceGit, registerServiceApplication } from '../../src/modules/workspace/application/service-application.ts';

const service: any = { source: { type: 'git', git: { url: 'https://example.com/api.git', remote: 'origin', integrationBranch: 'dev' } } };

test('Service Git comparison 将当前分支与 integration branch 分离', () => {
  const result: any = compareServiceGit(service, { available: true, repository: true, remoteUrl: 'https://example.com/api.git', currentBranch: 'tasks/x', dirty: true, ahead: 1, behind: 0 }, (a: any, b: any) => a === b);
  assert.equal(result.status, 'drift');
  assert.deepEqual(result.findings.map((item: any) => item.code), ['service.git_branch_drift', 'service.git_dirty', 'service.git_upstream_drift']);
});

test('Service metadata update 使用白名单和 revision', () => {
  const writes: any[] = [];
  let entity: any = { id: 'bc098bd6-08a2-4e8b-883c-3ef5b188a86d', workspaceId: 'f2f40b71-2382-5906-82bd-76a7927b59f3', projectId: 'd15bde2c-9aab-4ed8-bf43-28a5372ca407', code: 'api', name: 'API', description: '接口', type: 'backend', source: { type: 'workspace', path: 'projects/product/services/api' } };
  const runtime: any = {
    ...
    withWorkspaceMutation: (_root: any, _name: any, _paths: any, work: any) => work(),
    writeServiceRegistry: (_file: any, _projectId: any, services: any) => { writes.push(services); entity = services.api; },
