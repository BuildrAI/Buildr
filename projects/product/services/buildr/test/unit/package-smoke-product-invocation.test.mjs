...
import { createBuildrSelfInvoker } from '../../src/agent-assets/application/package-maintenance/smoke-checks.mjs';
import { productInvocationArgs } from '../../src/infrastructure/product-invocation/index.mjs';

function exerciseInvocation(invocation) {
  const calls = [];
  let resolutions = 0;
  const invoker = createBuildrSelfInvoker({
...
test('package smoke recursive CLI calls preserve the current installation-channel invocation', () => {
  const host = exerciseInvocation({ command: '/product/node', argsPrefix: ['/product/bin/buildr.mjs'], kind: 'host-node' });
  assert.equal(host.resolutions, 1);
  assert.deepEqual(host.calls.map(({ runner, command, args }) => ({ runner, command, args })), [
