...
import { inspectCandidateFailedShardRetry } from '../../tools/release/candidate-failed-shard-retry.ts';
import { compactReleasePhaseTimeline, createReleasePhaseTimeline, projectCandidateAttempts, projectCandidateRetryAttempts } from '../../tools/release/release-phase-timeline.ts';
...
test('timeline derives duration only from two proven canonical boundaries', () => {
  const timeline: any = createReleasePhaseTimeline({
    version: '1.0.0-rc.1',
    generation: 2,
  ...
  assert.equal(createReleasePhaseTimeline({ version: '1.0.0-rc.1', generation: 2, terminalStatus: 'active', phases: timeline.phases.map((item: any) => ({ ...item, startedAt: item.timing.startedAt, finishedAt: item.timing.finishedAt })) }).identity, timeline.identity);
});

test('candidate attempts preserve reused evidence origin and actual rerun scope', () => {
  const phases: any = projectCandidateAttempts([
    {
      runId: 42,
      runAttempt: 1,
  ...
  const timeline: any = createReleasePhaseTimeline({ version: '1.0.0-rc.1', generation: 2, terminalStatus: 'active', phases });
  const retry: any = timeline.phases.find((item: any) => item.id === 'candidate-attempt:42:2');
  ...
  assert.equal(timeline.phases.find((item: any) => item.id === 'candidate-queue:42:2').waitType, 'platform-queue');
});
...
test('timeline consumes the delivered failed-shard retry and aggregate workflow facts', () => {
  ...
  const aggregateIdentity: any = digest('3');
  const attempts: any = projectCandidateRetryAttempts({
    retryResults: [retry],
    aggregateIdentity,
    aggregate: {
      status: 'passed',
  ...
  const timeline: any = createReleasePhaseTimeline({ version: '1.0.0-rc.1', generation: 2, terminalStatus: 'active', phases: projectCandidateAttempts(attempts) });
  const retryAttempt: any = timeline.phases.find((item: any) => item.id === `candidate-attempt:${runId}:2`);
  assert.deepEqual(retryAttempt.attempt.rerunScope, ['Candidate core (core-package-runtime-release-macos)']);
  assert.deepEqual(retryAttempt.attempt.evidence.map((item: any) => [item.id, item.disposition, item.origin.runAttempt]), [
...
test('compact timeline keeps identity and pointer without full owner evidence', () => {
  const timeline: any = createReleasePhaseTimeline({ version: '1.0.0', generation: 0, terminalStatus: 'closed', phases: [
  ...
  const compact: any = compactReleasePhaseTimeline(timeline);
