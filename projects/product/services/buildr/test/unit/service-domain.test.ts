...
test('Service Domain 保存父 UUID、业务字段和文件系统定位', () => {
  const service: any = createService({ id: ID, workspaceId: WORKSPACE_ID, projectId: PROJECT_ID, projectCode: 'product', code: 'api', name: ' API ', description: ' 服务 ', type: 'backend', source: { type: 'workspace', path: 'projects/product/services/api' } });
