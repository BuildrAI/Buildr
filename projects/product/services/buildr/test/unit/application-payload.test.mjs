...
function manifest() {
  ...
  const value = {
    ...
    files: requiredFiles.map((file, index) => ({
      path: file,
      mode: 0o644,
      size: index + 1,
...
test('npm installation origin uses the stable closed ownership tuple', () => {
  ...
  const receipt = createNpmInstallationOrigin(payload);
  const { ownershipIdentity, ...identity } = receipt;
  const expected = `sha256-${crypto.createHash('sha256').update(Buffer.from(JSON.stringify(identity), 'utf8')).digest('hex')}`;
