...
test('module registry resolves named dependencies and exposes closed contributions', () => {
  const dependency: any = Object.freeze({ value: 42 });
  const registry: any = createModuleRegistry({ capabilities: { 'foundation.clock': dependency } });
  const descriptor: any = registry.install(defineModule({
    id: 'example',
    requires: ['foundation.clock'],
    create: (requires: any) => ({
      provides: { 'example.application': { dependency: requires['foundation.clock'] } },
      contributions: { cli: [{ key: 'example run' }], http: [{ id: 'example.http', handle(): any  {} }] },
    }),
  }));

  assert.equal(descriptor.id, 'example');
  assert.equal(registry.provide('example.application').dependency, dependency);
  assert.deepEqual(registry.contributions('cli').map((item: any) => item.id), ['example run']);
...
test('module registry fails closed for invalid definitions, missing dependencies and duplicates', () => {
  assert.throws(() => defineModule({ id: 'bad', requires: [], create(): any  {}, extra: true }), { code: 'bootstrap_module_definition_invalid' });

  const registry: any = createModuleRegistry();
  assert.throws(() => registry.install(defineModule({ id: 'missing', requires: ['missing.port'], create(): any  { return {}; } })), { code: 'bootstrap_module_dependency_missing' });
  ...
  assert.throws(() => registry.install(defineModule({ id: 'bad-lifecycle', requires: [], create: () => ({ lifecycle: { start(): any  {} } }) })), { code: 'bootstrap_module_lifecycle_invalid' });
});
...
test('module lifecycle rolls back only successfully started resources', async () => {
  ...
  registry.install(defineModule({ id: 'two', requires: [], create: () => ({ lifecycle: { start(): any  { events.push('start:two'); throw new Error('boom'); }, stop: () => events.push('stop:two') } }) }));
