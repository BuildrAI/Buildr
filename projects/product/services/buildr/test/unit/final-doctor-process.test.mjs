...
test('final Doctor runner始终请求bounded compact JSON', () => {
  ...
  const executed = runFinalDoctor({ executable: '/node', cliPath: '/buildr.mjs', agent: 'codex', targetRoot: '/workspace', cwd: '/product', spawn });
...
test('final Doctor runner不捕获超过1 MiB的full inventory', () => {
  const full = JSON.stringify({ inventory: 'x'.repeat(1024 * 1024) });
...
test('final Doctor runner区分业务失败、输出超限与进程执行失败', () => {
  const business = classifyFinalDoctorResult({ status: 1, signal: null, stdout: '{"ok":false}', stderr: '' });
  assert.equal(business.status, 'doctor-failed');
  assert.equal(business.code, 'doctor.not-passed');

  const overflow = classifyFinalDoctorResult({ status: null, signal: null, stdout: '', stderr: '', error: { code: 'ENOBUFS', message: 'spawnSync ENOBUFS' } });
  ...
  const execution = classifyFinalDoctorResult({ status: null, signal: null, stdout: '', stderr: '', error: { code: 'ENOENT', message: 'missing executable' } });
