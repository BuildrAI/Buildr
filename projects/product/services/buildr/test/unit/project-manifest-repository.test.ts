...
} from '../../src/modules/workspace/persistence/project-manifest-repository.ts';
...
test('canonical Project Manifest v2 round trip 使用封闭 Domain schema', () => {
  const content: any = renderProjectsManifest({
    product: {
      id: PROJECT_ID,
  ...
  const parsed: any = parseProjectsManifest(content, { workspaceId: WORKSPACE_ID });
...
test('canonical Project Manifest保留Attached Root且managed shape不变', () => {
  const content: any = renderProjectsManifest({ external: { id: PROJECT_ID, workspaceId: WORKSPACE_ID, code: 'external', name: 'External', description: 'External project', source: { type: 'git', root: 'attached', path: '/repos/external', git: { url: 'https://example.com/external.git', remote: 'origin', integrationBranch: 'dev' } } } });
