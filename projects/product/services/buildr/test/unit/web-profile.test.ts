...
test('Web profile只接受closed product channel与runtime role组合', () => {
  ...
  ]) assert.throws(() => webProfileName(identity), (error: any) => error.code === 'web_profile_identity_invalid');
});
...
test('BUILDR_APP_DATA_DIR只覆盖Web Root且不改变profile或共享Product Root', () => {
  const env: any = { BUILDR_APP_DATA_DIR: '/tmp/buildr-web', BUILDR_PRODUCT_DATA_DIR: '/tmp/buildr-product' };
  const profile: any = resolveWebProfile(developmentIdentity, { platform: 'linux', home: '/home/demo', env });
...
test('显式Web Root把对侧Profile限制在同一隔离命名空间', () => {
  const current: any = resolveWebProfile(developmentIdentity, { env: { BUILDR_APP_DATA_DIR: '/tmp/buildr-web' } });
  const peer: any = oppositeWebProfile(current, developmentIdentity, { env: { BUILDR_APP_DATA_DIR: '/tmp/buildr-web' } });
...
test('Launcher identity必须与Web profile的channel和runtime role一致', () => {
  const development: any = resolveWebProfile(developmentIdentity, { env: { BUILDR_APP_DATA_DIR: '/tmp/dev' } });
  ...
  assert.throws(
    ...
    (error: any) => error.code === 'web_launcher_profile_mismatch',
  );
});

test('Development Launcher必须绑定当前protocol、checkout与Host Node', () => {
  const sourceRoot: any = path.resolve('/tmp/buildr-development');
  ...
  const profile: any = resolveWebProfile(productIdentity, { env: { BUILDR_APP_DATA_DIR: '/tmp/dev' } });
  const launcher: any = {
