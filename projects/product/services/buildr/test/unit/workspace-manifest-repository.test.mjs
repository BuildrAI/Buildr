...
test('canonical Workspace Manifest 使用封闭 schema 并保留兼容 metadata', () => {
  const content = renderWorkspaceManifest({
    workspace: { id: WORKSPACE_ID, name: 'Buildr', description: 'Product workspace', runtime: { node: { version: '22.14.0' } } },
  ...
  assert.deepEqual(parsed.workspace, { id: WORKSPACE_ID, name: 'Buildr', description: 'Product workspace', runtime: { node: { version: '22.14.0' } } });
...
test('canonical Workspace Manifest 缺少 Node 声明时要求迁移并拒绝模糊或不兼容版本', () => {
  ...
  for (const version of ['22', '22.x', 'v22.1', '18.20.0']) {
