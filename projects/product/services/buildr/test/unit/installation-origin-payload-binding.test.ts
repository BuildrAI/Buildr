...
function formalOrigin(payload: any): any  {
...
test('formal installation origin requires the exact validated application payload identity', () => {
  ...
  const matching: any = formalOrigin(payload);
  assert.deepEqual(validateFormalInstallationOriginPayloadBinding(matching, payload), matching);

  const otherPayloadOrigin: any = createInstallationOrigin({
    ...matching,
  ...
  assert.throws(
    ...
    (error: any) => /applicationPayloadDigest/.test(error.message) && /sourceCommit/.test(error.message),
  );
