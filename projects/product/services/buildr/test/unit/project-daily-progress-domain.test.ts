...
import { groupDailyProgressCommits, localCalendarDate } from '../../src/modules/workspace/application/project-daily-progress-application.ts';
import {
...
} from '../../src/modules/workspace/domain/project-daily-progress.ts';
...
test('日历日只接受合法 YYYY-MM-DD，省略时使用本机本地时区', () => {
  ...
  assert.throws(() => normalizeDailyProgressDate('2026-13-01'), (error: any) => error.code === 'daily_progress_date_invalid');
});
...
test('提交允许空 Task，他人提交禁止挂 Task', () => {
  ...
  assert.throws(() => normalizeDailyProgressPayload({
    ...
    commits: [],
    files: [],
  }), (error: any) => error.code === 'daily_progress_field_invalid');
  assert.throws(() => normalizeDailyProgressPayload({
    daySummary: summary(),
    commits: [],
    files: [],
    gitAuthors: [],
  }), (error: any) => error.code === 'daily_progress_field_forbidden');
});
...
test('按人按 email 分组，按任务排除他人提交', () => {
  ...
  const people: any = groupDailyProgressCommits(commits, 'person');
  assert.equal(people.length, 2);
  const tasks: any = groupDailyProgressCommits(commits, 'task');
  assert.ok(tasks.some((group: any) => group.key === 'task-one'));
  assert.ok(tasks.some((group: any) => group.key === 'unlinked'));
  assert.equal(tasks.some((group: any) => group.commits.some((commit: any) => commit.authorship === 'other')), false);
});

test('创建文档使用 v2 closed schema，v1 标为 incompatible', () => {
  const document: any = createDailyProgressDocument({
    project: 'product',
    date: '2026-08-18',
  ...
  assert.throws(() => normalizeDailyProgressDocument({
    ...
    recordedAt: '2026-08-18T10:00:00.000Z',
    items: [],
  }), (error: any) => error.code === 'daily_progress_schema_incompatible');
  assert.throws(() => normalizeDailyProgressDocument({
  ...
  const error: any = dailyProgressError('daily_progress_task_missing', 'missing', 409);
