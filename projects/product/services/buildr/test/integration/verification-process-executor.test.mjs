...
test('formal command executor bounds a process that ignores normal completion', async () => {
  const result = await executeVerificationCommand({ name: 'process-timeout', command: { argv: [process.execPath, '-e', 'process.on("SIGTERM",()=>{}); setInterval(()=>{},1000)'], timeoutMs: 1_000 } }, { terminationGraceMs: 20, terminationConfirmMs: 200 });
