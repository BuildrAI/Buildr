...
import { WORKSPACE_ROOT_GITIGNORE_ENTRIES } from '../../src/modules/workspace/module.ts';
import { createBuildrApplicationTest } from '../context/buildr-node-test.ts';

const test: any = createBuildrApplicationTest('integration-project-daily-progress-application');

function fixture(t: any): any  {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-daily-progress-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const previousLog: any = console.log;
  console.log = () => {};
  const runtime: any = t.buildrContexts.application;
  try {
...
function payload(overrides: any = {}): any  {
  return {
    daySummary: {
      added: '新增提交列表。',
      updated: '更新 schema。',
...
test('init 写入每日演进 ignore，且不得忽略整个 .buildr', (t: any) => {
  const { root }: any = fixture(t);
  const lines: any = fs.readFileSync(path.join(root, '.gitignore'), 'utf8').split(/\r?\n/);
  assert.equal(lines.filter((line: any) => line === '/.buildr/daily-progress/').length, 1);
  assert.equal(lines.includes('/.buildr/'), false);
  assert.ok(WORKSPACE_ROOT_GITIGNORE_ENTRIES.includes('/.buildr/daily-progress/'));
});

test('未登记 Project 与非法日期 fail closed 且不创建目录', (t: any) => {
  const { root, runtime }: any = fixture(t);
  const before: any = fs.existsSync(path.join(root, '.buildr', 'daily-progress'));
  assert.throws(() => runtime.recordProjectDailyProgress(root, {
    project: 'missing',
    date: '2026-08-18',
    payload: payload(),
  }), (error: any) => error.code === 'daily_progress_project_unknown');
  assert.throws(() => runtime.recordProjectDailyProgress(root, {
    project: 'demo',
    date: '2026-02-30',
    payload: payload(),
  }), (error: any) => error.code === 'daily_progress_date_invalid');
...
test('合法 record 写入 v2 YAML、覆盖重跑，且不写 Task Record', (t: any) => {
  ...
  const file: any = path.join(root, '.buildr', 'daily-progress', 'demo', '2026-08-18.yml');
  const other: any = path.join(root, '.buildr', 'daily-progress', 'demo', '2026-08-17.yml');
  fs.writeFileSync(other, 'keep\n');
  const document: any = YAML.parse(fs.readFileSync(file, 'utf8'));
  ...
  const overwritten: any = runtime.recordProjectDailyProgress(root, {
    project: 'demo',
    date: '2026-08-18',
    payload: payload({
      commits: [{
...
test('缺失 Task 或他人提交挂 Task 整次失败且不覆盖已有文件', (t: any) => {
  ...
  const file: any = path.join(root, '.buildr', 'daily-progress', 'demo', '2026-08-18.yml');
  const previous: any = fs.readFileSync(file, 'utf8');
  assert.throws(() => runtime.recordProjectDailyProgress(root, {
    project: 'demo',
    date: '2026-08-18',
    payload: payload({
  ...
  }), (error: any) => error.code === 'daily_progress_task_missing');
  assert.throws(() => runtime.recordProjectDailyProgress(root, {
    project: 'demo',
    date: '2026-08-18',
    payload: payload({
  ...
  }), (error: any) => error.code === 'daily_progress_foreign_task_forbidden');
  assert.equal(fs.readFileSync(file, 'utf8'), previous);
});

test('inspect 投影、v1 incompatible、未解析 Task 不改文件', (t: any) => {
  const { root, runtime }: any = fixture(t);
  runtime.recordProjectDailyProgress(root, { project: 'demo', date: '2026-08-18', payload: payload() });
  const file: any = path.join(root, '.buildr', 'daily-progress', 'demo', '2026-08-18.yml');
  const previous: any = fs.readFileSync(file, 'utf8');
  const document: any = YAML.parse(previous);
  document.commits[0].taskIds = ['task-one', 'gone-task'];
  fs.writeFileSync(file, YAML.stringify(document));
  const inspected: any = runtime.inspectProjectDailyProgress(root, { project: 'demo', date: '2026-08-18', group: 'task' });
  assert.equal(inspected.status, 'inspected');
  assert.equal(inspected.unresolvedTaskCount > 0, true);
  const unresolved: any = inspected.commits[0].tasks.find((task: any) => task.taskId === 'gone-task');
  assert.equal(unresolved.resolved, false);
  const byPerson: any = runtime.inspectProjectDailyProgress(root, { project: 'demo', date: '2026-08-18', group: 'person' });
  assert.ok(byPerson.groups.some((group: any) => group.label.includes('王志宏')));
  const listed: any = runtime.listProjectDailyProgress(root, { project: 'demo' });
  assert.deepEqual(listed.dates, ['2026-08-18']);
  const missing: any = runtime.inspectProjectDailyProgress(root, { project: 'demo', date: '2026-08-01' });
  assert.equal(missing.status, 'not-found');
  const v1: any = path.join(root, '.buildr', 'daily-progress', 'demo', '2026-08-17.yml');
  ...
  const incompatible: any = runtime.inspectProjectDailyProgress(root, { project: 'demo', date: '2026-08-17' });
  assert.equal(incompatible.status, 'incompatible');
  const reverse: any = runtime.inspectTaskDailyProgress(root, 'task-one');
  assert.equal(reverse.itemCount, 2);
  assert.equal(fs.readFileSync(file, 'utf8').includes('gone-task'), true);
});

test('CLI record/inspect/list 使用稳定 JSON schema 且 discovery 可用', (t: any) => {
  const { root, runtime }: any = fixture(t);
  const buildr: any = path.resolve(import.meta.dirname, '../../bin/buildr.mjs');
  const input: any = path.join(root, 'payload.json');
  ...
  const record: any = spawnSync(process.execPath, [buildr, 'project', 'daily-progress', 'record', '--project', 'demo', '--date', '2026-08-18', '--input', input, '--target', root, '--json'], { encoding: 'utf8' });
  assert.equal(record.status, 0, record.stderr);
  const recorded: any = JSON.parse(record.stdout);
  assert.equal(recorded.schemaVersion, 'buildr.project-daily-progress-record-result/v1');
  assert.equal(recorded.itemCount, 1);
  const inspect: any = spawnSync(process.execPath, [buildr, 'project', 'daily-progress', 'inspect', '--project', 'demo', '--date', '2026-08-18', '--target', root, '--json'], { encoding: 'utf8' });
  assert.equal(inspect.status, 0, inspect.stderr);
  assert.equal(JSON.parse(inspect.stdout).schemaVersion, 'buildr.project-daily-progress-inspect-result/v1');
  const list: any = spawnSync(process.execPath, [buildr, 'project', 'daily-progress', 'list', '--project', 'demo', '--target', root, '--json'], { encoding: 'utf8' });
