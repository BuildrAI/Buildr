...
import { getRuntimeAdapter, SUPPORTED_AGENT_IDS } from '../../src/modules/agent-assets/infrastructure/runtime/adapter-contract.ts';
import { assembleRuntimeProjection } from '../../src/modules/agent-assets/infrastructure/runtime/projection.ts';
import { buildSkillContent, buildSkillRenderPlan, hasManagedSkillMarker, resolveRenderSkills } from '../../src/modules/agent-assets/infrastructure/runtime/skills/render-plan.ts';
import { parseSkillProjectionReceipt } from '../../src/modules/agent-assets/infrastructure/runtime/skills/projection-files.ts';
...
function fixture(): any  {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-capability-runtime-'));
  ...
  for (const id of ['provider', 'second-provider', 'required-consumer', 'optional-consumer']) {
    const directory: any = path.join(root, 'skills', id);
  ...
  for (const id of ['required', 'optional']) {
    fs.writeFileSync(path.join(root, 'skills', 'contracts', 'example', `${id}.md`), [
      ...
      `# example.${id}`,
      '',
      ...sections.flatMap((section: any) => [`## ${section}`, '', `${section} text.`, '']),
    ].join('\n'));
  }
  return root;
}

function writeManifest(root: any, overrides: any = {}): any  {
  const document: any = {
...
function writeDependencyComponent(root: any, { mode = 'required', state = 'installed', dependencySkill = 'required-consumer' }: any = {}): any  {
  const componentRoot: any = path.join(root, 'components', 'workspace', 'dependency-fixture');
  const fragmentRelative: any = 'components/workspace/dependency-fixture/contributions/consumer.md';
  const fragmentFile: any = path.join(root, fragmentRelative);
  fs.mkdirSync(path.dirname(fragmentFile), { recursive: true });
  fs.writeFileSync(fragmentFile, '## Component dependency boundary\n');
  const integrity: any = `sha256-${crypto.createHash('sha256').update(fs.readFileSync(fragmentFile)).digest('hex')}`;
...
test('全部 supported adapters 投射一致的 ready/degraded binding，且不修改 Skill source', (t: any) => {
  const root: any = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  writeManifest(root);
  const sourceFile: any = path.join(root, 'skills', 'required-consumer', 'SKILL.md');
  const sourceBefore: any = fs.readFileSync(sourceFile, 'utf8');

  for (const runtime of SUPPORTED_AGENT_IDS) {
    const skills: any = resolveRenderSkills(root, '.', runtime);
    const required: any = skills.find((skill: any) => skill.id === 'required-consumer');
    const optional: any = skills.find((skill: any) => skill.id === 'optional-consumer');
    ...
    const requiredContent: any = buildSkillContent(root, { ...required, runtime });
    const optionalContent: any = buildSkillContent(root, { ...optional, runtime });
    ...
    const target: any = path.join(root, `target-${runtime}`);
    fs.mkdirSync(target);
    const receiptWrite: any = buildSkillRenderPlan(root, target, [{ ...required, runtime }], runtime).writes.find((item: any) => item.kind === 'skill-projection-receipt');
    const receipt: any = parseSkillProjectionReceipt(receiptWrite.content);
...
test('Component dependency contribution 拒绝未接收 fragment 的 target', (t: any) => {
  const root: any = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  writeManifest(root);
...
test('required provider 缺失时保留 blocked consumer，contract 变化只更新 receipt 机器证据', (t: any) => {
  const root: any = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const target: any = path.join(root, 'target');
  fs.mkdirSync(target);
  const blockedDocument: any = writeManifest(root, {
    bindings: [],
    skills: [
      { id: 'required-consumer', path: 'required-consumer', requires: [{ capability: 'example.required', version: 1, mode: 'required' }] },
    ],
  });
  let consumer: any = resolveRenderSkills(root, '.', 'codex').find((skill: any) => skill.id === 'required-consumer');
  const blocked: any = buildSkillContent(root, { ...consumer, runtime: 'codex' });
  ...
  consumer = resolveRenderSkills(root, '.', 'codex').find((skill: any) => skill.id === 'required-consumer');
  const ready: any = buildSkillContent(root, { ...consumer, runtime: 'codex' });
  ...
  const readyReceiptWrite: any = buildSkillRenderPlan(root, target, [{ ...consumer, runtime: 'codex' }], 'codex').writes.find((item: any) => item.kind === 'skill-projection-receipt');
  const readyReceipt: any = parseSkillProjectionReceipt(readyReceiptWrite.content);
  ...
  const contractFile: any = path.join(root, 'skills', 'contracts', 'example', 'required.md');
  fs.appendFileSync(contractFile, '\nProvider-neutral clarification.\n');
  consumer = resolveRenderSkills(root, '.', 'codex').find((skill: any) => skill.id === 'required-consumer');
  const contractChanged: any = buildSkillContent(root, { ...consumer, runtime: 'codex' });
  const changedReceiptWrite: any = buildSkillRenderPlan(root, target, [{ ...consumer, runtime: 'codex' }], 'codex').writes.find((item: any) => item.kind === 'skill-projection-receipt');
  const changedReceipt: any = parseSkillProjectionReceipt(changedReceiptWrite.content);
...
test('全部 supported adapters 的产品入口保持 adapter-neutral 且不注入全局 routing dump', (t: any) => {
  const root: any = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  writeManifest(root);

  let expectedContent: any = null;
  for (const runtime of SUPPORTED_AGENT_IDS) {
    const target: any = path.join(root, `target-${runtime}`);
    fs.mkdirSync(target);
    const plan: any = assembleRuntimeProjection({
      repoRoot: root,
      targetRoot: target,
      adapterId: runtime,
      selection: { productSkill: true, workspaceSkills: true },
    });
    const productSkill: any = plan.plan.writes.find((item: any) => item.capability === 'product-buildr-skill' && item.skillRelativePath === 'SKILL.md');
...
test('Component dependency contribution 与 fragment 同生命周期合并且 required 覆盖 optional', (t: any) => {
  const root: any = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  writeManifest(root, {
    bindings: [],
