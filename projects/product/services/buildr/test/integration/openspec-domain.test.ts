...
test('OpenSpec deltaHash 不包含 checkout 绝对路径', (t: any) => {
  ...
  const firstFile: any = writeDelta(first, 'demo', normalized.replaceAll('\n', '\r\n').replace('identity。', 'identity。   '));
  const secondFile: any = writeDelta(second, 'demo', normalized);
  const runtime: any = createRuntime();

  const firstDelta: any = runtime.parseOpenSpecChangeDelta(first);
  const secondDelta: any = runtime.parseOpenSpecChangeDelta(second);
...
test('OpenSpec deltaHash 在逻辑 delta 输入变化时改变', (t: any) => {
  ...
  const baseHash: any = runtime.parseOpenSpecChangeDelta(base).hash;
