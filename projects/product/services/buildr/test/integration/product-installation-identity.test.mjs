...
function windowsPlatformOrigin(version = '1.2.3') {
  return createInstallationOrigin({
    channel: 'platform',
    runtimeRole: 'product',
    package: '@buildr-ai/buildr',
...
function payloadManifest(origin) {
  const value = {
...
function originForManifest(channel, manifest) {
  return createInstallationOrigin({
    channel,
...
test('formal origin is bound to all payload identity fields while development remains payload-independent', () => {
  const initial = npmOrigin();
  const manifest = payloadManifest(initial);
  const matching = originForManifest('npm', manifest);
...
test('npm origin is read only from its explicit payload envelope receipt', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-origin-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const productRoot = path.join(root, 'payload', 'product');
  ...
  const manifest = payloadManifest(npmOrigin());
  const boundOrigin = originForManifest('npm', manifest);
  ...
  const origin = readCurrentInstallationOrigin(productRoot, { payloadRoot: root });
  ...
  const mismatched = readCurrentInstallationOrigin(productRoot, { payloadRoot: root });
...
test('product invocation always uses explicit Host or development Node without PATH lookup', () => {
  const host = currentProductInvocation({ cliPath: '/immutable/runtime/buildr.cjs', kind: 'host-node' });
