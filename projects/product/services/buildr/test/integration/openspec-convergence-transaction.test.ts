...
import { applyCanonicalPlan } from '../../src/task/openspec/application/canonical-applier.ts';
import { createConvergencePlan } from '../../src/task/openspec/application/convergence-planner.ts';
import { observeConvergence } from '../../src/task/openspec/application/convergence-observer.ts';
import { createConvergenceReceipt, portableExecutableIdentity } from '../../src/task/openspec/application/convergence-model.ts';
import { convergenceReceiptPath, runOpenSpecConvergence } from '../../src/task/openspec/application/openspec-converge.ts';
import { parseChangeChecklistText } from '../../src/task/openspec/application/change-checklist.ts';
import { createDeterministicSyncPlan } from '../../src/task/openspec/application/deterministic-sync.ts';

const requirement: any = (title: any, body: any = '系统 MUST 保持行为。', scenario: any = '正常') => `### Requirement: ${title}\n${body}\n\n#### Scenario: ${scenario}\n- **WHEN** 输入有效\n- **THEN** 系统 MUST 成功\n`;
...
function planFor(content: any, operations: any, options: any = {}): any  {
...
test('planner相同输入产生相同identity且不依赖baseline', () => {
  ...
  const first: any = planFor(before, operations);
  const second: any = planFor(before, operations);
...
test('planner聚合语义冲突并拒绝可执行写入', () => {
  const before: any = canonical(requirement('Existing'));
  const operations: any[] = [{ type: 'MODIFIED', capability: 'demo', title: 'Missing', requirement: requirement('Missing') }];
  const plan: any = planFor(before, operations, { activeConflicts: [{ code: 'active-change-conflict', change: 'other' }] });
  assert.equal(plan.status, 'blocked');
  assert.equal(plan.blocked.some((item: any) => item.code === 'requirement-not-unique'), true);
  assert.equal(plan.blocked.some((item: any) => item.code === 'active-change-conflict'), true);
});

test('planner按确定顺序返回MODIFIED省略的Scenario identities', () => {
  const existing: any = `${requirement('Existing')}\n#### Scenario: zeta\n- **WHEN** zeta成立\n- **THEN** 系统 MUST 保留\n\n#### Scenario: alpha\n- **WHEN** alpha成立\n- **THEN** 系统 MUST 保留\n`;
  const before: any = canonical(existing);
  const operations: any[] = [{ type: 'MODIFIED', capability: 'demo', title: 'Existing', requirement: requirement('Existing', '系统 MUST 更新行为。') }];
  const plan: any = planFor(before, operations);
  assert.equal(plan.status, 'blocked');
...
test('observer只按before expected实际digest判断恢复状态', () => {
  const before: any = canonical(requirement('Existing'));
  const plan: any = planFor(before, [{ type: 'ADDED', capability: 'demo', title: 'Added', requirement: requirement('Added') }]);
  const receipt: any = createConvergenceReceipt({ plan, executableIdentity });
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-observer-'));
  try {
    const file: any = path.join(root, receipt.files[0].path);
...
test('observer遇到多文件before expected混合状态时fail closed', () => {
  ...
  const receipt: any = createConvergenceReceipt({ plan, executableIdentity });
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-observer-mixed-'));
  try {
    receipt.files.forEach((item: any, index: any) => {
      const file: any = path.join(root, item.path);
      fs.mkdirSync(path.dirname(file), { recursive: true });
...
test('canonical applier在before漂移时整批零写入', () => {
  const before: any = canonical(requirement('Existing'));
  const plan: any = planFor(before, [{ type: 'ADDED', capability: 'demo', title: 'Added', requirement: requirement('Added') }]);
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-applier-'));
  try {
    const file: any = path.join(root, plan.files[0].path);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, `${before}\nconcurrent\n`);
    const applied: any = applyCanonicalPlan({ projectRoot: root, plan, currentDeltaDigest: plan.deltaDigest, currentExecutableIdentity: executableIdentity, io: fs });
...
test('删除capability全部Requirements时投影并原子删除canonical spec', () => {
  const before: any = canonical(requirement('Only'));
  const plan: any = planFor(before, [{ type: 'REMOVED', capability: 'demo', title: 'Only' }]);
  ...
  const receipt: any = createConvergenceReceipt({ plan, executableIdentity });
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-applier-delete-'));
  try {
    const file: any = path.join(root, plan.files[0].path);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, before);
...
test('完整REMOVED delta重放时保持已经不存在的capability为expected absent', () => {
  const operations: any[] = [{ type: 'REMOVED', capability: 'demo', title: 'Only' }];
  const plan: any = createConvergencePlan({
...
function journey(options: any = {}): any  {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-converge-journey-'));
  const changeRoot: any = path.join(root, 'openspec', 'changes', 'change-a');
  const file: any = path.join(root, 'openspec', 'specs', 'demo', 'spec.md');
  ...
  const operations: any = options.operations || [{ type: 'ADDED', capability: 'demo', title: 'Added', requirement: requirement('Added') }];
  const context: any = { change: 'change-a', project: 'product', projectRoot: root, changeRoot, archived: false, delta: delta(operations, options.hash) };
  let archiveCalls: any = 0;
  let releaseCalls: any = 0;
  let currentExecutableIdentity: any = executableIdentity;
  const archivedRoot: any = path.join(root, 'openspec', 'changes', 'archive', '2026-07-27-change-a');
  const releaseReceipt: any = options.releaseFailsOnce ? (target: any) => {
    releaseCalls += 1;
  ...
  const run: any = () => runOpenSpecConvergence({
    context,
    ...
    writeReceipt: (target: any, value: any) => { fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, `${JSON.stringify(value, null, 2)}\n`); },
    releaseReceipt,
    io: fs,
  });
  return {
    ...
    setExecutableIdentity: (value: any) => { currentExecutableIdentity = value; },
  };
}
...
test('未完成checklist在receipt与canonical写入前fail closed', () => {
  const fixture: any = journey({ tasks: '- [x] planned\n- [ ] post-archive work\n' });
  try {
    const before: any = fs.readFileSync(fixture.file, 'utf8');
    const result: any = fixture.run();
...
test('archive成功但事务Receipt释放失败时只重试终态释放', () => {
  const fixture: any = journey({ releaseFailsOnce: true });
  try {
    const first: any = fixture.run();
    ...
    assert.equal(second.execution.some((item: any) => ['plan', 'apply', 'archive'].includes(item.id)), false);
  } finally { fs.rmSync(fixture.root, { recursive: true, force: true }); }
});
...
test('历史Archived Change的旧Receipt保持原样且不重新审计canonical', () => {
  ...
  try {
    ...
    const receiptFile: any = convergenceReceiptPath(fixture.archivedRoot);
    const historical: any = JSON.parse(fs.readFileSync(receiptFile, 'utf8'));
    ...
    const repeated: any = fixture.run();
...
test('projected strict失败时canonical零写入', () => {
  const fixture: any = journey({ validateProjected: () => ({ status: 'blocked', code: 'projected-strict-validation-failed', durationMs: 1, commandCount: 1 }) });
  try {
    const before: any = fs.readFileSync(fixture.file, 'utf8');
    const result: any = fixture.run();
    assert.equal(result.status, 'blocked');
...
test('apply完成receipt仍planned时按actual expected继续确认', () => {
  const fixture: any = journey({ archiveFailsOnce: true });
  try {
    const first: any = fixture.run();
    assert.equal(first.status, 'blocked');
    const receiptFile: any = convergenceReceiptPath(fixture.changeRoot);
    const receipt: any = JSON.parse(fs.readFileSync(receiptFile, 'utf8'));
    receipt.disposition = 'planned-not-applied';
    fs.writeFileSync(receiptFile, `${JSON.stringify(receipt, null, 2)}\n`);
    const second: any = fixture.run();
    assert.equal(second.status, 'passed');
    assert.equal(second.execution.some((item: any) => item.id === 'apply'), false);
  } finally { fs.rmSync(fixture.root, { recursive: true, force: true }); }
});
...
test('archive失败后只重试确认与archive', () => {
  ...
  try {
    ...
    const second: any = fixture.run();
    assert.equal(second.status, 'passed');
    assert.equal(second.execution.some((item: any) => item.id === 'plan' || item.id === 'apply' || item.id === 'projected-validation'), false);
...
test('delta在收尾期间变化时丢弃旧计划并以当前canonical重新规划', () => {
  ...
  try {
    assert.equal(fixture.run().status, 'blocked');
    const nextOperations: any[] = [{ type: 'ADDED', capability: 'demo', title: 'Added Again', requirement: requirement('Added Again') }];
    fixture.context.delta = delta(nextOperations, 'sha256-delta-next');
    const result: any = fixture.run();
    assert.equal(result.status, 'passed');
    assert.equal(result.execution.some((item: any) => item.id === 'plan'), true);
    const content: any = fs.readFileSync(fixture.file, 'utf8');
...
test('executable identity变化时旧验证不复用并重新规划验证', () => {
  ...
  try {
    ...
    assert.equal(result.status, 'passed');
    assert.equal(result.execution.some((item: any) => item.id === 'plan'), true);
    assert.equal(result.execution.some((item: any) => item.id === 'projected-validation'), true);
  } finally { fs.rmSync(fixture.root, { recursive: true, force: true }); }
});
...
test('旧convergence receipt缺少plan时返回recovery-unprovable且不写canonical', () => {
  ...
  try {
    const before: any = fs.readFileSync(fixture.file, 'utf8');
    const legacyFile: any = path.join(fixture.changeRoot, '.buildr', 'deterministic-convergence.json');
...
test('完整旧plan与post-sync receipt按真实expected迁移且不恢复canonical', () => {
  const fixture: any = journey();
  try {
    const oldPlan: any = createDeterministicSyncPlan({
    ...
    const buildrRoot: any = path.join(fixture.changeRoot, '.buildr');
    ...
    const result: any = fixture.run();
    assert.equal(result.status, 'passed');
    assert.equal(result.execution.some((item: any) => item.id === 'apply'), false);
...
test('旧v2 receipt与plan自洽但delta已变化时先观察旧状态再按当前delta重规划', () => {
  ...
  try {
    ...
    const result: any = fixture.run();
    assert.equal(result.status, 'passed');
    assert.equal(result.execution.some((item: any) => item.id === 'observe'), true);
    assert.equal(result.execution.some((item: any) => item.id === 'plan'), true);
    assert.equal(result.execution.some((item: any) => item.id === 'apply'), true);
...
test('孤立旧recovery receipt无法证明时fail closed', () => {
  const fixture: any = journey();
  try {
    const recoveryFile: any = path.join(fixture.changeRoot, '.buildr', 'convergence-recovery.json');
...
test('portable executable identity不保存机器绝对路径', () => {
  const identity: any = portableExecutableIdentity({ projectRoot: '/workspace/project', executable: '/Users/example/bin/openspec', version: '1.6.0', sha256: 'abc' });
