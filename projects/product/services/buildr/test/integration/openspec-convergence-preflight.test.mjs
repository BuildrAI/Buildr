...
function run({ before, operations, activeConflicts = [], activeChanges = [], validation = { status: 'passed', code: null, durationMs: 1, commandCount: 1 }, canonicalIdentity = 'sha256-canonical' }) {
  const context = { change: 'change-a', project: 'product', delta: delta(operations) };
  const canonicalFiles = new Map([['demo', { path: 'openspec/specs/demo/spec.md', exists: true, content: before }]]);
...
test('semantic readiness preflight复用planner并只返回当前ready观察', () => {
  ...
  const first = run({ before, operations });
  const second = run({ before, operations });
...
test('readiness identity绑定canonical与全部active Change observations', () => {
  const before = canonical(requirement('Existing'));
  const operations = [{ type: 'ADDED', capability: 'demo', title: 'Added', requirement: requirement('Added') }];
  const baseline = run({ before, operations });
  const canonicalChanged = run({ before, operations, canonicalIdentity: 'sha256-new-canonical' });
  const activeChanged = run({
    before,
    operations,
