...
test('任务详情使用概览、预演、研发、证据、复盘、环境六个一级视图', () => {
  ...
  assert.match(coordination, /parent-overview-hero[\s\S]*父任务目标[\s\S]*当前动作[\s\S]*parent-summary-strip[\s\S]*可启动[\s\S]*最终验收/);
  ...
  assert.match(coordinationStyles, /parent-progress-group[\s\S]*parent-progress-card/);
  assert.match(coordinationStyles, /parent-actual-children[\s\S]*parent-delivery-summary[\s\S]*parent-wait-reason/);
