...
test('fresh Workspace 按完整 SQL scripts 初始化且重复只读打开零写入', (t) => {
  ...
  assert.equal(writable.version, 5);
  ...
  assert.deepEqual(writable.database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name").all().map((row) => row.name), [
    'schema_migrations', 'task_changes', 'task_development_current', 'task_projects', 'task_retrospective_current', 'task_review_current', 'task_services', 'task_verification_current', 'tasks',
  ]);
  ...
  for (const table of ['task_development_current', 'task_verification_current', 'task_review_current', 'task_retrospective_current']) {
  ...
  assert.equal(readOnly.version, 5);
  readOnly.database.close();
  assert.equal(fs.statSync(file).mtimeMs, before);
  assert.deepEqual(runtime.inspectWorkspaceStructuredStore(root), { status: 'healthy', version: 5, integrity: 'ok' });
});
...
test('checksum 漂移、版本超前和损坏数据库均 fail closed', (t) => {
  ...
  opened.database.prepare(`INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (6, '0006_future.sql', 'sha256-${'f'.repeat(64)}', ?)`).run(new Date().toISOString());
  opened.database.close();
...
test('Doctor 区分未初始化、healthy 与 unavailable 且不暴露数据库 path', (t) => {
  ...
  assert.deepEqual(result.structuredStore, { status: 'healthy', version: 5, integrity: 'ok' });
