...
const operations: any = new Map(TASK_RECORD_HTTP_OPERATIONS.map((operation: any) => [operation.id, operation]));
...
function assertSchema(operationId: any, kind: any, value: any): any  {
  ...
  const result: any = TASK_RECORD_HTTP_VALIDATORS.validate(schemaId, value);
...
test('Task Record contract catalog、DTO drift 与未迁移诊断保持局部', async () => {
  ...
  assert.deepEqual(TASK_RECORD_HTTP_OPERATIONS.map((operation: any) => operation.id), [
    'task-record.list',
    'task-record.detail',
    'task-record.update',
  ...
  const coverage: any = inspectTaskRecordHttpContractCoverage([
    ...TASK_RECORD_HTTP_OPERATIONS.map((operation: any) => operation.id),
...
test('Ajv request validation rejects retired and unknown fields without mutating input', () => {
  ...
  const result: any = TASK_RECORD_HTTP_VALIDATORS.validate(operations.get('task-record.complete').requestSchemaId, input);
...
test('非法 DTO 在调用 Task Record Application writer 前被拒绝', async () => {
  ...
  await assert.rejects(
    handleTaskRecordHttpRequest({
      ...
      root: '/unused',
      runtime: { updateTaskRecord(): any  { calls += 1; } },
      authorizeWrite(): any  {},
...
test('六个 Task Record operation 的真实 HTTP 成功与错误响应匹配 Schema', async (t: any) => {
  ...
  const updateTask: any = runtime.createTaskRecord(root, { taskId: 'contract-update', title: '更新前', intent: 'HTTP contract', projects: [], services: [], changes: [] });
  const completeTask: any = runtime.createTaskRecord(root, { taskId: 'contract-complete', title: '待完成', intent: 'HTTP contract', projects: [], services: [], changes: [] });
  const abandonTask: any = runtime.createTaskRecord(root, { taskId: 'contract-abandon', title: '待放弃', intent: 'HTTP contract', projects: [], services: [], changes: [] });
  ...
  const parentRecord: any = runtime.inspectTaskRecord(root, 'http-parent');
