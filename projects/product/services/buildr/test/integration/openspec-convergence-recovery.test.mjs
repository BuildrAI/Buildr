...
import { continueConvergenceRecoveryReceipt, createConvergenceRecoveryReceipt, inspectConvergenceRecovery } from '../../src/task/openspec/application/convergence-recovery.mjs';
import {
  applyDeterministicSyncPlan,
  DETERMINISTIC_SYNC_PLAN_SCHEMA,
  deterministicSyncContentDigest,
  deterministicSyncPlanIdentity,
} from '../../src/task/openspec/application/deterministic-sync.mjs';

function fixture(t) {
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-convergence-recovery-'));
  t.after(() => fs.rmSync(projectRoot, { recursive: true, force: true }));
  const relative = 'openspec/specs/demo/spec.md';
  const file = path.join(projectRoot, relative);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const before = '# demo\n\n### Requirement: One\nBefore.\n\n#### Scenario: A\n- **WHEN** a\n- **THEN** a\n';
  const expected = before.replace('Before.', 'After.');
  ...
  const baseline = { schemaVersion: 'buildr.openspec-contract-baseline/v1', change: 'demo-change', project: 'product', upstreamVersion: '1.6.0', deltaHash: 'sha256-old', adopted: false, targets: [] };
  const executableIdentity = { sourceKind: 'external-declared', reference: 'external:openspec', version: '1.6.0', sha256: 'binary-1' };
  const receipt = {
...
test('post-sync精确匹配时生成身份绑定的反向恢复计划', (t) => {
  ...
  const receipt = createConvergenceRecoveryReceipt(result);
  assert.equal(receipt.stage, 'planned');
  assert.equal(receipt.oldPlanIdentity, state.syncPlan.identity);
});

test('反向恢复复用确定性同步的严格验证和原子替换', (t) => {
  const state = fixture(t);
  const result = inspect(state);
  const applied = applyDeterministicSyncPlan({
    projectRoot: state.projectRoot,
    plan: result.reversePlan,
    validateExpected: ({ files }) => ({ status: 'passed', expectedDigests: Object.fromEntries(files.map((item) => [item.path, item.digest])) }),
  });
  assert.equal(applied.status, 'passed');
  assert.equal(fs.readFileSync(state.file, 'utf8'), state.before);
  const resumed = inspect(state);
...
test('canonical额外漂移需要语义处理且零写入', (t) => {
  const state = fixture(t);
...
test('缺少旧计划或证明链不匹配时明确不可恢复', (t) => {
  const state = fixture(t);
  const missing = inspectConvergenceRecovery({
  ...
  const mismatch = inspectConvergenceRecovery({
    projectRoot: state.projectRoot, change: 'demo-change', project: 'product', newDeltaHash: 'sha256-new',
...
test('连续delta修订轮换已完成凭证并保留历史链', (t) => {
  const firstPlan = inspect(fixture(t));
  const first = createConvergenceRecoveryReceipt(firstPlan, 'completed', [{ stage: 'completed' }]);
  const secondPlan = { ...firstPlan, identity: 'sha256-second', oldDeltaHash: firstPlan.newDeltaHash, newDeltaHash: 'sha256-third' };
  const rotated = continueConvergenceRecoveryReceipt(secondPlan, first);
  ...
  const unrelated = continueConvergenceRecoveryReceipt({ ...secondPlan, oldDeltaHash: 'sha256-unrelated' }, first);
