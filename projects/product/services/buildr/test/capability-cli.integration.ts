...
function run(args: any, expected: any = 0, options: any = {}): any  {
  return new Promise((resolve: any, reject: any) => {
    const child: any = spawn(process.execPath, [buildr, ...args], options);
    let stdout: any = '';
    let stderr: any = '';
...
function writeSkill(root: any, id: any): any  {
  const directory: any = path.join(root, `${id}-source`);
  fs.mkdirSync(directory, { recursive: true });
  fs.writeFileSync(path.join(directory, 'SKILL.md'), `---\nname: ${id}\ndescription: ${id} fixture\n---\n\n# ${id}\n`);
  return directory;
}

function manifest(root: any, scope: any = '.'): any  {
  const scopeRoot: any = scope === '.' ? root : path.join(root, scope);
  return YAML.parse(fs.readFileSync(path.join(scopeRoot, 'skills', 'manifest.yml'), 'utf8'));
}

async function doctor(root: any, scope: any = '.', expected: any = 0): Promise<any>  {
  const result: any = await run(['doctor', '--target', root, '--scope', scope, '--json', '--detail', 'full'], expected);
  return JSON.parse(result.stdout);
}

function consumer(result: any, scope: any, id: any): any  {
  const graph: any = result.capabilities.graphs.find((item: any) => item.scope === scope);
  return graph?.consumers.find((item: any) => item.consumer === id);
}
...
test('CLI 集成验证 provider 替换、绑定与 builtin 恢复', { concurrency: true }, async (t: any) => {
  ...
  assert.equal(manifest(root).skills.some((item: any) => item.id === 'invalid-provider'), false, 'unknown contract declaration must fail before write');
  ...
  let workspaceManifest: any = manifest(root);
  assert.equal(workspaceManifest.schemaVersion, 'buildr.skills/v3');
  assert.equal(workspaceManifest.bindings.find((item: any) => item.capability === 'buildr.git-operations').provider, 'git-operations', 'provider install must not silently change binding');

  await run(['skills', 'bind', 'buildr.git-operations@1', '--provider', 'internal-git', '--scope', '.', '--target', root]);
  const internalVerificationSource: any = writeSkill(root, 'internal-verification');
  await run([
    'skills', 'add', '--source', internalVerificationSource, '--scope', '.', '--target', root,
    '--provides', 'buildr.task-verification@4',
  ]);
  assert.equal(manifest(root).bindings.find((item: any) => item.capability === 'buildr.task-verification').provider, 'task-verification');
  ...
  const uninstallGit: any = await run(['builtin', 'uninstall', 'git-operations', '--target', root, '--reason', 'internal replacement']);
  ...
  assert.equal(workspaceManifest.skills.find((item: any) => item.id === 'git-operations').state, 'uninstalled');
  const readyDoctor: any = await doctor(root);
  assert.equal(consumer(readyDoctor, '.', 'task-finish').readiness, 'ready');
  assert.equal(consumer(readyDoctor, '.', 'task-finish').dependencies.some((item: any) => item.capability === 'buildr.task-verification'), false);
  assert.doesNotMatch(JSON.stringify(readyDoctor.capabilities), /sourceFile|absolutePath|skillContributions/);
  const humanDoctor: any = await run(['doctor', '--target', root, '--scope', '.']);
  ...
  assert.equal(manifest(root).skills.find((item: any) => item.id === 'git-operations').state, 'installed');
});
...
test('CLI 集成验证 Git Operations required/optional consumers、legacy Project 拒绝与 Project override', { concurrency: true }, async (t: any) => {
  ...
  const unboundReport: any = await doctor(root);
  const workspaceFinish: any = consumer(unboundReport, '.', 'task-finish');
  assert.equal(workspaceFinish.readiness, 'degraded');
  assert.equal(workspaceFinish.dependencies.some((item: any) => item.capability === 'buildr.git-operations'), true);
  assert.equal(unboundReport.capabilities.graphs.some((graph: any) => graph.providers?.some((item: any) => item.provider === 'task-metadata-publication')), false);
  ...
  const projectManifestPath: any = path.join(root, 'projects', 'demo', 'skills', 'manifest.yml');
  ...
  assert.ok((await doctor(root, 'projects/demo', 1)).findings.some((finding: any) => finding.code === 'skills.project_assets_unsupported' && finding.status === 'error' && !finding.suggestion.includes('migrate-project-assets')));
  const projectSource: any = writeSkill(root, 'project-git');
  await run([
  ...
  const projectFinish: any = consumer(await doctor(root, 'projects/demo', 1), 'projects/demo', 'task-finish');
  assert.equal(projectFinish.readiness, 'ready', 'optional Git Operations dependency resolves through Project override');
  const projectGitDependency: any = projectFinish.dependencies.find((item: any) => item.capability === 'buildr.git-operations');
  assert.equal(projectGitDependency.selectedProvider.id, 'project-git');

  const projectCapabilities: any = YAML.parse(fs.readFileSync(path.join(root, 'projects', 'demo', 'capabilities.yml'), 'utf8'));
...
test('skills render 将 source workspace 与 user/workspace destination 分离并复用用户投射', { concurrency: true }, async (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-skill-destination-'));
  const userHome: any = path.join(root, 'user-home');
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test('skills render 对用户层同名外部资产输出稳定 JSON 并整次零写入', { concurrency: true }, async (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-skill-conflict-'));
  const userHome: any = path.join(root, 'user-home');
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const external: any = path.join(userHome, '.agents', 'skills', 'task-triage');
  ...
  const conflict: any = report.conflicts.find((item: any) => item.skillId === 'task-triage');
...
test('已删除 Project Skill 自动迁移 route 返回 unknown-command 且不改写旧 source', { concurrency: true }, async (t: any) => {
  ...
  const beforeManifest: any = fs.readFileSync(path.join(legacyRoot, 'manifest.yml'));
  const beforeSkill: any = fs.readFileSync(path.join(legacyRoot, 'legacy-demo', 'SKILL.md'));
  for (const mode of ['--check', '--apply']) {
    const result: any = await run(['skills', 'migrate-project-assets', '--target', root, mode, '--json'], 2);
