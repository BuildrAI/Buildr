...
import { createReleaseTransactionEvidence, inspectHostedReleaseTransaction, validateReleaseTransactionEvidence } from '../../tools/release/release-transaction-evidence.ts';
import { ensureReleaseTag, inspectReleaseTag } from '../../tools/release/release-tag-ensure.ts';
...
const runId: any = 987;
const runAttempt: any = 1;
const fixtureCommit: any = 'a'.repeat(40);
const candidateBase: any = 'b'.repeat(40);
const candidateTree: any = 'c'.repeat(40);
const candidateSourceCommit: any = 'e'.repeat(40);
const version: any = '0.1.0-rc.15';
...
const gitSha: any = (letter: any) => letter.repeat(40);

function readyTaskCorrelation(releaseTask: any, supportTasks: any): any  {
  return createReleaseTaskEvidenceCorrelation({
    releaseTask: { ...releaseTask, recordDigest: digest('1') },
    supportTasks: supportTasks.map((task: any) => ({ ...task, recordDigest: digest('1') })),
    source: { sourceCommit: fixtureCommit, sourceTree: candidateTree, remoteRef: fixtureCommit },
  });
}

function hostedEnvironment(sourceCommit: any = fixtureCommit): any  {
  return {
...
function releaseContext(): any  {
  ...
  preparation.identity = `sha256-${sha256(JSON.stringify(preparation))}`;
...
function probeRepo(t: any, prefix: any): any  {
  const repo: any = fs.mkdtempSync(path.join(os.tmpdir(), prefix));
...
test('hosted OIDC probe exchanges identity but never retains either token', async (t: any) => {
  const repo: any = probeRepo(t, 'buildr-authority-probe-');
  const created: any = '2026-08-13T00:04:00.000Z';
  const expires: any = '2026-08-13T01:04:00.000Z';
  const requests: any[] = [];
  const result: any = await runReleaseAuthorityOidcProbe({ repo, sourceCommit: fixtureCommit, workflowSha256: sha256(workflow) }, {
...
test('hosted OIDC probe normalizes Registry Unix-second exchange timestamps', async (t: any) => {
  const repo: any = probeRepo(t, 'buildr-authority-probe-unix-time-');
  const created: any = '2026-08-13T00:04:00.000Z';
  const expires: any = 1786583040;
  let requestCount: any = 0;
  const result: any = await runReleaseAuthorityOidcProbe({ repo, sourceCommit: fixtureCommit, workflowSha256: sha256(workflow) }, {
    env: hostedEnvironment(),
    now: () => '2026-08-13T00:05:00.000Z',
    fetchImpl: async () => {
...
test('hosted OIDC probe fails closed on exchange rejection without parsing a secret body', async (t: any) => {
  const repo: any = probeRepo(t, 'buildr-authority-probe-reject-');
  let count: any = 0;
  ...
  }), (error: any) => error.code === 'npm_oidc_exchange_failed' && error.actual.status === 403);
});

test('hosted OIDC probe CLI creates the nested evidence directory even when blocked', (t: any) => {
  const repo: any = probeRepo(t, 'buildr-authority-probe-output-');
  const output: any = path.join(repo, 'runner-temp', 'authority', 'release-authority.json');
  const result: any = spawnSync(process.execPath, [
    path.join(serviceRoot, 'tools', 'release', 'release-authority-oidc-probe.ts'),
    '--repo', repo,
  ...
  const evidence: any = JSON.parse(fs.readFileSync(output, 'utf8'));
...
test('release transaction runner dispatches and follows exactly one frozen workflow run', async () => {
  ...
  const context: any = releaseContext();
  const readiness: any = await runHostedReleaseTransaction({ repo: '/fixture', sourceCommit: 'origin/main', remoteMain: 'origin/main', version, candidateBase, candidateTree, releaseContext: context, ghCommand: 'gh', timeoutMs: 1_000 }, { execute, wait: async () => {}, releaseId: 'fixture-release-id', onStatus: () => {} });
  assert.equal(readiness.status, 'ready');
  assert.deepEqual(readiness.effects, []);
  const unauthorized: any = await runHostedReleaseTransaction({ action: 'dispatch', repo: '/fixture', sourceCommit: 'origin/main', remoteMain: 'origin/main', version, candidateBase, candidateTree, releaseContext: context, ghCommand: 'gh', timeoutMs: 1_000 }, { execute, wait: async () => {}, releaseId: 'fixture-release-id', onStatus: () => {} });
  ...
  for (const input of [`release_id=fixture-release-id`, `version=${version}`, `source_commit=${fixtureCommit}`, `candidate_base=${candidateBase}`, `candidate_tree=${candidateTree}`, `workflow_sha256=${sha256(workflow)}`, `context_digest=${context.identity}`, 'candidate_run_id=654', `release_context=${JSON.stringify(context)}`]) {
    assert.equal(calls.some((item: any) => item.includes(input)), true, input);
  }
  assert.equal(calls.some((item: any) => item.startsWith('git tag ') || item.startsWith('git push ')), false);
  assert.equal(calls.some((item: any) => item.startsWith('npm ')), false);
});

test('release transaction runner binds preparation inputs to the final frozen source commit', async (t: any) => {
  const repo: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-source-binding-'));
  t.after(() => fs.rmSync(repo, { recursive: true, force: true }));
  ...
  for (const [file, contents] of sourceFiles) {
    const target: any = path.join(repo, file);
  ...
  const devCommit: any = 'd'.repeat(40);
  const oldPackage: any = `${JSON.stringify({ name: '@buildr-ai/buildr', version: '0.1.0-rc.14' })}\n`;
  const calls: any[] = [];
  const execute: any = (command: any, args: any) => {
    const key: any = [command, ...args].join(' ');
    calls.push(key);
    ...
    if (key === `git show ${fixtureCommit}:.github/workflows/publish.yml`) return { status: 0, stdout: workflow };
    if (key === 'npm ci') return { status: 0, stdout: 'installed' };
...
test('release transaction evidence is a closed correlated read model', () => {
  ...
  const extended: any = structuredClone(evidence);
  extended.sidecar = {};
  assert.throws(() => validateReleaseTransactionEvidence(extended), /sidecar is not supported/u);
  const beforePublicWrite: any = createReleaseTransactionEvidence({
    context,
  ...
  assert.deepEqual(beforePublicWrite.attempt.steps.find((step: any) => step.id === 'tag'), { id: 'tag', status: 'not-reached' });
  const afterTagFailure: any = createReleaseTransactionEvidence({
    context,
    publish: evidence.publish,
    outcome: 'failed',
  ...
  assert.deepEqual(afterTagFailure.attempt.steps.find((step: any) => step.id === 'npm'), { id: 'npm', status: 'failed' });
  const conflicted: any = createReleaseTransactionEvidence({
    context,
    publish: evidence.publish,
    outcome: 'failed',
  ...
  const retrying: any = createReleaseTransactionEvidence({
    context,
    publish: evidence.publish,
    outcome: 'failed',
...
test('release transaction finalizer preserves official Registry integrity in the hosted artifact', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-finalize-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const contextFile: any = path.join(root, 'context.json');
  const registryFile: any = path.join(root, 'registry.json');
  const output: any = path.join(root, 'release-transaction-evidence.json');
  ...
  const result: any = spawnSync(process.execPath, [
    path.join(serviceRoot, 'tools', 'release', 'release-transaction-evidence.ts'), 'finalize',
  ...
  const evidence: any = validateReleaseTransactionEvidence(JSON.parse(fs.readFileSync(output, 'utf8')));
  assert.equal(evidence.release.registryIntegrity, 'sha512-aW50ZWdyaXR5');
  assert.equal(evidence.release.registrySmoke, 'passed');
});

test('release transaction inspect reads exactly one hosted artifact and rejects cross-run drift', async (t: any) => {
  const evidence: any = createReleaseTransactionEvidence({
  ...
  const temporary: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-inspect-test-'));
  t.after(() => fs.rmSync(temporary, { recursive: true, force: true }));
  let readbackRunId: any = runId;
  const execute: any = (_command: any, args: any) => {
    if (args[0] === 'run' && args[1] === 'download') {
      const directory: any = args[args.indexOf('--dir') + 1];
      const artifact: any = path.join(directory, 'release-evidence-fixture', 'contract');
  ...
  const result: any = await inspectHostedReleaseTransaction({ runId, repository: 'BuildrAI/Buildr', ghCommand: 'gh' }, { execute, makeTempDirectory: () => temporary, removeDirectory: () => {} });
...
test('release tag ensure creates once, reuses the same source, and rejects drift', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-tag-ensure-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const remote: any = path.join(root, 'remote.git');
  const repo: any = path.join(root, 'repo');
  ...
  const first: any = git(repo, 'rev-parse', 'HEAD');
  ...
  const second: any = git(repo, 'rev-parse', 'HEAD');
  const drift: any = inspectReleaseTag({ repo, tag: 'v0.1.0', sourceCommit: second });
...
test('release tag ensure accepts a concurrent writer only when remote resolves to the same source', () => {
  const sourceCommit: any = 'd'.repeat(40);
  let pushed: any = false;
  const execute: any = (command: any, args: any) => {
    const key: any = [command, ...args].join(' ');
...
test('release authority preflight CLI writes static ready evidence without control-plane mutation', (t: any) => {
  const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-release-authority-cli-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const repo: any = path.join(root, 'repo');
  ...
  const sourceCommit: any = git(repo, 'rev-parse', 'HEAD');
  const fakeGh: any = path.join(root, 'fake-gh.mjs');
...
`, { mode: 0o755 });
  ...
  const evidence: any = JSON.parse(fs.readFileSync(evidencePath, 'utf8'));
