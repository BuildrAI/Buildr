...
const tarballForbiddenPrefixes: any[] = [
  '.agents/', '.claude/', '.codex/', '.git/', '.github/', '.worktrees/', 'openspec/', 'projects/', 'test/',
];

function finding(rule: any, relativePath: any, message: any): any  {
...
export function inspectPackageMetadata(metadata: any): any  {
  const findings: any[] = [];
...
export function inspectPackageVersionConsistency(metadata: any, lockfile: any): any  {
  const findings: any[] = [];
  if (lockfile?.version !== metadata?.version) findings.push(finding('package.version-lock', 'projects/product/services/buildr/package-lock.json', 'top-level lockfile version must match package.json'));
  if (lockfile?.packages?.['']?.version !== metadata?.version) findings.push(finding('package.root-version-lock', 'projects/product/services/buildr/package-lock.json', 'root package lock version must match package.json'));
  return findings;
}

export function inspectTarballFiles(files: any): any  {
  const findings: any[] = [];
  const paths: any = new Set(files.map((entry: any) => entry.path));
  ...
  for (const entry of paths) {
    if (tarballForbiddenPrefixes.some((prefix: any) => entry === prefix.slice(0, -1) || entry.startsWith(prefix))) {
...
function trackedFiles(): any  {
  const result: any = spawnSync('git', ['ls-files', '-z', '--cached', '--others', '--exclude-standard'], { cwd: workspaceRoot, encoding: 'buffer' });
  if (result.status !== 0) throw new Error(`git ls-files failed with exit ${result.status}`);
  return result.stdout.toString('utf8').split('\0').filter(Boolean);
}

export function inspectCandidatePaths(root: any, relativePaths: any): any  {
  const findings: any[] = [];
...
function inspectReadmes(): any  {
  const findings: any[] = [];
  const readmes: any = ['README.md', 'README.en.md'].map((file: any) => [file, fs.readFileSync(path.join(workspaceRoot, file), 'utf8')]);
...
export async function runOpenSourceCandidateCheck(): Promise<any>  {
  const metadata: any = JSON.parse(fs.readFileSync(path.join(productRoot, 'package.json'), 'utf8'));
  const lockfile: any = JSON.parse(fs.readFileSync(path.join(productRoot, 'package-lock.json'), 'utf8'));
  return [
...
async function main(): Promise<any>  {
  const findings: any = await runOpenSourceCandidateCheck();
  if (findings.length) {
