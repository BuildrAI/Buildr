...
export function launchPlatformLauncher({ platform = process.platform, target, workspace, environment }: any): any  {
  if (platform === 'darwin') {
    const opened: any = spawnSync('/usr/bin/open', [
  ...
  const child: any = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', 'Start-Process -FilePath $env:BUILDR_LAUNCHER_SHORTCUT -Wait'], {
    cwd: workspace,
