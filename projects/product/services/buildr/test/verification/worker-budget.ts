export const VERIFICATION_WORKER_BUDGET_ENV: any = 'BUILDR_VERIFICATION_WORKER_BUDGET';

export function resolveVerificationWorkerBudget({ env = process.env, fallback, maximum, label }: any): any  {
  ...
  const configured: any = env[VERIFICATION_WORKER_BUDGET_ENV];
  ...
  const budget: any = Number(configured);
