...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--profile') {
      if (!['core', 'candidate'].includes(args[index + 1])) throw new Error('Missing or invalid --profile value.');
...
const lane = request.profile === 'core' ? { budgetMs: CORE_TOTAL_BUDGET_MS, kind: 'core', source: 'core-profile' } : { budgetMs: CANDIDATE_TOTAL_BUDGET_MS, kind: 'candidate', source: 'candidate-profile' };
const plan = admitVerificationPlanBudget(
  createVerificationAdmissionPlan(createVerificationPlan({ profiles: [request.profile] })),
...
if (request.json) {
  ...
  await new Promise((resolve, reject) => process.stdout.write(`${JSON.stringify({ schemaVersion: 'buildr.verification-full-plan/v1', status: plan.status, diagnostic: plan.diagnostic, base: null, source: lane.source, paths: plan.paths, scope: plan.scope, estimate: plan.estimate, delegated: plan.delegated, admissionStepIds: plan.admissionStepIds, preflightSteps: [], steps: plan.steps.map(project) }, null, 2)}\n`, (error) => error ? reject(error) : resolve()));
