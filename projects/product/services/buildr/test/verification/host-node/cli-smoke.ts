...
import { readSharedCandidatePackage } from '../release/candidate-package.ts';

const productRoot: any = path.resolve(import.meta.dirname, '../../..');
const root: any = fs.mkdtempSync(path.join(os.tmpdir(), 'buildr-host-node-cli-'));
...
const npmExecutable: any = process.platform === 'win32' ? 'npm.cmd' : 'npm';
const windowsRuntimeSource: any = process.platform === 'win32' ? path.dirname(fs.realpathSync(process.execPath)) : null;
...
async function waitForInstance(file: any, child: any, timeoutMs: any = 20_000): Promise<any>  {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const instance: any = JSON.parse(fs.readFileSync(file, 'utf8'));
...
async function stopWeb(child: any): Promise<any>  {
...
try {
  ...
  const buildrScript: any = path.join(modulesRoot, '@buildr-ai', 'buildr', 'bin', 'buildr.mjs');
  const instanceFile: any = path.join(appData, 'instance.json');
  ...
  const cliIdentity: any = JSON.parse(run(process.execPath, [buildrScript, 'version', '--json'], { cwd: workspace, env: runtimeEnv }));
  ...
  web.stdout.on('data', (chunk: any) => { stdout += chunk; });
  ...
  const health: any = await fetch(`${instance.url}/api/v1/health`, {
    headers: { 'x-buildr-instance': instance.secret },
    signal: AbortSignal.timeout(2_000),
  });
  assert.equal(health.status, 200);
  const readiness: any = await health.json();
  ...
  const shell: any = await fetch(instance.url, { signal: AbortSignal.timeout(2_000) });
