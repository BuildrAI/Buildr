...
import { resolveVerificationExecutionProfile, VERIFICATION_GROUPS, verificationSteps } from './registry.ts';
...
export function usage(stream: any = process.stdout): any  {
  stream.write('Usage: npm run test:focus -- [--plan|--json] <step-id|group:<group>>...\n');
  stream.write('       npm run test:focus -- --list\n');
}

export function parseFocusArgs(args: any): any  {
  const result: any = { help: false, list: false, planOnly: false, json: false, stepIds: [], groups: [] };
...
export function resolveFocusPlan(parsed: any): any  {
  const unknownGroups: any = parsed.groups.filter((group: any) => !VERIFICATION_GROUPS.includes(group));
...
export function printFocusCatalog(stream: any = process.stdout): any  {
  stream.write('Groups:\n');
...
if (process.argv[1] && sameFilesystemPath(process.argv[1], fileURLToPath(import.meta.url))) {
  let temporaryRoot: any;
  try {
    const parsed: any = parseFocusArgs(process.argv.slice(2));
    if (parsed.help) usage();
    else if (parsed.list) printFocusCatalog();
    else {
      const plan: any = resolveFocusPlan(parsed);
      const payload: any = {
      ...
      if (parsed.json) process.stdout.write(`${JSON.stringify(payload, null, 2)}\n`);
        ...
        const evidence: any = createVerificationEvidencePaths('focus', { temporaryRoot });
        ...
        else process.stdout.write(`Buildr focus verification passed: ${[...plan.stepIds, ...plan.groups.map((group: any) => `group:${group}`)].join(' ')}\n`);
      }
    }
  } catch (error: any) {
