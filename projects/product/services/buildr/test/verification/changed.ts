...
import { admitVerificationPlanBudget, createDevelopmentPlatformPlan, createVerificationAdmissionPlan, createVerificationPlan, createVerificationSelectionAudit } from './planner.ts';
import { executePlan, printPlan } from './plan-runner.ts';
import { resolveVerificationExecutionProfile } from './registry.ts';
import { CORE_TOTAL_BUDGET_MS } from './timing/budgets.ts';
...
function usage(stream: any = process.stdout): any  {
...
function parseArgs(args: any): any  {
  const result: any = { base: null, developmentRunner: null, planOnly: false, json: false, paths: [] };
...
let executionRoot: any;
let evidence: any;
let results: any[] = [];
let totalStartedAt: any;
let source: any;
try {
  ...
  if (args.help) {
    ...
    const changed: any = collectChangedProductPaths({ productRoot, projectRoot, base: args.base, explicitPaths: args.paths });
    const executionProfile: any = resolveVerificationExecutionProfile(process.env.BUILDR_VERIFICATION_PROFILE);
    const affectedPlan: any = args.developmentRunner
