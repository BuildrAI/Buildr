...
const governanceInvariants: any = Object.freeze([
  { id: 'action-local-doctor-readiness', owner: 'integration-declarations', files: ['test/integration/core-diagnostics-and-package.test.ts'] },
...
test('前序治理结果由现有最低充分 Integration owner 集中覆盖', () => {
  const slices: any = new Map(INTEGRATION_PRIMARY_SLICES.map((slice: any) => [slice.id, slice]));
  const fileOwners: any = new Map();
  ...
  for (const invariant of governanceInvariants) {
    const owner: any = slices.get(invariant.owner);
...
test('Affected反馈与完整Candidate保持不同范围且每个step只出现一次', () => {
  const affected: any = createVerificationPlan({ paths: ['resources/workspace/skills/buildr/task-finish/SKILL.md'] });
  const candidate: any = createVerificationPlan({ profiles: ['candidate'] });
  const affectedIds: any = affected.steps.map((step: any) => step.id);
  const candidateIds: any = candidate.steps.map((step: any) => step.id);
  ...
  assert.equal(candidate.steps.filter((step: any) => step.executor.type === 'candidate-artifact').length, 1);
});
...
test('分布式Candidate只有一个artifact producer且所有consumer显式依赖artifact', () => {
  ...
  assert.equal(CANDIDATE_CI_SHARDS.filter((shard: any) => shard.producesArtifact).length, 1);
  const byId: any = new Map(verificationSteps.map((step: any) => [step.id, step]));
  for (const shard of CANDIDATE_CI_SHARDS) {
    const consumesArtifact: any = shard.stepIds.some((id: any) => byId.get(id)?.executor?.consumesArtifact);
    if (consumesArtifact) assert.equal(shard.requiresArtifact, true, shard.id);
  }
});

test('正式Release消费冻结artifact且不重跑完整Product Candidate', () => {
  const workflow: any = YAML.parse(fs.readFileSync(path.join(repositoryRoot, '.github/workflows/publish.yml'), 'utf8'));
  const allRuns: any = Object.values(workflow.jobs)
  ...
  assert.equal(allRuns.includes('application-payload.ts build'), false);
...
test('治理质量contract不再用Skill篇幅或章节位置冒充行为证据', () => {
  const source: any = read('test/contract/direct-git-closeout.test.ts');
