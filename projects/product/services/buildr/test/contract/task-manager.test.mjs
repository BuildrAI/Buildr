...
test('task-manager routing 正向命中正式记录，负向排除普通任务和专业阶段', () => {
  ...
  for (const positive of ['创建、查看、更新、完成或放弃正式 Task Record', '按 Task ID 恢复', '首次持久交付写入前']) assert.ok(`${manager}\n${triage}`.includes(positive), positive);
...
test('task-triage 的 optional provider 不 ready 只降级依赖，不让其他分支伪造成功', () => {
  ...
  const file = path.join(root, 'skills', 'manifest.yml'); const manifest = YAML.parse(fs.readFileSync(file, 'utf8')); manifest.skills.find((item) => item.id === 'task-manager').state = 'uninstalled'; fs.writeFileSync(file, YAML.stringify(manifest, { lineWidth: 0 }));
  const graph = resolveSkillCapabilityGraph(root, null, { runtime: 'codex' }); const triage = graph.consumers.find((item) => item.consumer === 'task-triage'); const dependency = triage.dependencies.find((item) => item.capability === 'buildr.task-record');
  assert.equal(triage.readiness, 'degraded'); assert.equal(dependency.readiness, 'degraded'); assert.equal(dependency.reason, 'invalid_binding');
