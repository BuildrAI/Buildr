...
const skillRoot = 'resources/workspace/skills/buildr/ux-design-laws';
...
test('ux-design-laws 作为无 capability contract 的 optional builtin Skill 投射完整目录', () => {
  ...
  const frontmatterDescription = skill.match(/^description: (.+)$/m)?.[1];
  const packaged = manifest.builtins.skills.find((entry) => entry.id === 'ux-design-laws');
  ...
  assert.equal(manifest.capabilityContracts.some((entry) => entry.id.includes('ux-design-laws')), false);
  assert.equal(manifest.initialSkillBindings.some((entry) => entry.capability.includes('ux-design-laws')), false);
...
test('ux-design-laws 覆盖 30 个主题并保持卡片和来源完整', () => {
  const references = referenceFiles.map((relative) => read(`${skillRoot}/${relative}`)).join('\n');
  const lawHeadings = [...references.matchAll(/^## .+（.+）$/gm)].map(([heading]) => heading);
  const sourceUrls = [...references.matchAll(/https:\/\/lawsofux\.com\/[^)]+/g)].map(([url]) => url);
  ...
  const index = read(`${skillRoot}/references/law-index.md`);
...
test('ux-design-laws 从真实证据选择法则并保持原型、实现和用户利益边界', () => {
  ...
  const uiPrototype = read('resources/workspace/skills/buildr/ui-prototype/SKILL.md');
  assert.match(uiPrototype, /未明确确认时不得使用/);
});

test('ux-design-laws Codex 元数据保持自动发现和默认提示一致', () => {
  const metadata = YAML.parse(read(`${skillRoot}/agents/openai.yaml`));
