...
function mappingSource(entry: any): any  {
  if (typeof entry === 'string') return entry.split(/\s+=>\s+/u, 1)[0].trim();
  return entry?.source;
}

test('用户 Registry 不作为产品包源或映射存在', () => {
  const manifest: any = YAML.parse(fs.readFileSync(path.join(serviceRoot, 'resources/manifest.yml'), 'utf8'));
  const mappedSources: any = [...manifest.workspaceFiles, ...manifest.projectFiles].map(mappingSource);
...
test('产品内容 YAML 仍由包发布，不与用户 Registry 混淆', () => {
  const targetRoot: any = path.join(serviceRoot, 'resources/workspace');
  const yamlFiles: any = fs.readdirSync(targetRoot, { recursive: true, withFileTypes: true })
    .filter((entry: any) => entry.isFile() && /\.ya?ml$/u.test(entry.name))
    .map((entry: any) => path.relative(targetRoot, path.join(entry.parentPath, entry.name)).split(path.sep).join('/'));
  assert.equal(yamlFiles.every((relativePath: any) => relativePath.startsWith('skills/') || relativePath.startsWith('commands/buildr/') || relativePath.startsWith('components/buildr/')), true);
