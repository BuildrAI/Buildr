...
test('task-verification v4 keeps Agent orchestration and minimal Applications', () => {
  const skill = ['SKILL.md', 'references/maintain-map.md', 'references/record-report.md']
    .map((file) => read(`resources/workspace/skills/buildr/task-verification/${file}`)).join('\n');
