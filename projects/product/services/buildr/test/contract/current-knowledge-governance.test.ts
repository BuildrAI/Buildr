...
test('terminology 与 current knowledge contracts 具有稳定 identity 和固定语义章节', () => {
  const terminology: any = path.join(WORKSPACE_TARGET, 'skills/contracts/buildr/terminology-governance/v1.md');
  const knowledgeV1: any = path.join(WORKSPACE_TARGET, 'skills/contracts/buildr/current-knowledge-maintenance/v1.md');
  const knowledgeV2: any = path.join(WORKSPACE_TARGET, 'skills/contracts/buildr/current-knowledge-maintenance/v2.md');
...
test('默认 providers 与 bindings 可解析，当前认知与收尾只按实际需要消费已有能力', () => {
  ...
  assert.ok(triage.requires.some((item: any) => item.capability === 'buildr.current-knowledge-maintenance' && item.version === 2 && item.mode === 'optional'));
...
test('current knowledge provider 同时提供 v1/v2 且 maintain 不伪造 Change', () => {
  ...
  const provider: any = manifest.builtins.skills.find((item: any) => item.id === 'current-knowledge-maintenance');
  ...
  const skill: any = read(path.join(WORKSPACE_TARGET, 'skills/buildr/current-knowledge-maintenance/SKILL.md'));
...
test('OpenSpec capability dependencies 由 Component 与 fragments 原子维护', () => {
  ...
  const has: any = (skill: any, capability: any, mode: any) => dependencies.some((item: any) => item.skill === skill && item.capability === capability && item.mode === mode);
