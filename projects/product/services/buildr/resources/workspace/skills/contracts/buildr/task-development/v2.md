...
维护正式 Task 自身选择的研发过程，维护唯一 Development Receipt、可选 planning facts、可选Parent Plan/Contribution binding、Content Target、verification policy、Task Candidate generation、专业 gate/disposition、推进决定与不可变 handoff。Task Development 是研发聚合事实 authority；它不是 Task Core、artifact/Result内容 writer、测试执行器、Git交付器或通用状态机。
...
- Contribution Handoff只作为同一immutable handoff的可选专业事实；必须绑定current Parent Plan与已保存planned binding。Parent coordination read model只组合Task Record、已保存研发事实、历史交付与明确的贡献处置，不新增progress/lifecycle/event/history store，不回写Child状态。
- 已完成且非no-change的直接Child可通过既有`task parent reconcile-child-delivery`显式登记贡献映射；输入绑定current Parent Plan、直接关系、`expectedTaskDigest`、reason与source，应用保存任务结果identity并在事务内重验相关事实。无需Child Development或旧Finish，完成状态本身不推断贡献。历史v1只读，新的v2记录继续使用同一表，不覆盖已有不同处置。
...
贡献登记是显式结果判断，不是机器验证；调用方必须先核对真实成果。父任务的最终验收仍独立，不补造旧候选、交接或收尾运行。
