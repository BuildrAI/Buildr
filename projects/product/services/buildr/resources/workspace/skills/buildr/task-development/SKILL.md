...
description: 正式Task从首个proposal、方案或直接实现开始，到稳定Content Target、Task Candidate、Completion Review、Current Knowledge、推进决定与研发交接的全过程使用；不用于Task Record、Task Verification、测试执行或交付执行。
---
...
5. 通过`task-review`读取Planning Review。Result不存在、not-applicable与waived必须明确区分。
...
Planning current、Content Target稳定且Change非pending后，可调用`freeze`形成或复用Task Candidate。Candidate只绑定generation、Content Target与Task Context；Task Verification报告变化不得改变Candidate或generation。
...
Candidate形成后，使用`task-review`完成Completion Review，或记录明确not-applicable/waived。根据Development自身事实调用`decide`记录proceed/blocked；新的risk只允许绑定Completion Review，不接受Task Verification digest。

只有Planning、Candidate、Completion Review、Current Knowledge和proceed decision均current时，才能调用`handoff`形成append-only快照。Task Finish是否使用该快照由收尾目标决定。

## 与Task Verification的边界
...
只报告实际存在的planning identity、Content Target、Candidate generation、Completion Review、Current Knowledge、decision与handoff。Task Verification报告单独报告，不写入Development证据，也不得把commit、branch或worktree当Candidate。
