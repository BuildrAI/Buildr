...
`passed` 至少需要一个实际检查且全部通过；`not-passed` 必须有失败检查；`incomplete` 用于没有失败但仍有必要边界未覆盖的情况。查看当前适用性时传入真实 `--content-identity`；提供时内容适用性为 `current`（与报告内容版本一致）或 `stale`（内容版本已变），未提供时为 `unknown`，不据此重新测试。登记结果已返回当前报告、`slot.reportDigest` 和适用性，可直接核对；仅响应丢失、冲突或相关事实变化时补读。
