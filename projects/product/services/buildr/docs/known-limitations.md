...
- `@buildr-ai/buildr@0.1.0-rc.8` 是当前已发布 RC，`next` 指向该版本；`0.1.0-rc.9` 正在准备，尚未发布。`latest` 仍可能指向历史 prerelease，它不代表稳定版。稳定版 `0.1.0` 尚未发布，公开试用应显式安装 `@next`。`0.1.0-rc.4` 因发布范围错误已弃用。
