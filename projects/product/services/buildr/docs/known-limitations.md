...
- `@buildr-ai/buildr@0.1.0-rc.12` 是当前已发布 RC，`next` 指向该版本；`v0.1.0-rc.9`、`v0.1.0-rc.10`、`v0.1.0-rc.11` 和 `v0.1.0-rc.13` 的 tag workflow 都在任何 npm 写入与 GitHub Release 创建前失败，当前仓库正在准备 `0.1.0-rc.14`。`latest` 仍指向历史 prerelease，它不代表稳定版。稳定版 `0.1.0` 尚未发布，公开试用应显式安装 `@next`。`0.1.0-rc.4` 因发布范围错误已弃用。
