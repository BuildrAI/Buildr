...
3. 冻结前完成full Product Candidate、current knowledge、OpenSpec convergence与release authority tuple检查；完成release Task交付及`dev → main`历史收敛后，只有main/dev version、tree、tag target和release contract一致才允许创建tag。tag、publish、Release mutation与凭证配置各自需要额外明确授权。
...
6. 可逆门禁通过后才进入受`npm-production`Environment保护的公开写入。目标npm version缺失时发布同一tarball；已存在时只接受`dist.integrity`完全相同。publish后有界核对version/integrity/dist-tag并从Registry安装精确版本复跑Host Node CLI/Web/Launcher smoke。
