...
3. `release-<version>`协调Task覆盖selection、完整Candidate、唯一tarball、release→main与readiness并在终点前保持active/blocked；版本材料、测试修复或owner修复使用窄support Task提前完成Development/Verification/Finish。support terminal、Delivery或Activation不使release协调Taskcompleted。release Task Environment只由`service:product/buildr/buildr.npm-ci`在Buildr Service root准备依赖并冻结Plan/declaration/recipe/lockfile/exact Node identities；Task/Environment/Development/Finish/self-bootstrap各自提供current read model，不复制Result或建立旁路store。
