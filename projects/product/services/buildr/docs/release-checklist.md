...
5. Candidate通过后，若current main与冻结release发生历史或内容冲突，先在release execution worktree执行一次`release-selection.mjs reconcile-main --confirm --reason <text>`；冲突由维护者明确解决，成功后生成新的release generation并对新SHA重跑完整Candidate。随后为current generation创建或复用确定性`codex/release-main-<version>-g<generation>` carrier，并只从该carrier创建一个release→main受保护PR；正式远端`release-<version>`保持独立。该PR固定使用GitHub `Create a merge commit`，readback必须证明两个父提交与`origin/main^{tree}`精确等于当前冻结release tree。tree、父提交、carrier、PR head或ownership漂移时在publication前停止。
