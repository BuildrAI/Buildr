...
`0.1.0-rc.1`、`0.1.0-rc.2`、`0.1.0-rc.3`、`0.1.0-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7` 和 `0.1.0-rc.8` 已完成 npm 发布和 GitHub prerelease 创建；`0.1.0-rc.4` 因发布范围错误已弃用；`v0.1.0-rc.9` tag 已存在，但tag workflow在任何npm写入和GitHub Release创建前因Host Node checkout验证依赖缺失而失败，npm `next`仍指向rc.8。rc.10以新版本修复该链路，不删除、移动或覆盖rc.9 tag；后续正式发布仍需单独明确授权。
