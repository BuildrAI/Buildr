...
1. 维护者明确目标`<version>`、精确`<dev-baseline>`和有序选择commit；release owner从可由current `dev`证明的baseline创建唯一`release-<version>`，后续只接受明确选择且带`-x`provenance的dev commit或明确授权release-only metadata。普通dev前进不改变release，冲突不自动解决；freeze同时保存不可变`freezes/<generation>`历史ref，frozen不能直接update。
2. 唯一身份链为`dev baseline → selection chain → release HEAD/tree → Product Candidate generation → frozen tarball → generation carrier → main → post-publish dev convergence → closeout → transaction evidence`。任一上游identity变化使旧Candidate、artifact、readiness和context stale。
3. `release-<version>`协调Task覆盖selection、完整Candidate、唯一tarball、release→main、readiness、Publication、main→dev与必需closeout，在lifecycle `closed`前保持active/blocked；版本材料、测试修复或owner修复使用窄support Task提前完成Development/Verification/Finish。support terminal、Delivery、Activation、Candidate或readiness通过不使release协调Task completed。release Task Environment只由`service:product/buildr/buildr.npm-ci`在Buildr Service root准备依赖并冻结Plan/declaration/recipe/lockfile/exact Node identities；Task/Environment/Development/Finish/self-bootstrap各自提供current read model，不复制Result或建立旁路store。
4. current release HEAD/tree运行同一分布式Candidate owner集合：preflight、唯一tarball、macOS core、Windows runtime/Launcher、Workspace/Task、fresh build和四个Host Node tuple。现有exact Node/PATH、primary owner、affected/full、bounded scheduling、heartbeat/checkpoint与timing保持不变；普通changed/affected反馈不是完整Candidate。aggregate失败或source不匹配时release协调Task保持active/blocked；若需要选择新dev修复，先回读GitHub、tag、npm与GitHub Release，证明无公开mutation后显式`reopen --confirm --reason`，逐commit update、refreeze、普通push并对新SHA重跑完整Candidate。已有公开事实则使用新version。
5. Candidate通过后为current generation创建或复用确定性`codex/release-main-<version>-g<generation>` carrier，并只从该carrier创建一个release→main受保护PR；正式远端`release-<version>`保持独立。squash commit可以不同，但`origin/main^{tree}`必须精确等于冻结release tree。tree、carrier、PR head或ownership漂移时在publication前停止。
6. 准备阶段transaction runner默认只执行shared readiness，收集selection、Candidate/artifact、Task correlation、Environment/exact Node、main/dev与workflow facts，返回frozen context digest、collect-all findings、hosted deferred checks与`effects: []`；全部current后lifecycle进入`awaiting-publication-authorization`，release协调Task仍保持active，不dispatch、不请求`npm-production`审批、不模拟OIDC、不创建tag。
...
9. Publication成功后才收敛main→dev；先校验dev branch policy允许普通双亲merge commit，并保留release创建后进入dev的新内容。策略不兼容、冲突或remote race返回`published-but-dev-convergence-blocked`与稳定recovery identity，不管理员绕过、`ours`、reset、force push、删tag或unpublish。
10. 已发布版本不覆盖；RC问题发新prerelease，GA问题发patch。Environment按owner清理；release closeout默认保留并核验正式远端release ref，必需清理generation carrier、本地selection branch/refs、owned worktree与临时资源。全部通过后才以no-change完成唯一协调Task；正式ref可选删除仍需独立授权且不阻止完成。

`0.1.0-rc.1`、`0.1.0-rc.2`、`0.1.0-rc.3`、`0.1.0-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8`、`0.1.0-rc.12`、`0.1.0-rc.14`、`0.1.0-rc.15`、`0.1.0-rc.18`、`0.1.0-rc.19`、`0.1.0-rc.20`和`0.1.0-rc.21`已完成npm发布和GitHub prerelease创建；`0.1.0-rc.4`因发布范围错误已弃用。`v0.1.0-rc.9`tag workflow因Host Node checkout验证依赖缺失而失败；`v0.1.0-rc.10`已补齐独立`npm ci`，但两个Host Node jobs未向verifier传入冻结`release-artifact.json`而确定性失败；`v0.1.0-rc.11`修复Host Node wiring后进入publish job，但frozen Application Payload仍把开发仓用户态`.buildr/workspace.yml`当作必需资源，因该源已被正确移除而在任何公共写入前fail closed。rc.12随后移除全部Workspace、Project与Service用户态配置发布源；`v0.1.0-rc.13`的publish job在安装依赖前加载release contract时因间接依赖`yaml`而失败，同样没有执行任何公共写入。rc.14、rc.15、rc.18、rc.19、rc.20和rc.21均已修复并发布；rc.15将正式发布收敛为单次transaction与一次`npm-production`审批。rc.16通过源码Candidate gate，但发布任务的closeout evidence在后续正式修复任务交付后不再匹配current `dev`，因此没有创建tag、npm version或GitHub Release；rc.17完成正式验证与自举激活后，因rc.16 squash merge的`main → dev`历史衔接缺失导致新PR冲突，同样没有创建任何公开版本。该历史已在保持`dev`内容不变的前提下修复。rc.20首次Candidate因retained cleanup测试入口递归使一个capability不退出；修复后完整Candidate与发布通过，执行器可观测性和发布关联缺口由后续正式Change治理，不把进程采样或runner暂态当作该根因。当前仓库候选版本为`0.1.0-rc.22`；首次Candidate run `32729369444`在任何tag/npm/GitHub Release或正式release transaction前失败，修复通过support Task进入`dev`后按受控reopen/refreeze恢复。原`release-0.1.0-rc.22`协调Task被提前completed，保留为历史异常事实，剩余准备使用明确active recovery Task承载。
