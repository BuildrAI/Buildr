...
Task Finish的CLI adapter只解析`run|inspect`。首次run从Environment与Task Development Application取得Task、current handoff、Candidate和Content Target，不接受Project/Change、Candidate/generation或Verification authority输入。`task-finish-run.mjs`持有breaking v2 canonical run store、产品resume token与结果投射；`task-finish-product-executor.mjs`只组合内容等价Git carrier、fast-forward/push、retained sync/install/doctor和Environment cleanup。它不执行OpenSpec、formal Verification、Completion Review、Candidate freeze或风险决定；target race与内容漂移终止run并返回Development，只有carrier未变的暂态外部条件可以resume。没有action registry、caller completion、typed recovery parser或旧v1 reader；retained metadata-only精确Git handoff属于Task Finish Skill，不进入CLI产品executor。
