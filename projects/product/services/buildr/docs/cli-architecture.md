...
Task Finish 的 current run、lease 和 compact terminal Result 统一写入 Workspace SQLite；完整诊断与 Carrier 只登记在 run-owned transient root，并在成功后清理。`.buildr/task-finish` 是已退役的旧文件协议，启用 SQLite-only 前由受控步骤直接清理，不是执行 authority。`task complete` 只表达 Task Record terminal status，不建立第二套 Finish 状态机。
