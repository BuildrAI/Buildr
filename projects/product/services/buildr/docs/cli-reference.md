...
| `buildr verification run --project <code> --capability <id> ... --target-identity <identity>` | 使用 Workspace 受管 Node 执行 Project `verification.yml` v2 中显式选择的 command capabilities。绑定正式环境时同时传 `--environment <task-id> --workspace <canonical-workspace> --candidate-identity <identity> --candidate-generation <n>`；Receipt 固定的 retained controller 编排正式 execution 与 canonical record，capability 仍在候选 execution root 执行。正式Task的exact invocation已有active或terminal record时默认零执行复用，只有显式`--retry`创建同identity独立run/record。返回`buildr.verification-execution/v1`，不选择applicability、不写current Result。 |
