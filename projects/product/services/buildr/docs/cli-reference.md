...
| `buildr verification run --project <code> --level affected\|candidate` | 使用 Workspace 受管 Node 执行 Project `verification.yml`。需要绑定正式环境时同时传 `--environment <task-id> --workspace <canonical-workspace>`；执行根必须属于该 Environment。返回候选、环境 source/projection identity 与验证 evidence，不证明 Agent session。 |
...
Environment Receipt 固定保存在 canonical Workspace 的 `.buildr/tasks/<task-id>/environment.json`，使用 `buildr.task-environment-receipt/v2`。它记录实际 scope、执行根、任务验证工作区根、稳定 controller、Runtime/CLI/依赖/projection probes、provider evidence 引用、已知动态资源以及最近 ready/cleanup 事实；不写入 Task Record，不保存凭证、任意 cleanup 命令或 Agent session。共享执行根只允许一个未清理的重叠 Task 占用；同一 Task 的 `prepare` 复用原放置计划，不在恢复时静默切换 shared/Git。
