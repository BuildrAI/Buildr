...
Task Metadata Publication首版没有公共Application或CLI。Agent通过唯一`task-metadata-publication` Skill组合`task.yml`、`development.yml`、`verification.yml`、`reviews/planning.yml`、`reviews/completion.yml`五个writer-declared exact paths，并required调用`buildr.git-operations/v1`完成独立commit/push。Skill helper只做只读snapshot、commit tree、等价commit与完整range检查；Environment、Finish、asset-review、runtime、Candidate、delivery source与其他Task不进入scope。无Git Workspace返回local-only并保留records。
