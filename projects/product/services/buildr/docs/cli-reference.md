...
Task Finish 的 current run、target lease 和 compact terminal Result 统一写入 Workspace SQLite；完整诊断与 Delivery Carrier 仅登记在 run-owned transient root，并在成功后清理。`.buildr/task-finish` 只用于一次性 legacy cutover 或 Doctor 诊断，不是执行 authority。`task complete` 仅表达 Task Record 的 terminal status，不是第二个 Finish 流程。
