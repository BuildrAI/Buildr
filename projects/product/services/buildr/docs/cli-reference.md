...
| `buildr task finish run\|inspect` | 首次 `run` 接收 `--task <task-id>` 并要求 ready Environment 与当前研发交接（Development Handoff）；可显式给出 agent、target branch/remote，但不接受 Project/Change、Candidate/generation 或 Verification authority 输入。Git-backed run默认冻结retained checkout当前符号分支，显式target必须与其一致，Environment `startPoint`不作为交付分支；再按显式值、Environment、target branch upstream、唯一配置remote解析真实remote，缺失、歧义或不一致时停止。固定执行 `preflight → prepare → verify → deliver → cleanup`：在隔离交付载体（Delivery Carrier）上机械复用或保留交付适配（Delivery Adaptation）现场、fast-forward/普通 push、push后远端ref回读、retained sync/install/doctor并把cleanup交给Environment；只有after ref等于carrier才报告远端交付完成，formal Verification次数固定为0。P0.5 不公开 `task development` CLI，bundled `task-development` Skill 通过内部 driver 调用唯一 Application；Local App 仅提供 `inspect` 只读投影。 |
...
Task Record 使用 closed `buildr.task-record/v1` schema。Project、Service 通过 Workspace registry 校验，新增 Change 引用通过 Task-scoped Change Reference Resolver 从 ready Task Environment Project root或 retained Project 解析。Change 不需要为适配校验而提前写入 retained Project。引用暂时不可用时 inspect/list 返回诊断，删除该引用或修改无关字段仍可继续。Task Record 不保存 Environment path、identity 或 provenance。Repository 在 `.buildr/local/workspace.sqlite` 的规范化关系表中以单事务维护 Task；读取结果附带不持久化的逻辑 `recordDigest`，供 Local App 拒绝陈旧页面。只读操作不会初始化数据库；旧 `task.yml` 不读取、不迁移、不双写、不删除。

Task Metadata Publication首版没有公共Application或CLI。Agent通过唯一`task-metadata-publication` Skill组合`development.yml`、`verification.yml`、`reviews/planning.yml`、`reviews/completion.yml`四个writer-declared exact paths，并required调用`buildr.git-operations/v1`完成独立commit/push。Task Record SQLite 数据是local-only，不进入Git或同步。Skill helper只做只读snapshot、commit tree、等价commit与完整range检查；Environment、Finish、asset-review、runtime、Candidate、delivery source与其他Task不进入scope。无Git Workspace返回local-only并保留records。
