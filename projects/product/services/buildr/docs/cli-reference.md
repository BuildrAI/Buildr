...
Environment Receipt使用`buildr.task-environment-receipt/v4`，由Task Environment Application固定保存在canonical Workspace SQLite的`task_environment_current`。Agent先根据Task完整Service scope与构建/验证事实登记closed`buildr.task-environment-plan/v1`：每个Service包含有序Step或显式not-applicable。`task environment plan record|inspect`管理saved Plan，`prepare --plan <file>`支持一次登记并准备。Step只接受无shellexecutable、args、Service相对cwd、inputs、expected outputs、required和timeout；核心不枚举package manager、不扫描manifest。Receipt保存逐Service聚合及逐Step executable/input current/prepared identity、output状态与诊断。任一required Step缺失、漂移或失败都会阻断整体ready。`inspect`只读实时观察且不执行Step、不创建输出、不回写；Local App GET只读取saved current。v2/v3只读兼容，必须显式提供Plan才能升级。
