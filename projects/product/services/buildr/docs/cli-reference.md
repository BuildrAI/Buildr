...
Task Finish 的 current run、target lease 和 compact terminal Result 统一写入 Workspace SQLite；完整诊断与 Delivery Carrier 仅登记在 run-owned transient root，并在成功后清理。`.buildr/task-finish` 是已退役的旧文件协议，启用 SQLite-only 前由受控步骤直接清理，不是执行 authority。`task complete` 仅表达 Task Record 的 terminal status，不是第二个 Finish 流程。
...
Environment Receipt 使用 `buildr.task-environment-receipt/v3`，由 Task Environment Application 固定保存在 canonical Workspace SQLite 的 `task_environment_current`，公开 locator 为 `workspace-sqlite:task-environment/<task-id>`。它记录实际 scope、Task checkout/provider evidence、执行根、任务验证工作区根、Receipt 创建时的 manager 指纹、逐 dependency-root 的 manifest/lockfile 与已准备 identity、scope 聚合 probe、Runtime/CLI/projection、已知动态资源以及最近 ready/cleanup 事实；不写入 Task Record，不保存凭证、任意 cleanup 命令或 Agent session。每个 npm root 使用自己的 lockfile 和 worktree-local `node_modules`，安装只通过 Workspace Foundation 的受管 npm；任一 required root 缺失、漂移或失败都会阻断整体 ready。`inspect` 只读实时观察依赖漂移，`prepare` 才执行首次安装或幂等恢复；Local App GET 只读取已保存 current。v2 current 保持可读，但 active v2 必须显式 `prepare` 才升级。manager 指纹不决定 Task 源码版本、ready、资源 ownership 或 Verification applicability。共享执行根只允许一个未清理的重叠 Task 占用；同一 Task 的 `prepare` 复用原放置计划，不在恢复时静默切换 shared/Git，也不自动 fetch/rebase Task checkout。旧 Environment 文件不再读取、导入或作为 fallback。
