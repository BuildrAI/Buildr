...
| `buildr task finish run\|inspect` | 首次`run`接收`--task <task-id>`并要求ready Environment与current Development handoff；可显式给出agent、target branch/remote，但不接受Project/Change、Candidate/generation或Verification authority输入。Git-backed run默认冻结retained checkout当前符号分支，显式target必须与其一致，Environment `startPoint`不作为交付分支；再按显式值、Environment、target branch upstream、唯一配置remote解析真实remote，缺失、歧义或不一致时停止。固定执行`preflight → prepare → verify → deliver → cleanup`：只准备内容等价carrier、fast-forward/普通push、push后远端ref回读、retained sync/install/doctor并把cleanup交给Environment；只有after ref等于carrier才报告远端交付完成，formal Verification次数固定为0。P0.5不公开`task development`CLI，bundled`task-development`Skill通过内部driver调用唯一Application。 |
