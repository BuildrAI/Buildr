...
| `buildr verification run --project <code> --capability <id> ... --target-identity <identity>` | 使用 Workspace 受管 Node 执行 Project `verification.yml` v2 中显式选择的 command capabilities。需要绑定正式环境时同时传 `--environment <task-id> --workspace <canonical-workspace>`；返回 transient `buildr.verification-execution/v1`，不选择 applicability、不写 current Result。`--declaration-root` 只属于 `task verification inspect\|record`，不属于 execution 入口。 |
