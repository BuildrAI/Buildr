...
const STAGES: any = new Set(['pre-candidate', 'pre-main', 'dispatch-check', 'pre-tag']);
...
function canonical(value: any): any  {
  ...
  return Object.fromEntries(Object.keys(value).sort().map((key: any) => [key, canonical(value[key])]));
}

export function releaseContextIdentity(value: any): any  {
  return `sha256-${crypto.createHash('sha256').update(JSON.stringify(canonical(value))).digest('hex')}`;
}

function optionalProjection(value: any, fields: any, label: any): any  {
  if (value == null) return null;
  closed(value, fields, label);
  return canonical(value);
}

export function createReleaseContext(input: any): any  {
...
export function validateReleaseContext(value: any): any  {
  ...
  const { schemaVersion: _schemaVersion, identity: _identity, ...input }: any = value;
  const recreated: any = createReleaseContext(input);
...
function finding(code: any, owner: any, expected: any, actual: any, nextAction: any): any  {
  return { code, severity: 'blocked', owner, expected, actual, nextAction };
}

function checkIdentity(findings: any, value: any, field: any, owner: any): any  {
...
function checkSha(findings: any, value: any, field: any, owner: any): any  {
...
export function evaluateReleaseReadiness(input: any): any  {
  ...
  const context: any = validateReleaseContext(input.context);
  const findings: any[] = [];
  ...
  if (context.selection?.reconciliationIdentity && ['dispatch-check', 'pre-tag'].includes(input.stage)) {
    ...
    const mergeParents: any = context.convergence?.mergeParents;
  ...
  const ready: any = findings.length === 0;
  return {
    ...
    effects: [],
    nextActions: ready ? (input.stage === 'dispatch-check' ? ['等待维护者对当前frozen context明确授权publication。'] : []) : [...new Set(findings.map((item: any) => item.nextAction))],
  };
}
...
if (process.argv[1] && sameFilesystemPath(process.argv[1], fileURLToPath(import.meta.url))) {
  try {
    ...
    const result: any = options.action === 'validate-context'
      ? validateReleaseContext(input)
      : options.action === 'evaluate'
        ? evaluateReleaseReadiness({ stage: options.stage, context: input })
        : (() => { throw new Error('Usage: release-readiness.ts validate-context --input <json> [--output <json>] | evaluate --stage <stage> --input <json> [--output <json>]'); })();
