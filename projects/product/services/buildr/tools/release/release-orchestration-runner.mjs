...
function normalizeDoctorResult(value) {
...
export function resolveRetainedController(canonicalWorkspace, { nodeExecutable = process.execPath } = {}) {
  const root = fs.realpathSync(path.resolve(canonicalWorkspace));
  const sourceRoot = fs.realpathSync(path.join(root, 'projects/product/services/buildr'));
  ...
  const executable = fs.realpathSync(path.resolve(nodeExecutable));
  if (!path.isAbsolute(executable)) throw new Error('Release retained controller executable is invalid.');
  const requiredNode = fs.readFileSync(path.join(root, 'projects/product/.node-version'), 'utf8').trim();
...
async function closeout(options, dependencies) {
  ...
  let controller;
  try { controller = (dependencies.resolveRetainedController ?? resolveRetainedController)(root); } catch (error) {
