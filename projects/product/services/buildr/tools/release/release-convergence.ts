...
function runGit(repo: any, args: any, { allowFailure = false }: any = {}): any  {
...
function rev(repo: any, ref: any): any  {
  return runGit(repo, ['rev-parse', ref]).stdout.trim();
}

function packageVersionAt(repo: any, ref: any): any  {
  const result: any = runGit(repo, ['show', `${ref}:projects/product/services/buildr/package.json`], { allowFailure: true });
  if (result.status !== 0) return null;
  try { return JSON.parse(result.stdout).version || null; } catch { return null; }
}

function isAncestor(repo: any, ancestor: any, descendant: any): any  {
  return runGit(repo, ['merge-base', '--is-ancestor', ancestor, descendant], { allowFailure: true }).status === 0;
}

function fileAt(repo: any, ref: any, file: any): any  {
  const result: any = runGit(repo, ['show', `${ref}:${file}`], { allowFailure: true });
  return result.status === 0 ? result.stdout : null;
}

export function checkReleaseAuthorityEvidence({ evidence, sourceCommit, workflowSource, nowMs = Date.now() }: any): any  {
  const findings: any[] = [];
  ...
  const observedAtMs: any = Date.parse(evidence.observedAt ?? '');
  const evidenceAgeMs: any = nowMs - observedAtMs;
  ...
  const workflowDigest: any = typeof workflowSource === 'string' ? sha256(workflowSource) : null;
  ...
  const github: any = evidence.github;
  const expectedGithub: any = {
  ...
  const expiresMs: any = Date.parse(evidence.npm?.exchange?.expires ?? '');
...
function releaseTaskRefs(repo: any, version: any): any  {
  const result: any = runGit(repo, ['for-each-ref', '--format=%(refname) %(objectname)', `refs/heads/tasks/release-${version}`, `refs/remotes/*/tasks/release-${version}`]);
  return result.stdout.trim().split('\n').filter(Boolean).map((line: any) => {
    const [ref, commit]: any = line.split(/\s+/);
    return { ref, commit };
  });
}
...
export function checkReleaseConvergence({
  ...
  const findings: any[] = [];
  const checksMain: any = stage === 'post-main' || stage === 'pre-tag';
  const refs: any = {
...
if (process.argv[1] && sameFilesystemPath(process.argv[1], fileURLToPath(import.meta.url))) {
  try {
    const result: any = checkReleaseConvergence(parseArgs(process.argv.slice(2)));
