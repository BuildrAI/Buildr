...
function taskDtoOutputPaths(outputRoot?: string) {
  return {
    backend: contractOutputPaths('task-dto.ts', outputRoot).backend,
    web: contractOutputPaths('task-dto.ts', outputRoot).web,
  };
}
...
export async function renderTaskRecordHttpDto(): Promise<string> {
  const definitions = {
    ...
    TaskEndRequest: body(TASK_HTTP_SCHEMAS.endRequest),
    TaskEndResponse: body(TASK_HTTP_SCHEMAS.endResponse),
  ...
  return [
    ...
    'export type TaskEndResponse = TaskRecordMutationResponse;',
