...
function normalizeSerializable(value, location = '$', seen = new Set()) {
    ...
    if (Array.isArray(value)) {
        if (seen.has(value))
            throw testContextError('test_context_configuration_invalid', 'Context configuration must not contain cycles.', { location });
        seen.add(value);
    ...
    if (plainObject(value)) {
        ...
        for (const key of Object.keys(value).sort()) {
            if (value[key] === undefined)
                throw testContextError('test_context_configuration_invalid', 'Context configuration must not contain undefined values.', { location: `${location}.${key}` });
...
export function defineTestContext(input) {
    if (!plainObject(input))
        throw testContextError('test_context_definition_invalid', 'Context definition must be an object.');
    if (!KEY_PATTERN.test(input.id ?? ''))
        throw testContextError('test_context_definition_invalid', 'Context id must be a stable kebab/dotted identifier.', { id: input.id ?? null });
    if (!Number.isInteger(input.version) || input.version < 1)
        throw testContextError('test_context_definition_invalid', `Context ${input.id} version must be a positive integer.`);
    if (!SCOPES.has(input.scope))
        throw testContextError('test_context_definition_invalid', `Context ${input.id} has an invalid scope.`, { scope: input.scope });
    if (!PARALLEL_SAFETY.has(input.parallelSafety))
        throw testContextError('test_context_definition_invalid', `Context ${input.id} has invalid parallel safety.`, { parallelSafety: input.parallelSafety });
    if (typeof input.create !== 'function')
        throw testContextError('test_context_definition_invalid', `Context ${input.id} is missing create().`);
    for (const hook of ['acquire', 'release', 'reset', 'inspect', 'destroy']) {
        if (input[hook] != null && typeof input[hook] !== 'function')
            throw testContextError('test_context_definition_invalid', `Context ${input.id} ${hook} must be a function.`);
    }
...
export function normalizeContextRequest(value, alias = null) {
    ...
    if (!request || !isTestContextDefinition(request.definition))
        throw testContextError('test_context_request_invalid', 'Context request must reference a definition.', { alias });
