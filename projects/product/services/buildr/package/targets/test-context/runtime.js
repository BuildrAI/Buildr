...
import { canonicalContextConfiguration, contextConfigurationIdentity, normalizeContextRequest, testContextError, } from './definition.js';
...
function inspectDisposition(value) {
    if (value == null || value === true || value === 'clean' || value.status === 'clean')
        return { dirty: false, reason: null };
    if (value === false || value === 'dirty')
        return { dirty: true, reason: 'provider-inspection' };
    if (value?.dirty === true || value?.status === 'dirty')
        return { dirty: true, reason: value.reason ?? 'provider-inspection' };
...
function scopeIdentity(definition, owner) {
    ...
    if (definition.scope === 'suite') {
        if (!owner.suiteId)
            throw testContextError('test_context_scope_identity_missing', `Suite Context ${definition.key} requires suiteId.`);
        return `suite:${owner.suiteId}`;
    }
    if (!owner.testId)
        throw testContextError('test_context_scope_identity_missing', `Test Context ${definition.key} requires testId.`);
...
export function createTestContextRuntime(options = {}) {
    ...
    const ensureEntry = async (definition, config, owner) => {
        if (closed)
            throw testContextError('test_context_runtime_closed', 'Test Context Runtime is closed.');
        ...
        if (typeof sourceIdentity !== 'string' || !sourceIdentity)
            throw testContextError('test_context_source_identity_invalid', `${definition.key} returned an invalid source identity.`);
    ...
    const waitForEntry = async (entry) => {
        if (entry.definition.parallelSafety !== 'exclusive' || entry.active === 0)
            return 0;
        const startedAt = Date.now();
    ...
    const activateEntry = async (entry) => {
        ...
        entry.active += 1;
        if (waitMs > 0)
            record({ operation: 'wait', context: entry.definition.key, cacheKey: entry.cacheKey, identity: entry.identity, durationMs: waitMs });
    };
    const deactivateEntry = (entry) => {
        entry.active -= 1;
        if (entry.active < 0)
            throw testContextError('test_context_lease_invalid', `${entry.definition.key} active lease count became negative.`);
        if (entry.active === 0)
            entry.waiters.shift()?.();
    };
    const destroyEntry = async (entry, reason) => {
        if (entry.destroyed)
            return;
        if (entry.active !== 0)
            throw testContextError('test_context_destroy_active', `Cannot destroy active Context ${entry.definition.key}.`, { active: entry.active });
        const startedAt = Date.now();
        entry.destroyed = true;
        entries.delete(entry.cacheKey);
        if (entry.dirty)
            record({ operation: 'evict', context: entry.definition.key, cacheKey: entry.cacheKey, identity: entry.identity, reason });
        try {
    ...
    const acquire = async (requests, owner = {}) => {
        ...
        if (normalized.length === 0)
            throw testContextError('test_context_request_invalid', 'At least one Context request is required.');
        if (new Set(normalized.map((item) => item.alias)).size !== normalized.length)
            throw testContextError('test_context_request_invalid', 'Context request aliases must be unique.');
        const requested = [];
        for (const request of normalized)
            requested.push({ request, entry: await ensureEntry(request.definition, request.config, owner) });
        const reachableEntries = new Map();
        const collectEntry = (entry) => {
            for (const dependency of entry.dependencies)
                collectEntry(dependency);
            reachableEntries.set(entry.cacheKey, entry);
        };
        for (const item of requested)
            collectEntry(item.entry);
        ...
        const lease = {
            ...
            markDirty(alias, reason = 'test-marked-dirty') {
                ...
                if (!item)
                    throw testContextError('test_context_dirty_alias_unknown', `Unknown Context alias: ${alias}`);
                item.dirty = true;
            ...
            async release(releaseOptions = {}) {
                ...
                for (const item of [...acquired].reverse()) {
                    ...
                    try {
                        await item.entry.definition.release?.(hookContext);
                    }
                    catch (error) {
                        failures.push(error);
                        item.dirty = true;
                        item.dirtyReason ??= 'release-failed';
                    }
                    try {
                ...
                for (const entry of [...lockEntries].reverse()) {
                    ...
                    if (entry.definition.scope === 'test' || entry.dirty) {
                        try {
                            await destroyEntry(entry, entry.dirty ? entry.dirtyReason ?? 'dirty' : 'test-scope-complete');
                        }
                        catch (error) {
                            failures.push(error);
                        }
                ...
                if (failures.length === 1)
                    throw failures[0];
                if (failures.length > 1)
                    throw new AggregateError(failures, 'Test Context release failed.');
    ...
    const closeSuite = async (suiteId) => {
        ...
        for (const entry of selected)
            await destroyEntry(entry, 'suite-close');
        return Object.freeze({ status: 'closed', suiteId, destroyed: selected.length });
    };
    const close = async () => {
        if (closed)
            return Object.freeze({ status: 'already-closed', events: Object.freeze([...events]) });
        const failures = [];
        for (const lease of [...activeLeases]) {
            try {
                await lease.release({ outcome: 'runtime-close' });
            }
            catch (error) {
                failures.push(error);
            }
        ...
        for (const entry of selected) {
            if (entry.destroyed)
                continue;
            try {
                await destroyEntry(entry, 'runtime-close');
            }
            catch (error) {
                failures.push(error);
            }
        ...
        if (failures.length === 1)
            throw failures[0];
        if (failures.length > 1)
            throw new AggregateError(failures, 'Test Context Runtime close failed.');
