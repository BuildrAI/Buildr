...
- 面向用户说明或回复时，应使用直接、简练、易于理解的表达。专业术语可以使用中文，也可以使用中英文并列；凡使用英文专业术语，必须同时提供对应的中文名称或中文含义，采用“中文（English Term）”形式，不得单独使用英文专业术语。没有稳定中文译名时，应提供准确的中文释义，并在括号内保留英文原词。同一描述范围内的中文名称或中文释义必须保持一致。命令、代码标识、字段名、接口名、文件路径、错误原文和产品专名等必须精确对应原文的内容可以保留英文；但将其作为专业概念向用户说明时，仍须补充中文说明。
