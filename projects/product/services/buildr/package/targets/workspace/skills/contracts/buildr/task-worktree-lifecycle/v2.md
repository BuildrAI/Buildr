...
provider 必须为 environment 和每个 repository 返回可核对 identity，并用 environment receipt、membership、allowed roots、checkout-local CLI/runtime projection 与明确 target/workdir 判断 `executionReady`。session root 不要求等于 environment root。Adapter activation 只在 Rules、Skills/runtime adapter 变更的专项验收中检查；`worktree adopt` evidence 是 agent-attested，不代表 Buildr 内省或自动 handoff。
