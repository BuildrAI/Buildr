...
DROP TABLE task_retrospective_sources;
...
CREATE TABLE tasks (
  ...
  schema_version TEXT NOT NULL CHECK (schema_version = 'buildr.task-record/v3'),
...
INSERT INTO tasks(
  ...
  result_no_change, created_at, updated_at, parent_task_id, is_parent,
  parent_completion_json, result_history_json, legacy_parent_plan_json,
  retrospective_state, retrospective_document_digest
)
SELECT
  task_id, 'buildr.task-record/v3', title, intent, status, result_summary,
  result_no_change, created_at, updated_at, parent_task_id, is_parent,
  parent_completion_json, result_history_json, legacy_parent_plan_json,
  NULL, NULL
FROM tasks_v2;
