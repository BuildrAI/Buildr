...
function normalizeFiles(files, cwd) {
  ...
  for (const item of result) {
    ...
    if (item.signature != null && typeof item.signature !== 'string') throw new Error('node_test_context_runner_input_invalid: file signature must be a string.');
  }
...
export async function runNodeTestContextHosts(options) {
  ...
  if (!Number.isInteger(requestedWorkers) || requestedWorkers < 1) throw new Error('node_test_context_runner_input_invalid: workers must be a positive integer.');
