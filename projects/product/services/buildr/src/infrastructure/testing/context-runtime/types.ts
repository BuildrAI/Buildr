...
export interface TestContextCreateHook<Config extends JsonValue, Dependencies extends Readonly<Record<string, unknown>>>
  extends TestContextHookBase<Config> {
...
export interface TestContextDefinition<
  Id extends string = string,
  State = unknown,
  Config extends JsonValue = JsonObject,
  Acquired = State,
> {
  readonly id: Id;
  ...
  readonly sourceIdentity: string | ((context: { config: Config }) => MaybePromise<string>) | null;
}

// The erased definition is used only as a heterogeneous registry constraint.
// Concrete definitions retain State/Config/Acquired inference through the
// conditional helpers below.
export type AnyTestContextDefinition = TestContextDefinition<string, any, any, any>;

export type TestContextDependency<Definition extends AnyTestContextDefinition = AnyTestContextDefinition> =
  | Definition
  | {
...
type DefinitionFromDependency<Value> = Value extends AnyTestContextDefinition
  ? Value
  : Value extends { definition: infer Definition extends AnyTestContextDefinition }
    ? Definition
    : never;
...
export interface TestContextDefinitionInput<
  Id extends string,
  State,
  Config extends JsonValue,
  Acquired,
  Dependencies extends readonly TestContextDependency[],
> {
  id: Id;
  version: number;
  ...
  sourceIdentity?: string | ((context: { config: Config }) => MaybePromise<string>);
}

export type TestContextRequest<Definition extends AnyTestContextDefinition = AnyTestContextDefinition> =
  | Definition
  | {
    definition: Definition;
...
export type CheckedTestContextRequests<Requests extends TestContextRequests> = {
  [Alias in keyof Requests]: Requests[Alias] extends infer Definition extends AnyTestContextDefinition
    ? Definition
    : Requests[Alias] extends { definition: infer Definition extends AnyTestContextDefinition }
      ? Omit<Requests[Alias], 'config'> & { definition: Definition; config?: ConfigOf<Definition> }
      : never;
};
type DefinitionFromRequest<Value> = Value extends AnyTestContextDefinition
  ? Value
  : Value extends { definition: infer Definition extends AnyTestContextDefinition }
    ? Definition
    : never;
...
export interface TestContextLease<Requests extends TestContextRequests> extends TestContextLeaseControl<Requests> {
  ...
  release(options?: { outcome?: TestContextOutcome }): Promise<Readonly<{ status: 'released' | 'already-released' }>>;
}
...
export interface TestContextRuntime {
  acquire<const Requests extends TestContextRequests>(
    requests: Requests & CheckedTestContextRequests<Requests>,
    owner?: TestContextOwner,
  ): Promise<TestContextLease<Requests>>;
  closeSuite(suiteId: string): Promise<Readonly<{ status: 'closed'; suiteId: string; destroyed: number }>>;
  close(): Promise<Readonly<{ status: 'closed' | 'already-closed'; events: readonly TestContextEvent[] }>>;
  record(event: Omit<TestContextEvent, 'sequence' | 'observedAt' | 'pid'>): TestContextEvent;
  events(): readonly TestContextEvent[];
  snapshot(): Readonly<{ closed: boolean; entries: number; activeLeases: number; events: number }>;
}
...
export type ContextTestCallback<Requests extends TestContextRequests> = (
  context: TestContext,
  values: TestContextValues<Requests>,
  control: TestContextLeaseControl<Requests>,
) => unknown | Promise<unknown>;

export interface NodeTestContextAdapter {
  test<const Requests extends TestContextRequests>(
    name: string,
...
export interface NodeTestContextRunResult {
  ...
  readonly events: readonly (TestContextEvent & { host: number })[];
}
