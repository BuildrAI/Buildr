...
export function applyCanonicalPlan({ projectRoot, plan, currentDeltaDigest, currentExecutableIdentity, io }: any) {
  ...
  const committed: any[] = [];
  try {
    for (const item of prepared.filter((entry: any) => entry.changed && entry.expectedExists)) {
  ...
  return {
    status: 'passed',
    effects: prepared.filter((item: any) => item.changed).map((item: any) => ({ path: item.path, digest: item.expectedDigest, type: item.expectedExists ? 'updated' : 'deleted' })),
  };
}
