...
export type TaskCommandRuntime = {
  ...
  activateTaskRecord(targetRoot: string, taskId: string): TaskCliResult;
...
function parseTaskRecordCli(action: TaskAction, args: string[]) {
  ...
  const allowedByAction = {
    ...
    update: new Set(['--status', '--reason', '--summary', '--no-change', '--parent-completion', '--title', '--intent', '--parent', '--clear-parent', '--parent-task', '--expected-record', '--retrospective-state', '--retrospective-document-digest', '--clear-retrospective', '--add-project', '--remove-project', '--add-service', '--remove-service', '--add-change', '--remove-change', '--target', '--json']),
  ...
  const boolean = new Set(['--json', '--no-change', '--clear-parent', '--parent-task', '--clear-retrospective']);
