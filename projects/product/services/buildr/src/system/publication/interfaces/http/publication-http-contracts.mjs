...
export const PUBLICATION_HTTP_SCHEMAS = Object.freeze({
...
const json = (id, method, path, request, success) => Object.freeze({
...
export function validatePublicationHttp(schemaId, value, operationId, phase = 'request') {
  const result = PUBLICATION_HTTP_VALIDATORS.validate(schemaId, value);
  if (result.valid) return value;
  const error = new Error(`Publication HTTP ${phase} DTO 不符合契约：${operationId}。`);
...
export function publicationOperation(id) {
  const operation = PUBLICATION_HTTP_OPERATIONS.find((item) => item.id === id);
