...
export async function renderTasks({ root, api, onWorkspace, navigate }) {
  root.innerHTML = `
    ...
    <section class="panel task-create-panel">
      ...
      <form id="task-create-form" class="prompt-grid">
        ...
        <label>Parent Task<select id="task-create-parent"><option value="">无 Parent（独立 Task）</option></select></label>
  ...
  async function load() {
    ...
    const parentSelect = document.getElementById('task-create-parent');
    parentSelect.replaceChildren(new Option('无 Parent（独立 Task）', ''));
    for (const item of data.tasks.filter((item) => item.record.status === 'active')) parentSelect.append(new Option(`${item.record.title} · ${item.record.taskId}`, item.record.taskId));
    ...
    for (const item of data.tasks) {
      ...
      const children = document.createElement('small'); children.textContent = `直接 Child：${item.taskRelations.children.length}`;
  ...
  document.getElementById('task-create-form').addEventListener('submit', async (event) => {
    ...
    try {
      const created = await api('/api/v1/tasks', { method: 'POST', body: JSON.stringify({
        ...
        parentTaskId: document.getElementById('task-create-parent').value || null,
