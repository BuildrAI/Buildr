...
export function sourceRootKind(source) {
  return source?.root === SOURCE_ROOT_ATTACHED ? SOURCE_ROOT_ATTACHED : SOURCE_ROOT_MANAGED;
}

export function normalizeSourceLocation(source, expectedManagedPath, label) {
...
export function resolveSourceRoot(workspaceRoot, source) {
  return sourceRootKind(source) === SOURCE_ROOT_ATTACHED ? source.path : path.resolve(workspaceRoot, source.path);
}

export function sourceIdentity(entityId, source) {
...
export function sourceOwnership(source) {
