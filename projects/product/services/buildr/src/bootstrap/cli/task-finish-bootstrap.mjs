...
function assertInitializedBuildrWorkspace(targetRoot) {
  ...
  const required = [
    ...
    path.join(root, '.buildr', 'workspace.yml'),
    path.join(root, 'projects'),
  ];
  if (required.some((file) => !fs.existsSync(file))) throw new Error(`Target is not an initialized Buildr workspace: ${root}. 请先运行 buildr init。`);
}
