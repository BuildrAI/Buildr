const DEFINITION_FIELDS = new Set(['id', 'requires', 'create']);
const DESCRIPTOR_FIELDS = new Set(['provides', 'contributions', 'lifecycle']);
const CONTRIBUTION_TYPES = Object.freeze(['cli', 'http', 'diagnostics']);

function moduleError(code, message, details = undefined, cause = undefined) {
...
function assertClosedObject(value, allowed, label, code) {
...
function normalizeLifecycle(moduleId, lifecycle) {
...
export function createModuleRegistry({ capabilities = {} } = {}) {
  ...
  const providers = new Map(Object.entries(capabilities));
  const modules = [];
  const moduleIds = new Set();
  const contributionOwners = new Map(CONTRIBUTION_TYPES.map((type) => [type, new Map()]));
  let started = [];

  function install(rawDefinition) {
    ...
    const missing = definition.requires.filter((required) => !providers.has(required));
    ...
    const requires = Object.freeze(Object.fromEntries(definition.requires.map((required) => [required, providers.get(required)])));
  ...
  function contributions(type) {
    ...
    return Object.freeze(modules.flatMap((descriptor) => descriptor.contributions[type]));
  }

  async function stopDescriptors(descriptors) {
    const failures = [];
    for (const descriptor of [...descriptors].reverse()) {
      if (!descriptor.lifecycle) continue;
      try { await descriptor.lifecycle.stop(); }
      catch (error) { failures.push({ module: descriptor.id, error }); }
    }
    return failures;
  }
  ...
  async function start() {
    ...
    try {
      ...
      started = [];
      throw moduleError('bootstrap_module_start_failed', `Bootstrap module lifecycle start failed: ${error.message}`, { cleanupFailures: cleanupFailures.map(({ module, error: failure }) => ({ module, message: failure.message })) }, error);
    }
  }
  ...
  async function stop() {
    ...
    if (failures.length) throw moduleError('bootstrap_module_stop_failed', `Bootstrap module lifecycle stop failed for: ${failures.map(({ module }) => module).join(', ')}.`, { failures: failures.map(({ module, error }) => ({ module, message: error.message })) });
  }

  function snapshot() {
    return Object.freeze(modules.map((descriptor) => Object.freeze({
      id: descriptor.id,
      requires: descriptor.requires,
      provides: Object.freeze(Object.keys(descriptor.provides)),
      contributions: Object.freeze(Object.fromEntries(CONTRIBUTION_TYPES.map((type) => [type, Object.freeze(descriptor.contributions[type].map((item) => item.id))]))),
