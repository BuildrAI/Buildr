...
import { capabilityKey, isSourceLabel, parseSkillsManifestDocument } from './manifests.ts';
...
function readPackageSkillEntries(section: any, runtime: any): any  {
...
function readPackageAgentSkills(runtime: any): any  {
  return readPackageSkillEntries('agentSkills', runtime);
}

function readPackageSkillSources(runtime: any): any  {
...
function hashContent(content: any): any  {
  return crypto.createHash('sha256').update(content).digest('hex');
}

function verifyIntegrity(content: any, integrity: any, label: any): any  {
  if (!integrity) return;
...
function resolveSkillUrl(skill: any, layerOrigin: any): any  {
...
function describeSkillUrl(skill: any, layerOrigin: any): any  {
  return {
    id: skill.id,
    origin: layerOrigin,
    sourceKind: 'resolved',
...
function resolveAgentInstallSkill(skill: any, layerOrigin: any, manifestPath: any): any  {
  return {
    id: skill.id,
    origin: layerOrigin,
...
function resolveReferencedSkill(skill: any, packageSourcesById: any, runtime: any): any  {
...
function loadLayer(manifestPath: any, options: any = {}): any  {
  ...
  return document.skills.map((skill: any) => {
...
export function resolveSkills(organizationRoot: any, projectRoot: any, options: any = {}): any  {
  ...
  const productSkillsById: any = new Map(readPackageAgentSkills(runtime).map((skill: any) => [skill.id, skill]));
  const packageSourcesById: any = new Map(readPackageSkillSources(runtime).map((skill: any) => [skill.id, skill]));
  const resolved: any = new Map();
  ...
  for (const contribution of componentContributions.dependencies) {
    ...
    const existing = target.requires.find((item: any) => capabilityKey(item.capability, item.version) === key);
