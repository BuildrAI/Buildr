...
export function createAgentAssetsCliContributions(): any  {
  return Object.freeze([
    ...
    route({
      ...
      run: (runtime: any, context: any) => runtime.commandsCheck(context.argv.slice(4)),
    }),
    route({
      key: 'commands add',
      ...
      run: (runtime: any, context: any) => runtime.commandsAdd(context.argv.slice(4)),
    }),
    route({
      key: 'commands remove',
      ...
      run: (runtime: any, context: any) => runtime.commandsRemove(context.argv.slice(4)),
    }),
    ...[
      ['component list', '列出 workspace Components。当前不支持 Project 或 Service scope。', 'Usage: buildr component list [--target <dir>] [--json]', (runtime: any, context: any) => runtime.componentListOrCheck(context.argv.slice(4), false)],
      ['component check', '检查 Component definition、成员 integrity 和唯一所有权。', 'Usage: buildr component check [<id>] [--target <dir>] [--json]', (runtime: any, context: any) => runtime.componentListOrCheck(context.argv.slice(4), true)],
      ['component install', '安装 workspace Component，reconcile 指定 Agent runtime，并运行 doctor。', 'Usage: buildr component install <id> --agent <claude-code|codex|cursor|qoder|trae|trae-work|workbuddy> [--target <dir>]', (runtime: any, context: any) => runtime.componentInstall(context.argv.slice(4))],
      ['component uninstall', '卸载 workspace Component 及其受管源资产；不会卸载外部 CLI，也不会删除 Project 内容。', 'Usage: buildr component uninstall <id> --agent <claude-code|codex|cursor|qoder|trae|trae-work|workbuddy> [--target <dir>] [--reason <text>]', (runtime: any, context: any) => runtime.componentUninstall(context.argv.slice(4))],
    ].map(([key, summary, usage, run]: any) => route({ key, summary, usage, match: ({ domain, action }: any) => domain === 'component' && action === key.split(' ')[1], run })),
    ...[
      ['rules add', '注册已存在的 root Rule 文件到 rules/manifest.yml。未传 --path 时默认使用 rules/<id>.md。', 'Usage: buildr rules add <id> [--path <rules/file.md>] --description <text> [--target <dir>] [--replace]', (runtime: any, context: any) => runtime.rulesAdd(context.argv.slice(4))],
      ['rules remove', '删除 root Rule 登记和规则文件。传入 --keep-file 时只取消注册并保留文件。', 'Usage: buildr rules remove <id> [--target <dir>] [--keep-file]', (runtime: any, context: any) => runtime.rulesRemove(context.argv.slice(4))],
    ...
    route({
      ...
      run: (runtime: any, context: any) => runtime.skillsAdd(context.argv.slice(4)),
    }),
    ...[
      ['skills remove', '删除 workspace Skills 源资产登记。', 'Usage: buildr skills remove <id> [--target <workspace>]', (runtime: any, context: any) => runtime.skillsRemove(context.argv.slice(4))],
      ['skills bind', '显式选择当前 scope 的 capability provider；不会安装 Skill 或证明其行为正确。', 'Usage: buildr skills bind <capability>@<version> --provider <skill-id> --scope <.|projects/project> [--target <dir>]', (runtime: any, context: any) => runtime.skillsBind(context.argv.slice(4))],
      ['skills unbind', '删除当前 scope 的显式 binding，由 resolver 重新判断唯一 provider、歧义或缺失。', 'Usage: buildr skills unbind <capability>@<version> --scope <.|projects/project> [--target <dir>]', (runtime: any, context: any) => runtime.skillsUnbind(context.argv.slice(4))],
