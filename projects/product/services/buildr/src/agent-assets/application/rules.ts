...
export function registerDomainsRules(runtime: any): any  {
  ...
  function rulesManifestPath(scopeRoot: any): any  {
    return path.join(scopeRoot, 'rules', 'manifest.yml');
  }

  function parseRulesManifestYaml(content: any): any  {
    const manifest = parseYamlDocument(content, 'rules/manifest.yml');
    return manifest;
  }

  function renderRulesManifestYaml(manifest: any): any  {
    const lines: any[] = ['schemaVersion: buildr.rules/v1'];
  ...
  function validateRulesManifest(manifest: any): any  {
    const errors: any[] = [];
    ...
    const allowedKeys: any = new Set(['id', 'source', 'path', 'description', 'enabled', 'required', 'state', 'reason']);
  ...
  function readRulesManifestForWrite(scopeRoot: any): any  {
  ...
  function writeRulesManifest(scopeRoot: any, manifest: any): any  {
    const manifestPath = rulesManifestPath(scopeRoot);
    atomicWriteFile(manifestPath, renderRulesManifestYaml(manifest), 'utf8');
    return manifestPath;
  }

  function resolveRootRulesScope(args: any): any  {
  ...
  function buildRuleEntry(id: any, args: any, targetRoot: any): any  {
  ...
  function printRulesMutationReceipt(action: any, targetRoot: any, id: any, updatedPaths: any, nextAction: any): any  {
  ...
  function rulesAddUnsafe(args: any): any  {
    const allowedFlags: any = new Set(['--target', '--scope', '--path', '--description', '--replace']);
  ...
  function rulesAdd(args: any): any  {
    const targetRoot = path.resolve(optionValue(args, '--target', process.cwd()));
    return withWorkspaceMutation(targetRoot, 'rules.add', [path.join(targetRoot, 'rules')], () => rulesAddUnsafe(args));
  }

  function rulesRemoveUnsafe(args: any): any  {
    const allowedFlags: any = new Set(['--target', '--scope', '--keep-file']);
  ...
  function rulesRemove(args: any): any  {
    const targetRoot = path.resolve(optionValue(args, '--target', process.cwd()));
