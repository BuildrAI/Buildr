...
export function selectPackageVerifiers(selector: any = ''): any  {
  if (!selector) return [...PACKAGE_VERIFIERS];
  const requested = selector.split(',').map((value: any) => value.trim()).filter(Boolean);
  const unknown = requested.filter((id: any) => !PACKAGE_VERIFIERS.some((step: any) => step.id === id));
  if (unknown.length > 0) throw new Error(`Unknown package verifier: ${unknown.join(', ')}`);
  return PACKAGE_VERIFIERS.filter((step: any) => requested.includes(step.id));
}
