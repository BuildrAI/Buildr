...
export function createBuiltinLifecycle(deps: any): any  {
  ...
  function packageBuiltinComponent(id: any): any  {
    ...
    for (const kind of ['rules', 'skills', 'commands']) {
      const builtin = manifest.builtins[kind].find((item: any) => item.id === id);
      if (builtin?.component) return builtin.component;
    }
    return null;
  }

  function findBuiltinManifestEntry(targetRoot: any, id: any): any  {
    const rulesManifest = readRulesManifestForWrite(targetRoot);
    const ruleIndex = rulesManifest.rules.findIndex((rule: any) => rule.id === id && rule.source === 'buildr');
    ...
    const skillIndex = skillsManifest.findIndex((skill: any) => skill.id === id && skill.source === 'buildr');
    ...
    const commandIndex = commandsManifest.commands.findIndex((command: any) => command.id === id && command.source === 'buildr');
  ...
  function discloseBuiltinSkillImpact(targetRoot: any, id: any): any  {
  ...
  function builtinUninstallUnsafe(args: any): any  {
    ...
    const updated: any = { ...found.entry, enabled: false, state: 'uninstalled' };
    if (reason) updated.reason = reason;
    const changed: any[] = [];
    const receipts = readBuiltinReceipts(targetRoot);
    const receiptIndex = receipts.builtins.findIndex((item: any) => item.type === found.type && item.id === id);
    if (receiptIndex !== -1) {
  ...
  function builtinRestoreUnsafe(args: any): any  {
    const allowedFlags: any = new Set(['--target']);
    assertNoUnknownOptions(args, allowedFlags);
    const [id] = positionalArgs(args);
    if (!id) throw new Error('Missing builtin id');
    const targetRoot = path.resolve(optionValue(args, '--target', process.cwd()));
    ...
    const outcome = result.restoreOutcomes.find((item: any) => item.id === id) || null;
    if (!result.findings.some((finding: any) => finding.id === id)) throw new Error(`Buildr builtin not found in package: ${id}`);
    if (!outcome || outcome.status !== 'restored') {
      const reason = outcome?.reason || result.findings.find((finding: any) => finding.id === id)?.reason || 'restore did not establish the requested Builtin state';
  ...
  function builtinRestore(args: any): any  {
    const targetRoot = path.resolve(optionValue(args, '--target', process.cwd()));
    assertInitializedBuildrWorkspace(targetRoot);
    const [id] = positionalArgs(args);
    if (!id) throw new Error('Missing builtin id');
    ...
    const finding = preflight.findings.find((item: any) => item.id === id) || null;
    const outcome = preflight.restoreOutcomes.find((item: any) => item.id === id) || null;
