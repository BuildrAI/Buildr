...
function portableRun(run) {
  ...
  const result = {
    ...
    repositories: (run?.repositories || []).map((repository) => ({
      ...
      disposition: repository.disposition || null,
      taskContributionIdentity: repository.taskContribution?.identity || null,
      ...
      cleanupProofIdentity: repository.cleanupProof?.identity || null,
    })),
