...
type TaskModuleRequires = {
  [WORKSPACE_TASK_SUPPORT]: Pick<TaskQueryApplicationRuntime & TaskCommandApplicationRuntime,
    'assertCanonicalStructuredWorkspace' | 'prepareWorkspaceStructuredStore' | 'runWorkspaceSqliteRead' |
...
export function createWorktreeProviderModule(runtime: GitWorktreeRuntime) {
  return Object.freeze({
    ...
    requires: Object.freeze([TASK_QUERY_APPLICATION, WORKSPACE_QUERY]),
    create(requires: RuntimeRequires) {
      const composition = Object.assign(Object.create(runtime), requires[TASK_QUERY_APPLICATION], requires[WORKSPACE_QUERY]) as GitWorktreeProviderRuntime;
      ...
      return Object.freeze({
        provides: {
          [TASK_WORKTREE_PROVIDER]: application,
        },
        contributions: { cli: createGitWorktreeCliContributions(application as GitWorktreeCliRuntime) },
      });
    },
  });
}

function taskVerificationCliContributions(application: TaskVerificationCliRuntime | null = null) {
  return Object.freeze([
    {
      ...
      run: (runtime: TaskVerificationCliRuntime, context: CliContext) => taskVerificationCommand(application || runtime, 'inspect', context.argv.slice(5)),
    },
    {
      ...
      run: (runtime: TaskVerificationCliRuntime, context: CliContext) => taskVerificationCommand(application || runtime, 'record', context.argv.slice(5)),
    },
  ].map(Object.freeze));
}

function parentCoordinationCliContributions(application: ParentCoordinationCliRuntime | null = null) {
  return Object.freeze([Object.freeze({
    ...
    run: (runtime: ParentCoordinationCliRuntime, context: CliContext) => parentCoordinationCommand(application || runtime, context.argv.slice(5)),
  })]);
}
...
function createTaskModule(requires: TaskModuleRequires) {
  let changeResolver: DynamicRuntime | null = null;
  const privateComposition = Object.assign({}, requires[WORKSPACE_TASK_SUPPORT], {
    resolveTaskScopedChange: (...args: unknown[]) => {
      const method = changeResolver?.resolveTaskScopedChange;
      if (typeof method !== 'function') throw new Error('Task Change resolver has not been bound.');
      return Reflect.apply(method as (...values: unknown[]) => unknown, changeResolver, args);
    },
  });
  ...
  const changeBinder = Object.freeze({
    bindChangeResolver(port: DynamicRuntime) {
      if (changeResolver) throw new Error('Task Change resolver is already bound.');
      if (typeof port?.resolveTaskScopedChange !== 'function') throw new Error('Task Change resolver capability is invalid.');
      changeResolver = port;
    },
  });
  return Object.freeze({
...
export const TASK_MODULE = Object.freeze({
  id: TASK_MODULE_ID,
  requires: Object.freeze([WORKSPACE_TASK_SUPPORT]),
  ...
  return Object.freeze({
    ...
    create(requires: RuntimeRequires) {
      ...
      return Object.freeze({
        ...
        contributions: {
          cli: taskVerificationCliContributions(application as TaskVerificationCliRuntime),
  ...
  return Object.freeze({
    ...
    create(requires: RuntimeRequires) {
      ...
      return Object.freeze({
        ...
        contributions: {
          cli: parentCoordinationCliContributions(application as ParentCoordinationCliRuntime),
