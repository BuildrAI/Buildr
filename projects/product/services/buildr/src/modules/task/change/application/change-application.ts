...
export function registerChangeApplication(runtime: ChangeRuntime, options: ChangeApplicationOptions = {}) {
  ...
  function listProjectChanges(targetRoot: string, projectCode: string): { project: { id: string; code: string; name: string }; changes: ChangeModel[] } {
    ...
    const active = readDirectories(changesRoot)
      ...
      .map((directory) => buildChange(requiredProjectQuery, targetRoot, project, directory, 'active', false, requiredOpenSpecQuery.inspectChangeChecklist));
    const archived = readDirectories(path.join(changesRoot, 'archive'))
      .map((directory) => buildChange(requiredProjectQuery, targetRoot, project, directory, 'archived', false, requiredOpenSpecQuery.inspectChangeChecklist));
    return {
      project: { id: project.id, code: project.code, name: project.name },
  ...
  function changeDetail(targetRoot: string, projectCode: string, ref: string): { change: ChangeModel } {
    ...
    const change = buildChange(requiredProjectQuery, targetRoot, project, directory, lifecycle, true, requiredOpenSpecQuery.inspectChangeChecklist);
