...
export function workContextCommand(application: TaskWorkContextApplication, args: string[]) {
  ...
  const allowed = new Set(['--target', '--json', '--expected-current', '--progress', '--next-step', '--attention-kind', '--attention-reason', '--clear-attention', '--attention', '--response']);
  const values = new Map<string, string>();
  for (let index = 0; index < rest.length; index += 1) {
    const name = rest[index];
    if (!allowed.has(name) || values.has(name)) throw new Error(`Unknown or duplicate argument: ${name}`);
    if (name === '--json' || name === '--clear-attention') values.set(name, 'true');
...
export function createWorkContextCliContributions(application: TaskWorkContextApplication) {
  return ['inspect', 'record', 'respond'].map((action) => Object.freeze({
    ...
    help: ['Usage: buildr task work-context <inspect|record|respond> <task-id> [--expected-current <absent|sha256>] [--progress <text> --next-step <text>] [--attention-kind <decision|acceptance|question> --attention-reason <text> | --clear-attention] [--attention <id> --response <text>] [--target <workspace>] [--json]', '', '写入必须提供已观察版本；省略事项保留原请求与答复，明确新请求产生新身份。回应不代表任务完成或通用授权。'],
