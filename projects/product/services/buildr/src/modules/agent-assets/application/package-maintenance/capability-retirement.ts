export function createCapabilityRetirement({ assertSafeAssetTarget, crypto, existsFile, fs, path, removePath }: any): any  {
  function applyCapabilityRetirements({ targetRoot, manifest, contracts, bindings, findings, changed, checkOnly }: any): any  {
    const retirements = (manifest.capabilityContracts || []).flatMap((contract: any) => contract.replaces || []);
    const currentKeys: any = new Set((manifest.capabilityContracts || []).map((contract: any) => `${contract.id}@${contract.version}`));
    const replacementKeys: any = new Set(retirements.map((retirement: any) => `${retirement.id}@${retirement.version}`));
    const currentProviders: any = new Set((manifest.builtins?.skills || []).map((skill: any) => skill.id));
    const orphanKeys: any = new Set(bindings
      .filter((binding: any) => binding.capability.startsWith('buildr.') && !currentProviders.has(binding.provider))
      .map((binding: any) => `${binding.capability}@${binding.version}`)
      .filter((key: any) => !currentKeys.has(key) && !replacementKeys.has(key)));

    for (const key of orphanKeys) {
      const contractMatches = contracts.filter((item: any) => `${item.id}@${item.version}` === key);
      const bindingMatches = bindings.filter((item: any) => `${item.capability}@${item.version}` === key);
    ...
    for (const retirement of retirements) {
      const contractMatches = contracts.filter((item: any) => item.id === retirement.id && item.version === retirement.version);
      const bindingMatches = bindings.filter((item: any) => item.capability === retirement.id && item.version === retirement.version);
      ...
      if (targetExists) {
        ...
        const acceptedIntegrities: any = new Set([retirement.integrity, ...(retirement.legacyIntegrities || [])]);
