  ...
  for (const skill of skills) {
    ...
    if (skill.installMode === 'agent') {
      for (const root of roots) {
        addWrite(byTarget, {
          targetFile: buildAgentInstallPlanTarget(targetRoot, skill, runtime, root),
  ...
  for (const projection of byRuntimePath.values()) {
    for (const root of roots) {
      ...
      const scope = root === primaryRoot ? projection : {
        ...
        writes: projection.writes.map((item: any) => ({ ...item, targetFile: path.join(rootTargetDir, ...item.skillRelativePath.split('/')) })),
      };
      ...
      for (const previous of previousByPath.values()) {
        ...
        removals.push({
          targetFile: path.join(scope.targetDir, ...previous.path.split('/')),
          expectedIntegrity: previous.integrity,
          expectedExecutable: previous.executable,
          pruneEmptyRoot: scope.targetDir,
      ...
      const inventory = scope.writes.map((item: any) => ({
        path: item.skillRelativePath,
