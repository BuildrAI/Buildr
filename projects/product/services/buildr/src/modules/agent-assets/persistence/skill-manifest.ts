...
import { ensureFile } from '../infrastructure/runtime/skills/primitives.ts';
import { capabilityKey, validateCapabilityIdentity } from '../domain/capability-identity.ts';

export { capabilityKey, validateCapabilityIdentity } from '../domain/capability-identity.ts';
...
function stableUuid(value: any): any  {
...
export function sourceIdentityForSkill(skill: any, workspaceId: any): any  {
...
export function assetIdentityForSkill(skill: any, workspaceId: any): any  {
  return skill.assetIdentity || `workspace:${workspaceId}:skill:${skill.id}`;
}

function parseYaml(content: any, label: any): any  {
  try {
...
export function parseCapabilityContract(contractPath: any, assertion: any = null): any  {
...
function validateDeclarations(items: any, label: any, modeRequired: any = false): any  {
...
export function validateSkillsManifestDocument(document: any, manifestPath: any, options: any = {}): any  {
  ...
  for (const [index, skill] of (document.skills || []).entries()) {
    ...
    if (skill.runtimes !== undefined && (!Array.isArray(skill.runtimes) || !skill.runtimes.every((item: any) => typeof item === 'string'))) throw new Error(`skills[${index}].runtimes must be an array of strings in ${manifestPath}`);
...
export function migrateSkillsManifestDocument(document: any, options: any = {}): any  {
  ...
  return {
    ...
    workspaceId,
    skills: (Array.isArray(document.skills) ? document.skills : []).map((skill: any) => ({
      ...skill,
...
export function parseProjectCapabilities(file: any): any  {
...
export function parseSkillsManifest(manifestPath: any, options: any = {}): any  {
  return parseSkillsManifestDocument(manifestPath, options).skills;
}

export function parseBooleanScalar(value: any): any  {
  if (value === 'true' || value === true) return true;
  if (value === 'false' || value === false) return false;
  throw new Error(`Expected boolean scalar, got: ${value}`);
}

export function isSourceLabel(value: any): any  {
