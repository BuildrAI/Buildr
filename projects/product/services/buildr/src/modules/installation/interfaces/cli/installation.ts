  ...
  return Object.freeze([
    {
      ...
      run: (runtime: any, context: any) => (application || runtime).installationStatus(context.argv.slice(4)),
    },
    {
      key: 'update check',
      surface: 'primary',
      ...
      run: (runtime: any, context: any) => (application || runtime).updateCheck(context.argv.slice(4)),
    },
    {
      key: 'update',
      surface: 'primary',
      ...
      run: (runtime: any, context: any) => (application || runtime).updateBuildr(context.argv.slice(3)),
    },
  ].map(Object.freeze));
}
