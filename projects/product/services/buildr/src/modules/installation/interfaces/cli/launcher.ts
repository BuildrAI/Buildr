...
export function registerLauncherInterface(runtime: any) {
  function manageBuildrWebLauncher(action: any, args: any) {
    ...
    assertNoUnknownOptions(args, new Set(['--target', '--platform', '--json', ...(supportsPort ? ['--port'] : [])]), new Set(['--json']));
    const rawPort = supportsPort && args.includes('--port') ? runtime.optionValue(args, '--port') : undefined;
...
export function createLauncherCliContributions(application: any = null) {
  return Object.freeze([
    ...
    },
    {
      ...
      run: (runtime: any, context: any) => (application || runtime).manageBuildrWebLauncher('status', context.argv.slice(5)),
    },
    {
      ...
      run: (runtime: any, context: any) => (application || runtime).manageBuildrWebLauncher('repair', context.argv.slice(5)),
    },
    {
      ...
      run: (runtime: any, context: any) => (application || runtime).manageBuildrWebLauncher('uninstall', context.argv.slice(5)),
    },
  ].map(Object.freeze));
}
